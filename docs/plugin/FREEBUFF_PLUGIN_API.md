...
```json
{
  "status": "ended",
  "session_id": "a1b2c3d4",
  "conspect_path": "/storage/.../context/summaries/conspect_topic_2026-07-29.md"
***REMOVED***
```
...
**STDIO (рекомендуется):**
```bash
python3 freebuff_plugin/mcp_server.py --transport stdio
```
...
```bash
python3 freebuff_plugin/wrapper.py launch "напиши парсер" --cwd . --timeout 300
python3 freebuff_plugin/wrapper.py run "тест" --timeout 120       # синхронно (отладка)
python3 freebuff_plugin/wrapper.py status                          # активные сессии
```

### 3.2 Bridge CLI

```bash
python3 freebuff_plugin/bridge.py start                           # → session_id
python3 freebuff_plugin/bridge.py end a1b2c3d4 --summary "done"   # → конспект
```

### 3.3 Scenario Engine CLI

```bash
python3 freebuff_plugin/scenario_engine.py list                      # все сценарии
python3 freebuff_plugin/scenario_engine.py list --category freelancing
python3 freebuff_plugin/scenario_engine.py list --tag telegram
python3 freebuff_plugin/scenario_engine.py get freelance_parser      # детали
python3 freebuff_plugin/scenario_engine.py search telegram            # поиск
python3 freebuff_plugin/scenario_engine.py apply freelance_parser \  # применить
  --vars '{"URL":"https://example.com"***REMOVED***'
python3 freebuff_plugin/scenario_engine.py reload                     # перезагрузка
```

### 3.4 Router CLI

```bash
python3 freebuff_plugin/router.py "напиши парсер"                    # одиночный запрос
python3 freebuff_plugin/router.py -i                                  # интерактивный режим
```

### 3.5 OOM Protection

```bash
bash scripts/oom_protect.sh --status    # статус памяти и процессов
bash scripts/oom_protect.sh --force     # принудительная очистка
bash scripts/oom_protect.sh             # автоматическая проверка+очистка
```
