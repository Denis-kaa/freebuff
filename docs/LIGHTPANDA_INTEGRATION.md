...
└── src/workers/lightpanda_worker.py  ← новый компонент
...
Lightpanda имеет native MCP server. Для включения в `scripts/mcp_server.py` добавьте:
...
python -m pytest tests/test_lightpanda_worker.py -v
```
