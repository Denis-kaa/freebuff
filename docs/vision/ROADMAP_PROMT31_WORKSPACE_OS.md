...
**Date:** 2026-07-31  
**Status:** Draft  
**Source:** `pompts/promt27.md` — `promt31.md`  
**Companion:** `docs/audits/ARCHITECTURAL_AUDIT_PROMT31_2026-07-31.md`

---

## 1. Principles

This roadmap implements the ideas from `promt27.md` (Project Book / StoryBook), `promt28.md` (Notification Framework), `promt29.md` (Integration Research & Registry), `promt30.md` (Knowledge Management System), and `promt31.md` (Architectural Audit).

**Guiding rule from `promt31.md`:**

> *Reuse First. Extend Second. Create Last.*

Therefore:
- No new engines unless existing ones cannot be extended.
- All new documentation artifacts reuse `scripts/engineering_memory.py`, `scripts/event_bus.py`, and existing Markdown templates.
- Every new module must be registered in `docs/core/SYSTEM_INVENTORY.md`.
- Every new integration must be registered in `docs/core/INTEGRATION_REGISTRY.md`.
...
**Deliverables:**
- `docs/core/KNOWLEDGE_POLICY.md`
- Refactored `scripts/knowledge_engine.py` (absorbs vector backend)
- `scripts/engineering_memory.py` auto-indexes records
...
|---|---|
| **Status** | ✅ Exists |
| **Current** | `docs/engineering-memory/PROJECT_BOOK.md` |
...
**Steps:**
1. Add a `compile_project_book()` function in `scripts/engineering_memory.py`.
2. Generate/append chapters from EM Milestone Chronicles after each version bump.
3. Add a CLI command `python scripts/engineering_memory.py compile-project-book`.
4. Link `PROJECT_BOOK.md` from `docs/INDEX.md` and `AGENTS.md`.

**Deliverables:**
- `scripts/engineering_memory.py :: compile_project_book()`
- Updated `docs/engineering-memory/PROJECT_BOOK.md`
...
|---|---|
| **Status** | 🟡 Partial |
| **Current** | EM templates in `docs/engineering-memory/templates/task_retrospective.md` |
...
2. Add EventBus subscriber in `scripts/event_subscribers.py` or `engineering_memory.py`.
3. Create a draft in `MemoryEngine.WORKING`.
4. Promote to `docs/engineering-memory/retrospectives/` on human/AI approval.
5. Add tests in `tests/test_engineering_memory.py`.

**Deliverables:**
- `scripts/engineering_memory.py` auto-retro subscriber
- `tests/test_engineering_memory.py` retro trigger tests
- New retrospectives in `docs/engineering-memory/retrospectives/`
...
**Steps:**
1. Convert each entry in `DECISIONS.md` to a Decision Journal file under `docs/engineering-memory/decisions/`.
...
| **Decision** | **Create.** Add `docs/core/ARCHITECTURAL_DEBT.md` populated by `drift_check.py`. |
...
3. Create `docs/core/ARCHITECTURAL_DEBT.md` and seed it from current drift findings.
4. Run `drift_check.py` in CI to keep the log fresh.

**Deliverables:**
- `docs/core/ARCHITECTURAL_DEBT.md`
- `scripts/drift_check.py` debt-report mode
...
|---|---|
| **Status** | 🟡 Partial |
| **Current** | `docs/core/SYSTEM_INVENTORY.md`, `docs/core/PROJECT_REGISTRY.md`, `docs/projects_meta/FILE_REGISTRY.md` |
| **Gap** | Manual, gets outdated, no freshness check |
| **Decision** | **Extend.** Generate `docs/core/MODULE_REGISTRY.md` from code + existing docs. |

**Steps:**
1. Create a registry schema: module name, file path, owner, status, dependencies, related tests, related docs.
2. Add a generator script (e.g., `scripts/generate_module_registry.py`) that scans `scripts/`, `core/`, `freebuff_plugin/`, `plugins/`.
...
|---|---|
| **Status** | 🟡 Partial |
| **Current** | `AGENTS.md`, `scripts/roles.py`, `scripts/presence.py`, `freebuff_plugin/acp_protocol.py` |
| **Gap** | No unified registry of agents, roles, and capabilities |
| **Decision** | **Extend.** Create `docs/core/AGENT_REGISTRY.md` synced with `roles.py` and `AgentMesh`. |
...
3. Add helper to export `AgentMesh` agents from `scripts/distributed_agents.py`.
4. Link `AGENT_REGISTRY.md` in `AGENTS.md`.

**Deliverables:**
- `docs/core/AGENT_REGISTRY.md`
- Export helpers in `scripts/roles.py` and `scripts/distributed_agents.py`
...
|---|---|
| **Status** | 🔴 Missing |
| **Current** | `runtime/providers/`, `plugins/`, `MARKETPLACE.md`, `INTEGRATION_CONTRACT.md` |
| **Gap** | No single map of all external integrations |
| **Decision** | **Create.** Add `docs/core/INTEGRATION_REGISTRY.md` listing all integrations with status and owner. |
...
4. Link registry from `docs/INDEX.md`.

**Deliverables:**
- `docs/core/INTEGRATION_REGISTRY.md`
- Integration checklist in `scripts/doctor.py`
...
**Deliverables:**
- `scripts/orchestrator.py` lifecycle event publishing
- Updated `scripts/project_pulse.py` subscriber
- Lifecycle tests
...
|---|---|
| **Status** | ✅ Exists |
| **Current** | `scripts/notification.py` |
...
**Deliverables:**
- `scripts/engineering_memory.py :: compile_chronicle()`
...
**Steps:**
1. Add `scripts/generate_architecture_map.py`.
2. Read `MODULE_REGISTRY.md`, `AGENT_REGISTRY.md`, `INTEGRATION_REGISTRY.md`.
3. Output Mermaid or ASCII diagram to `docs/core/ARCHITECTURE_MAP.md`.
4. Run on each commit or nightly.

**Deliverables:**
- `scripts/generate_architecture_map.py`
- `docs/core/ARCHITECTURE_MAP.md`
...
1. Extend `scripts/notification.py` with EventBus progress/stage notifications (Promt 28).
2. Create `docs/core/ARCHITECTURAL_DEBT.md` from current `drift_check.py` findings.
3. Merge `DECISIONS.md` into `docs/engineering-memory/decisions/` (Promt 27 / 31).

### Phase B — Short-term (next 1-3 sessions)

4. Implement EM auto-triggers for `task.completed`, `task.failed`, `git.merge`, `system.error`.
5. Create `docs/core/INTEGRATION_REGISTRY.md`.
6. Generate `docs/core/MODULE_REGISTRY.md` from code.
7. Generate `docs/core/AGENT_REGISTRY.md` from `roles.py` / `AgentMesh`.
...
- `docs/audits/ARCHITECTURAL_AUDIT_PROMT31_2026-07-31.md` — full audit with evidence
- `docs/engineering-memory/ARCHITECTURE.md` — EM architecture
- `docs/engineering-memory/PROJECT_BOOK.md` — project narrative
- `docs/core/SYSTEM_INVENTORY.md` — existing component inventory
- `docs/vision/ROADMAP.md` — technical phase roadmap
