...
graph TB
    subgraph freebuff
        ...
        TR[scripts/tool_runtime.py***REMOVED***
        LW[src/workers/lightpanda_worker.py***REMOVED***
        MS[scripts/mcp_server.py***REMOVED***
        CM[scripts/context_manager.py***REMOVED***
    end
...
| `scripts/tool_runtime.py` | Реестр инструментов с единым интерфейсом |
| `src/workers/lightpanda_worker.py` | Воркер для веб-автоматизации через Lightpanda |
| `scripts/mcp_server.py` | MCP сервер, экспонирующий инструменты |
| `scripts/context_manager.py` | SQLite-хранилище сессий и сообщений |
...
Правила документирования вынесены в `docs/RULES.md` и контролируются автоматически:
...
- Подробнее: [`docs/RULES.md` — Авто-триггер документирования***REMOVED***(RULES.md).
