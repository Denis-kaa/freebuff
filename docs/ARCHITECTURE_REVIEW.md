...
├── projects/                # 4 изолированных проекта
...
| **wingman_agent** | `~/wingman_agent/` | Python: agent.py, core/ (db, ai), dashboard/ | ⭐⭐⭐ |
...
| **termux-ai-agent** | Локальный агент | ⭐⭐⭐⭐⭐ | Активен |
...
LISAGateway (termux-ai-agent) — сложность → локальный/облачный
...
| Компонент | Почему |
|-----------|--------|
| **LEVIATHAN core/** | Роутер, оркестратор, pipeline, key_pool, circuit_breaker — production-grade |
...
**Рекомендация:** Вынести общие абстракции в `freebuff/core/` — единый SDK для всех проектов.
...
### Среднесрочно (Medium)
4. 🟡 **Вынести общие абстракции** в `freebuff/core/` (BaseAgent, Router, Orchestrator)
5. 🟡 **Наполнить phone-agent** — router.py есть, логики нет
6. 🟡 **Интегрировать Qwen memories** с ContextManager

### Стратегически
7. 🟢 **Единый SDK (`freebuff/core/`)** — устранить 5+ дублей SmartRouter
