...
## 1. ContextManager Bridge (`scripts/agent_context_bridge.py`)

### Что делает
Singleton-мост, который позволяет `termux-ai-agent` сохранять каждый диалог в `freebuff/data/context.db`.
...
4. `auto_conspect()` завершает сессию и сохраняет конспект в `context/summaries/`.

### Ключевые файлы
- `scripts/agent_context_bridge.py`
- `termux-ai-agent/main.py`
- `scripts/context_manager.py`

---

## 2. Lightpanda Worker (`src/workers/lightpanda_worker.py`)
...
### Ключевые файлы
- `src/workers/lightpanda_worker.py`
- `scripts/install_lightpanda.sh`
- `docs/LIGHTPANDA_INTEGRATION.md`
...
В будущем `LightpandaWorker` может быть зарегистрирован как tool `browser` в `scripts/tool_runtime.py`:
...
```bash
python -m pytest tests/test_agent_context_bridge.py tests/test_lightpanda_worker.py -v
```
