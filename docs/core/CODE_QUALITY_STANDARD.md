...
> **Основание:** [pompts/CODE_QUALITY_STANDART.md***REMOVED***(../../pompts/CODE_QUALITY_STANDART.md) (v1.0), [promt16.md***REMOVED***(../../pompts/promt16.md)
> **Область действия:** `scripts/`, `freebuff_plugin/`, `projects/`, `runtime/`, `cli/`
...
| 1.2 | Минимальная связанность между модулями | Core↔Plugin через `__init__.py` + `bridge.py` ([INTEGRATION_CONTRACT.md***REMOVED***(../../freebuff_plugin/INTEGRATION_CONTRACT.md)) |
| 1.3 | Понятная структура каталогов | См. [ARCHITECTURE_PRINCIPLES.md***REMOVED***(ARCHITECTURE_PRINCIPLES.md) |
| 1.4 | Нет дублирования кода (DRY) | Shared helpers через `freebuff_plugin/bridge.py` |
| 1.5 | Нет «магических» чисел и строк | Константы в `freebuff_plugin/config.py`, `__init__.py` модулей |
...
| 6.6 | Lazy loading — загружать только когда нужно | `__getattr__` в `freebuff_plugin/__init__.py`, ленивые accessor'ы в `mcp_server.py` |
...
| # | Требование | Buffy-специфика |
|---|-----------|-----------------|
| 10.1 | Инструкция установки | RECIPE.md для каждого Runtime ([runtime/recipes/***REMOVED***(../../runtime/recipes/)) |
| 10.2 | Инструкция запуска | `python scripts/mcp_server.py --http`, `python scripts/doctor.py` |
...
| # | Требование | Buffy-специфика |
|---|-----------|-----------------|
| 11.1 | Легко тестироваться | `pytest tests/ -q`, dependency injection (EventBus, workspace_root) |
...
| 11.6 | Регрессионные тесты | Полный прогон перед каждым merge (цель: 1143+ passed, 0 failures) |
...
| 12.3 | Поддерживать плагины/инструменты | `freebuff_plugin/`, `runtime/recipes/`, `runtime/adapters/` |
...
| 13.7 | mypy type checking | `mypy --strict scripts/ freebuff_plugin/` (цель) |
...
| 14.2 | **Plugin-safe imports** — ядро импортирует плагин только через `freebuff_plugin/__init__.py` |
| 14.3 | **Bridge-only** — плагин импортирует ядро только через `freebuff_plugin/bridge.py` |
...
- [INTEGRATION_CONTRACT.md***REMOVED***(../../freebuff_plugin/INTEGRATION_CONTRACT.md) — граница ядро↔плагин
