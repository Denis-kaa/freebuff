# Отчёт: инцидент потери `scripts/metrics.py` и восстановление из байткода

**Дата:** 2026-07-31
**Автор:** Buffy (сессия восстановления)
**Версия проекта:** v5.25.1 (HEAD `b4c52fc`)
**Файл отчёта:** `docs/audits/RECOVERY_REPORT_2026-07-31.md`
...
В ходе выполнения обязательного security-аудита (`pompts/TASK_SECURE_MCP_ACCESS.md`, Шаг 2 — Bearer-token аутентификация в `scripts/mcp_fastapi.py`) была совершена операционная ошибка: команда `git stash drop` удалила stash, содержавший некоммиченные изменения рабочего дерева. Среди потерянных файлов оказался `scripts/metrics.py` — модуль Metrics Engine (Phase 6, LEVIATHAN Phase C), добавленный в v5.11.0.

Особенность потери: **`scripts/metrics.py` никогда не существовал в git-истории** (ни в одном коммите, ни в HEAD, ни в dangling-объектах). Файл жил только в рабочем дереве как untracked. После `git stash push -u` + `git stash drop` он перестал существовать и на диске, и в объектной базе git.

Восстановление классическими способами (rollback HEAD~1, restore из fsck, checkout из коммитов) было невозможно. Единственным источником истины оказался **скомпилированный байткод** `scripts/__pycache__/metrics.cpython-314.pyc` (37 514 байт), сохранившийся на диске.
...
- кросс-проверка схемы БД по живой `data/metrics.db` (320 снапшотов, 64 отчёта).

Верификация:

- `python -m py_compile scripts/metrics.py` — 0 ошибок;
...
- `python -m pytest tests/test_mcp_fastapi.py`: **57 passed** (было 13 failed);
- code-reviewer-deepseek-flash: подтвердил поведенческую идентичность, отметил осознанно сохранённые quirks.

**Главные уроки:**

1. **Untracked-файлы — это риск потери.** Файл, который «работает и тестируется», но не закоммичен, технически не существует с точки зрения git. `scripts/metrics.py` существовал в CHANGELOG, документации, dashboard и 37 тестах — но не в git.
...
- **Шаг 0** — диагностика поверхности `check_command`/`check_params` через MCP-маршруты. Результат: маршрутов, прокидывающих пользовательский ввод в verifier, нет. Артефакт: `docs/audits/AUDIT_STEP0_2026-07-31.md`.
- **Шаг 1** (v5.25.0, коммит `c51ce49`) — закрытие свободного shell в `scripts/verifier.py`: удалены `_run_shell()` (subprocess с `shell=True`), `_check_shell()`, `_check_content_match()`; `_check_pytest()` переписан на argv-список с `shell=False`. 75 тестов, 0 failures.
- **Шаг 2** (v5.25.1, коммит `b4c52fc`) — Bearer-token auth в `scripts/mcp_fastapi.py`: `verify_bearer_token` с `hmac.compare_digest`, токен из Vault (hvac) с env-fallback, TTL-кеш 5 минут, AppRole/root-token, KV v2 path-stripping, двойной lock тестового bypass, DoS-кап len>1024, RFC 6750 `WWW-Authenticate`. 57 тестов (47 существующих + 10 TestAuthorization), 0 failures.

Именно при коммите Шага 2 и произошёл инцидент.

### 2.3 Ключевые файлы до инцидента

- `scripts/mcp_fastapi.py` — FastAPI-обёртка BuffyMcpServer, эндпоинты `/mcp` (POST/GET/DELETE), `/`, `/dashboard`, `/metrics/*`. В нём `_get_metrics()` — lazy singleton MetricsEngine.
- `scripts/metrics.py` — Metrics Engine (был **untracked**): классы `MetricResult`, `MetricsReport`, `MetricsEngine`, `Colors`; функции `_print_header`, `_format_metric`, `_cmd_report`, `_cmd_single`, `_cmd_trend`, `_cmd_status`, `_compute_health_score`, `main`. 5 метрик: VCR, SRG, CpVO, RRR, TTD-false. Источники: `data/context.db` (action_verifications), `data/verifier.db` (verification_results), кэш-снапшоты в `data/metrics.db`.
- `tests/test_metrics.py` — тестовый файл (также был untracked, потерян; сохранился только байткод `tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc`, 80 899 байт, 15 тестовых классов / 37 тестов).

### 2.4 История версий метрик (CHANGELOG)

- **v5.11.0** — LEVIATHAN Phase C: Metrics Engine (`scripts/metrics.py`), 5 метрик, SQLite-кэш снапшотов, CLI, интеграция с mcp_fastapi.
...
- **v5.19.0** — Metrics Dashboard (`buffy-playground/public/metrics-dashboard.html`), `/dashboard` endpoint.
...
modified:   docs/decisions/IDEAS.md
modified:   docs/vision/ROADMAP.md
modified:   freebuff_cli.py
modified:   freebuff_plugin/bootstrap/engine.py
modified:   freebuff_plugin/policy/config.py
modified:   freebuff_plugin/policy/engine.py
modified:   freebuff_plugin/runtime/registry.py
modified:   pompts/promt18.md
modified:   scripts/context_manager.py
modified:   scripts/mcp_fastapi.py
modified:   scripts/mcp_server.py
modified:   scripts/memory_engine.py
modified:   scripts/orchestrator.py
modified:   tests/test_bootstrap_engine.py
modified:   tests/test_mcp_fastapi.py
modified:   tests/test_mcp_server.py
modified:   tests/test_memory_engine.py
modified:   tests/test_orchestrator.py
modified:   tests/test_runtime_abstraction.py
Untracked:  scripts/metrics.py
Untracked:  tests/test_metrics.py
Untracked:  (+ прочие untracked: docs/core/*.md, docs/ops/*.md, plugins/, infa/*.docx и т.д.)
```

Ключевые наблюдения:

1. **21+ pre-existing рабочая модификация** не относились к Шагу 2 — они были накоплены предыдущими сессиями (в том числе незакоммиченные правки `scripts/memory_engine.py`, `scripts/orchestrator.py`, `freebuff_plugin/*`, тестовые правки).
2. **`scripts/metrics.py` и `tests/test_metrics.py` были untracked** — то есть в git-статусе числились как `??`, но не имели ни одного вхождения в объектную базу.
3. Среди untracked также находились десятки **незакоммиченных документационных артефактов** (`docs/core/*SPECIFICATION.md`, `docs/ops/*.md`, `docs/audits/*.md`, `plugins/*`) — судя по `git status` в начале сессии, многие из них тоже были untracked.
...
Важная ретроспектива: ещё **до** инцидента существовал сигнал о том, что `metrics.py` не в git. В `git diff` на старте сессии (см. раздел 11) файл `scripts/metrics.py` отсутствовал среди modified — он был только untracked. Однако:

- CHANGELOG.md описывал v5.11.0 «Metrics Engine» как сделанный;
- `docs/core/MODEL_GATEWAY_SPECIFICATION.md` и другие спеки ссылались на метрики;
- 37 тестов в `tests/test_metrics.py` проходили;
- dashboard работал.
...
| # | Время | Событие |
|---|-------|---------|
| 1 | ~15:45 | Реализация Шага 2 (Bearer auth) завершена. `pytest tests/test_mcp_fastapi.py` → **57 passed**. |
...
| 4 | ~15:48 | Выполнен `git checkout stash@{0***REMOVED*** -- scripts/mcp_fastapi.py tests/test_mcp_fastapi.py requirements.txt` — Шаг 2 файлы восстановлены из stash. |
...
| 6 | ~15:48 | Обнаружено, что `git status` чистый — 21 pre-existing модификация + untracked файлы (включая `scripts/metrics.py` и `tests/test_metrics.py`) исчезли. |
...
| 10 | ~15:50 | **Регрессия обнаружена:** `pytest tests/test_mcp_fastapi.py` → **44 passed, 13 failed**. |
...
4. `scripts/metrics.py` был **untracked** (никогда не добавлялся в git), поэтому даже восстановление tracked-файлов его не касалось.

### 3.2 Почему 13 тестов упали именно из-за metrics.py

`scripts/mcp_fastapi.py` содержит 8 эндпоинтов `/metrics/*`:
...
Вместе с `metrics.py` потеряны ещё 21 pre-existing working-tree модификация (включая правки в `scripts/memory_engine.py`, `scripts/orchestrator.py`, `freebuff_plugin/*`, тестовые правки) и несколько untracked-файлов. Часть из них была восстановлена из других источников (пересоздана/переприменена) в ходе последующих шагов; `metrics.py` и `test_metrics.py` — нет, из-за полного отсутствия в объектной базе.
...
```bash
git log --all --oneline -- scripts/metrics.py
```
...
Вывод: `scripts/metrics.py` и `tests/test_metrics.py` существовали **только** как untracked файлы рабочего дерева. Они не входили ни в один коммит за всю историю проекта (а история — десятки коммитов, включая `feat: Policy Engine v5.2.0` и более ранние).
...
Проверено `git ls-tree -r` по **всем** dangling-коммитам и дереву `8349921` (полный список `scripts/`):

```
scripts/auto_conspect.py, auto_save.py, bootstrap.py, buffy_stream_logger.py,
...
**`scripts/metrics.py` отсутствует во всех деревьях.** Это подтверждает: на момент stash `metrics.py` уже не был в объектной базе (или никогда не попадал в неё).
...
```bash
git check-ignore -v scripts/metrics.py tests/test_metrics.py
```
...
|----------|----------|---------|
| Файл был в более раннем коммите и удалён | `git log --all -- scripts/metrics.py` | Опровергнута (пусто) |
...
- (c) stub MetricsEngine — **возможно, но потерян весь функционал** Phase 6 (5 метрик, 8 HTTP-эндпоинтов, CLI, тренды), а Architecture Reality Check (promt22.md) оценивает реальные интеграции, а не заглушки;
- (d) **реконструкция из байткода** — найден `scripts/__pycache__/metrics.cpython-314.pyc`, загружается в Python 3.14.6. Это единственный путь к полному функционалу.
...
```
./tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc
./scripts/__pycache__/metrics.cpython-314.pyc
```

- `scripts/__pycache__/metrics.cpython-314.pyc` — 37 514 байт (magic `2b0e0d0a`, Python 3.14). Размер исходника из pyc-заголовка: 37 514 байт — соответствует ~950-стр. файлу.
- `tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc` — 80 899 байт, 15 тестовых классов, 37 тестов.
...
spec = importlib.util.spec_from_file_location(
    "metrics_recovered", "scripts/__pycache__/metrics.cpython-314.pyc")
...
`data/metrics.db` (118 784 байта) — реальные данные работы движка:
...
Схема `data/metrics.db` (`metric_snapshots`, `reports`, индексы `idx_ms_metric`, `idx_ms_report`) совпала с `executescript` из `_init_metrics_db` (восстановленным из `co_consts`) — побайтово.
...
**Почему не stub:** Architecture Reality Check (promt22.md) требует, чтобы компонент реально работал, а его тесты — проходили. Stub-`MetricsEngine` (возвращающий нули) сломал бы 12 тестов `TestMetricsEndpoints` семантически (формат ответов) и, главное, уничтожил бы функционал Phase 6 (VCR/SRG/CpVO/RRR/TTD — метрики качества, на которых строится Health Score и dashboard). Это закрывало бы 13 тестов, но оставляло «мёртвый» компонент — ровно то, что аудит promt22 помечает как 🔴.
...
3. полный прогон `tests/test_mcp_fastapi.py` (57 тестов, включая 13 ранее падавших);
...
`scripts/__pycache__/metrics.cpython-314.pyc` не удалялся после реконструкции — он остаётся эталоном для проверки (аналогично `test_metrics.cpython-314-pytest-9.1.1.pyc` для тестов). Это позволяет в любой момент повторить поведенческое сравнение.

### 7.9 Сводная матрица решений

Сводка всех решений с точки зрения «альтернативы → выбор → обоснование». Это тот формат, который просит Architecture Reality Check (promt22.md): каждый выбор должен быть доказан, а не декларирован.
...
```bash
python -m py_compile scripts/metrics.py        # compile_exit=0
...
Почему `total_tasks = 0` при живой БД? Потому что таблица `action_verifications` в `data/context.db` пуста (движок писал снапшоты, но не получал верификаций в этом прогоне). Это не дефект: обе реализации прочитали одни и те же данные и выдали одинаковый результат.

### 8.3 Полный прогон затронутых тестов

```bash
python -m pytest tests/test_mcp_fastapi.py -q --tb=short
```
...
`scripts/metrics.py` — 925 строк, 36 836 байт.
...
с локальным импортом `from scripts.event_bus import Event` внутри try/except (грациозная деградация без event_bus). Сигнатура `Event(type=..., source=..., data=...)` сверена с `scripts/event_bus.py` (`type: str`, `data: Dict = field(default_factory=dict)`, `source: str = "system"`).

### 9.5 CLI

`python scripts/metrics.py {report|vcr|srg|cpvo|rrr|ttd|trend|status***REMOVED*** [--json***REMOVED*** [--limit N***REMOVED*** [--metric X***REMOVED***` — цветной вывод через `Colors`, `_print_header` с рамкой `====...====`, `_format_metric` с иконками тренда (↑/↓/→) и цветовой кодировкой по типу метрики (vcr: up→green/down→red; srg/cpvo/rrr/ttd: down→green/up→red).
...
Байткод тестов (`tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc`) содержит 15 классов / 37 тестов, полностью покрывающих восстановленный API:
...
**Важно:** `tests/test_metrics.py` (исходник) тоже потерян. Его можно восстановить тем же методом (реконструкция из pyc) — это отдельная задача, предложена в follow-ups. После восстановления набор тестов проекта вернётся к полному (~1160+ тестов).

### 10.2 TestMetricsEndpoints (mcp_fastapi)

12 эндпоинт-тестов в `tests/test_mcp_fastapi.py` покрывают:
...
```
 m projects/diet_platform        (submodule — не трогали)
?? pompts/promt22.md             (новый аудит-промпт)
?? scripts/metrics.py            (восстановленный модуль — НЕ ЗАКОММИЧЕН)
?? docs/audits/RECOVERY_REPORT_2026-07-31.md  (этот отчёт)
```

### 11.2 Файлы

- `scripts/metrics.py` — восстановлен, 925 строк, верифицирован. **Untracked.**
- `scripts/__pycache__/metrics.cpython-314.pyc` — эталон для сверки (сохранён).
- `tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc` — байткод тестов (сохранён, источник для реконструкции test_metrics.py).
- `data/metrics.db` — не пострадал (118 КБ, 320 снапшотов, 64 отчёта).
- `docs/audits/RECOVERY_REPORT_2026-07-31.md` — этот отчёт.

### 11.3 Что осталось

1. **Закоммитить** `scripts/metrics.py` (+ при необходимости `tests/test_metrics.py` после реконструкции) — отдельным commit'ом с CHANGELOG/TASK-записью (по регламенту проекта docs-изменения коммитятся отдельно).
2. Реконструировать `tests/test_metrics.py` из pyc (37 тестов) — вернёт полный набор метрических тестов.
3. Прогнать полный `pytest tests/` для подтверждения отсутствия других регрессий.
4. Запустить **Architecture Reality Check** (`pompts/promt22.md`) на чистой базе.
...
1. **Untracked-файлы = несуществующие.** Любой новый модуль должен попадать в git в момент создания (хотя бы пустым коммитом/веткой). Критичные для продукта файлы — тем более. В данном проекте `scripts/metrics.py` и `tests/test_metrics.py` существовали с v5.11.0 (несколько релизов), но ни разу не были добавлены в git.
...
4. **Автодобавление тестовых файлов в репозиторий**: `tests/` должны быть tracked с самого начала (в проекте это нарушено для `test_metrics.py` и ряда новых тестовых модулей).
...
### 12.3 Для аудита promt22.md

Инцидент подтверждает важность «код важнее документации»: Metrics Engine существовал в CHANGELOG, документации и тестах, но **не существовал в git** — и это чуть не стало безвозвратной потерей. Архитектурный аудит должен учитывать не только наличие кода, но и его сохранность (tracked vs untracked). Рекомендация: добавить в Architecture Reality Check проверку «все ли untracked-файлы в `scripts/` и `tests/` критичны и должны быть добавлены в git».
...
python -c "import struct; d=open('scripts/__pycache__/metrics.cpython-314.pyc','rb').read(16); print(d[:4***REMOVED***.hex())"
...
spec = importlib.util.spec_from_file_location("m", "scripts/__pycache__/metrics.cpython-314.pyc")
mod = importlib.util.module_from_spec(spec)
...
git log --all --oneline -- scripts/metrics.py          # (пусто)
...
# БД
python -c "import sqlite3; con=sqlite3.connect('data/metrics.db'); \
...
1. **Metrics Engine не восстановился бы.** Файла `scripts/metrics.py` нет ни в `a269838`, ни в более ранних коммитах — это доказано `git log --all` и `git rev-list --all --objects`. Откат вернул бы ровно то же состояние отсутствия модуля.
...
2. **Phase 6 был бы мёртв.** Dashboard показывал бы нули, `/metrics/*` возвращал бы пустые данные, `data/metrics.db` перестал бы наполняться новыми снапшотами (320 существующих строк остались бы без продолжения).
3. **Architecture Reality Check (promt22.md) пометил бы компонент как 🔴** — потому что аудит оценивает реальные интеграции, а «компонент, который нигде не вызывается / ничего не считает, считается неиспользуемым».
...
2. **21 pre-existing модификация** — вернулись бы в рабочее дерево, но **без возможности отделить их от Шага 2** при коммите: `git add scripts/mcp_fastapi.py` зацепил бы и `scripts/memory_engine.py`, и `scripts/orchestrator.py`, и всё остальное. Пришлось бы делать сложный частичный коммит.
...
### 15.1 Почему `scripts/metrics.py` никогда не был закоммичен?
...
Да. Байткод `tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc` (80 899 байт) загружается как модуль (проверено) и содержит 15 тестовых классов / 37 тестов. Реконструкция тестов — это отдельная задача, которая выполняется тем же 4-этапным методом (раздел 6). Она запланирована как follow-up, чтобы вернуть полный набор метрических тестов (~1160+ тестов проекта).
...
`total_tasks` берётся из COUNT в `action_verifications` (таблица `data/context.db`), а снапшоты хранятся в `data/metrics.db`. Эти две таблицы независимы: снапшоты писались движком при вызовах `compute_report(save=True)` в прошлых прогонах, а `action_verifications` в текущем прогоне пуста (верификации не поступали). Обе реализации (pyc и реконструкция) прочитали одни и те же данные и выдали одинаковый результат — что и требовалось доказать для идентичности.
...
git check-ignore -v scripts/metrics.py tests/test_metrics.py   # exit 1 (не игнорируются)
...
ls -la data/metrics.db     # 118784 байт
python -c "import sqlite3; con=sqlite3.connect('data/metrics.db'); \
  [print(r[0***REMOVED***) for r in con.execute(\"SELECT sql FROM sqlite_master WHERE type='table'\")***REMOVED***"
...
6. Работа с untracked-критичными файлами (scripts/, tests/) без их `git add` — запрещено; файл считается несуществующим, пока не в git.
