...
**Дата:** 2026-07-31
**Источник задачи:** `pompts/promt23.md`
...
В инциденте с `git stash drop` (2026-07-31) утерян не только `scripts/metrics.py`, а **целые модули с их тестами**. Доказательства:

- `git log --all --oneline -- scripts/roles.py` → **пусто** (никогда не были в git)
- `git check-ignore scripts/roles.py` → **NOT_IGNORED** (не gitignore, а именно потеря)
- Байткод `scripts/__pycache__/*.cpython-314.pyc` **загружается** и содержит полные классы (проверено импортом)
- SQLite-файлы `data/*.db` (roles.db, presence.db, collaboration.db, project_pulse.db) существуют → модули реально работали

| Модуль (заявлен в CHANGELOG) | Версия | Классы в pyc (проверено импортом) | Тест (потерян) |
|---|---|---|---|
| `scripts/roles.py` | v5.22.0 | `RoleEngine`, `RoleDefinition`, `STANDARD_ROLES`, `AgentRole` | `tests/test_roles.py` |
| `scripts/presence.py` | v5.17.0 | `PresenceEngine`, `AgentPresence`, `PresenceHistoryEntry`, `DEFAULT_HEARTBEAT_INTERVAL` | `tests/test_presence.py` |
| `scripts/collaboration.py` | v5.18.0 | `CollaborationEngine`, `CollaborationSession`, `CollabMessage`, `Participant`, `ParticipantRole` | `tests/test_collaboration.py` |
| `scripts/distributed_agents.py` | v5.14.0 | `AgentMesh`, `TaskDistributor`, `DistributedCoordinator`, `DistributedWorkflowPlan`, `DistributedWorkflowStep`, `AgentTask` | `tests/test_distributed_agents.py` |
| `scripts/rag_engine.py` | v5.23.0 | `RAGEngine`, `RAGReport`, `FeatureVector`, `MAX_RERANK_CANDIDATES` | `tests/test_rag_engine.py` |
| `scripts/project_pulse.py` | v5.21.0 | `ProjectPulse`, `PulseEntry`, `PULSE_TYPES`, `PULSE_DB` | `tests/test_project_pulse.py` |
| `scripts/notification.py` | v5.24.x | модуль каскада уведомлений (`notify()`) | `tests/test_notification.py` |
...
> ⚠️ **Важно про `test_metrics.py`:** `scripts/metrics.py` восстановлен (коммит `4f91c29`) и верифицирован через 57 тестов `test_mcp_fastapi.py`, но его **профильный тест-файл `tests/test_metrics.py` потерян** — у восстановленного модуля нет собственных тестов. Реконструировать из `tests/__pycache__/test_metrics.cpython-314-pytest-9.1.1.pyc` (загружается, классы/функции интроспектируемы) в рамках Этапа 0.

**Побочный ущерб:** `MemoryLevel.VECTOR` (6-й уровень, v5.12.0, chromadb VectorBackend) отсутствует в текущем `scripts/memory_engine.py` (на диске 5 уровней: WORKING/PROJECT/KNOWLEDGE/PERSONAL/ARCHIVE) — VECTOR-код и `tests/test_vector_memory.py` также потеряны.
...
| Компонент | Почему |
|---|---|
| **EventBus** (`scripts/event_bus.py`) | интеграция в 10+ модулей, publish/subscribe/unsubscribe, wildcard |
| **ContextManager** (`scripts/context_manager.py`) | сессии/чекпоинты/сообщения, SQLite, auto-checkpoint, EventBus-события |
| **MemoryEngine** (`scripts/memory_engine.py`) | 5 уровней, build_context, токен-бюджет, EventBus |
| **KnowledgeEngine + SemanticIndex + GraphIndex** | FTS5 + TF-IDF + LSA, auto_discover, граф связей |
| **Verifier** (`scripts/verifier.py`) | VerificationRule (6 типов проверок), VerifierStorage, DEFAULT_RULES, EventBus, shell-защита (v5.25.0) |
| **ScenarioEngine** (`freebuff_plugin/scenario_engine.py`) | 7+ сценариев, интеграция REST + MCP + Telegram |
| **Runtime Adapter Layer** (`freebuff_plugin/runtime/`) | RuntimeAdapter ABC, StdioMCPAdapter, HTTPMCPAdapter, RuntimeRegistry, provider YAML |
| **ModelGateway + SmartRouter** | провайдеры deepseek/gemini/ollama, capability-роутинг |
| **PluginAPI** (`scripts/plugin_api.py`) | PluginManifest, PluginRegistry, discover, lifecycle, EventBus |
| **MCP Server** (`scripts/mcp_server.py`) | 20 tools, prompts, resources (stdio; Vault-Bearer auth — отдельно на HTTP-слое `mcp_fastapi.py`) ⚠️ часть tools (`distributed_*`/`pulse_*`/`roles_*`) зарегистрирована, но **недоступна** — бэкенды утеряны (см. 0.1) |
...
5. **Skills-реестр** и **структурированная Prompt Library** (сейчас pompts/ — набор файлов, не библиотека).
...
Модель из promt23.md проверена против кодовой базы. Уточнения отмечены `(уточнено)`.
...
│
├── Resources  ← 🟡 runtime/providers/*.yaml (freebuff, claude_code, openclaw) + RuntimeRegistry
...
│
├── Documentation  ← ✅ docs/ (core/plugin/vision/audits) + README + SPEC.md + BUFFY.md
│
...
| **Project Registry** | 🟡 | `scripts/scan_projects.py` | таблица `projects` в `data/context.db` (name/path/language/git_remote/category/status), upsert, `--rebuild`, `--status` | CLI + KnowledgeEngine + MemoryEngine | нет lifecycle (create/close/archive), нет связей с Work Area |
| **Project Discovery** | ✅ | `scan_projects()` + `detect_git_remote()` + `detect_language()` | `scripts/scan_projects.py:114` | запускается вручную | автоматическое сканирование есть, но не демонизировано |
| **Team (users/members/invites)** | 🔴 | нет | `grep invite/member` → только роли сообщений | — | нет ни одного объекта пользователя |
| **Permissions (RBAC/ACL)** | 🔴 | нет RBAC | `freebuff_plugin/policy/engine.py` — PolicyEngine (выбор runtime по capabilities, не RBAC) | PolicyEngine используется runtime-выбором | роли (roles.py) потеряны; после восстановления — фундамент для RBAC |

### 3.2. Runtime & AI

| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **Runtime Adapter Layer** | ✅ | `freebuff_plugin/runtime/adapter.py` | `RuntimeAdapter(ABC)`, `StdioMCPAdapter`, `HTTPMCPAdapter`; `registry.py`: `RuntimeRegistry`, `RuntimeCapabilityRegistry`, `discover()` | MCP tools `runtime_list/connect/disconnect/select/generate` в `scripts/mcp_server.py` | провайдеры: `runtime/providers/{freebuff,claude_code,openclaw***REMOVED***.yaml` |
| **AI Runtime (Model Gateway)** | ✅ | `scripts/model_gateway.py` | `ModelGateway` (deepseek/gemini/ollama/qwen, 30+ моделей, EventBus publish) | orchestrator, sdk_bridge, CLI | + `core/router.py`: `ModelCatalog`, `SmartRouter` (capability-роутинг) |
| **Runtime Permissions** | 🟡 | `freebuff_plugin/policy/engine.py` + `policy/rules.py` | `PolicyEngine`, `RuleEvaluator` (+MaxLatency/Exclude/RequiredFlags/MinConfidence), `freebuff_plugin/policy/config.py` | RuntimeCapabilityRegistry загружает scores из provider manifests | это capability-политики, не RBAC/ACL |

### 3.3. Context & Memory & Knowledge

| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **Context Manager** | ✅ | `scripts/context_manager.py` | `ContextManager`, `CheckpointType`, сессии/сообщения/чекпоинты SQLite, auto-checkpoint по сообщениям и токенам, EventBus `session.*/checkpoint.*` | freebuff_cli, stream_bridge, integrate_agent, auto_conspect, api.py | |
| **Context Builder** | ✅ | `scripts/context_builder.py` | `ContextBuilder` (токен-бюджет, включение памяти/задачи/changelog/сессии) | freebuff_cli (`builder = ContextBuilder(max_tokens=6000)`) | |
| **StreamBridge / Session** | ✅ | `scripts/stream_bridge.py`, `scripts/stream_session.py`, `scripts/session_utils.py`, `scripts/agent_context_bridge.py` | сессии, checkpoints, resume, auto-checkpoint каждые 10 сообщений | freebuff_cli `cmd_buffy()` | |
| **Memory Engine** | 🟡 | `scripts/memory_engine.py` | `MemoryEngine`, `MemoryLevel` (5 уровней: WORKING/PROJECT/KNOWLEDGE/PERSONAL/ARCHIVE), `ContentType`, `build_context` | scan_projects, seed_knowledge, graph_index, knowledge_engine, orchestrator, mcp_server | **VECTOR (6-й уровень) потерян** — `grep VECTOR` → 0; VectorBackend/chromadb отсутствует |
| **Knowledge Engine** | ✅ | `scripts/knowledge_engine.py` | `KnowledgeEngine` (FTS5 + TF-IDF + `SemanticIndex` LSA), `KnowledgeEngineStats`, EventBus `knowledge.*` | mcp_server (`knowledge_search`), seed_knowledge, scan_projects | |
| **Graph Index** | ✅ | `scripts/graph_index.py` | `GraphIndex`, `auto_discover()`, `auto_discover_from_memory()` | knowledge_engine (`graph_auto_discover`) | |
| **RAG 2.0** | 🔶 УТЕРЯН | `scripts/rag_engine.py` | pyc: `RAGEngine`, `RAGReport`, `FeatureVector`, `MAX_RERANK_CANDIDATES`, `rrf_merge` | был 3 MCP tools (`rag_search/rag_hybrid/rag_rerank`) | восстановим из pyc |
| **Knowledge Sync Plugin** | ✅ | `plugins/knowledge_sync/` | manifest.json + `__init__.py` (Memory→Knowledge sync, force_reindex) | EventBus `memory.stored` | |

### 3.4. Scenario & Process & Task

| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **Scenario Engine** | ✅ | `freebuff_plugin/scenario_engine.py` | `ScenarioEngine` (load/search/apply, 7+ .md сценариев в `freebuff_plugin/scenarios/`) | REST `/scenarios*` (api.py), MCP tools (freebuff_plugin/mcp_server.py), Telegram (tgbot.py) | категории: freelancing/agent/templates |
...
| **Orchestrator (Tasks)** | ✅ | `scripts/orchestrator.py` | `Orchestrator`, `Workflow`, `Step`, `StepValidator`, `DefaultPlanner`, `ToolExecutor`, retry, EventBus `workflow.*/step.*` | MCP tools, CLI | параллельное выполнение шагов, DAG-зависимости |

### 3.5. Plugins & Skills & Prompts

| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **Plugin API** | ✅ | `scripts/plugin_api.py` | `PluginManifest`, `PluginRegistry`, `discover()`, lifecycle (load/enable/disable/unload), EventBus | 4 плагина: `plugins/{hello_world,tg_messenger,system_monitor,knowledge_sync***REMOVED***` | каждый с manifest.json |
| **Skills** | 🔴 | нет реестра | `grep class Skill` → 0 | — | ближайшее: сценарии (интеллектуальные последовательности) |
| **Prompt Library** | 🟡 | `pompts/*.md` (27 файлов) + `scripts/mcp_server.py` `_register_prompts()` | MCP prompts: `context_resume`, `knowledge_search`, `task_start` | mcp_server | набор файлов, не структурированная библиотека с метаданными |

### 3.6. Connectors & MCP

| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **Telegram** | ⚠ дубль | `scripts/telegram_bot.py` + `freebuff_plugin/tgbot.py` + `plugins/tg_messenger/` | 3 реализации: session-бот, scenario-бот, plugin | run скрипты `start_telegram_bot.sh`/`start_tgbot.sh` | нужна канонизация |
| **MCP Client** | ✅ | `freebuff_plugin/mcp_client.py` | `MCPClientBase`, `list_prompts()`, `_send_request` | bridge | |
| **MCP Server (stdio)** | ⚠ частично | `scripts/mcp_server.py` | 20 tools (memory/knowledge/context/task/runtime/bridge/bootstrap + `distributed_*`/`pulse_*`/`roles_*`), prompts, resources, EventBus `_publish` | основной инструментальный сервер | **`distributed_*`/`pulse_*`/`roles_*` — registered but unavailable:** lazy-аксессоры (`_get_distributed_coordinator()` и аналоги) вернут ошибку/None при отсутствии потерянных бэкендов (roles.py/project_pulse.py/distributed_agents.py) — до Этапа 0 |
| **MCP Server (plugin)** | ✅ | `freebuff_plugin/mcp_server.py` | `MCPServer` (scenario tools) | bootstrap extensions | |
| **MCP HTTP (FastAPI)** | ✅ | `scripts/mcp_fastapi.py` | REST endpoints + metrics dashboard + **Vault-Bearer auth** | HTTP-клиенты | auth: `_get_active_token()`, hvac, TTL cache, hmac.compare_digest |
| **Bridge Layer (MCP↔ACP)** | ✅ | `freebuff_plugin/bridge_layer.py` | `BridgeLayer`, stdio/http connect, RPC | mcp_server (bridge.*) | |
| **ACP Protocol** | ✅ | `freebuff_plugin/acp_protocol.py` | `ACP_DISCOVER`, discover/request/response | distributed (утрачен), bridge | |
...
| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **Verifier** | ✅ | `scripts/verifier.py` | `VerificationRule` (6 типов: file_exists/content_match/pytest/shell/sqlite/http), `VerifierStorage`, `DEFAULT_RULES`, EventBus, подписка `task.claimed` | MCP tools, CLI `verify/rules` | **v5.25.0:** произвольный shell-исполнение закрыт (pytest через argv без shell=True) |
| **Policy Engine** | ✅ | `freebuff_plugin/policy/engine.py` | `PolicyEngine` (capability-based выбор runtime) | RuntimeCapabilityRegistry | не путать с RBAC |
| **Metrics Engine** | ✅ (восстановлен) | `scripts/metrics.py` | `MetricsEngine` (VCR/SRG/CpVO/RRR/TTD + Health Score), `MetricsReport`, `MetricResult`, EventBus `metrics.report` | HTTP `/metrics/*`, dashboard | восстановлен из pyc, 57/57 тестов mcp_fastapi |
| **Audit Log** | 🟡 | `freebuff_plugin/event/audit.py` | событийный аудит | event-слой | не полноценный Quality Gate pipeline |

### 3.8. Events & Security & Docs

| Компонент | Статус | Реализация | Доказательства | Используется | Замечания |
|---|---|---|---|---|---|
| **EventBus** | ✅ | `scripts/event_bus.py` | `EventBus` (publish/subscribe/unsubscribe, wildcard `*`), `Event`, `Subscription` | интегрирован в 10+ модулей (memory, knowledge, orchestrator, verifier, metrics, mcp_server, plugin_api, context_manager) | |
| **EventStore / Replay / Timeline** | ✅ | `freebuff_plugin/event/` | `store.py`: `EventStore` (SQLite schema.sql); `replay.py`: `ReplayEngine`; `timeline.py`: `TimelineEngine`; `pulse.py` | event-слой | |
| **Secrets (Vault)** | 🟡 | `scripts/mcp_fastapi.py` + `projects/diet_platform/building_blocks/vault_integration.py` | hvac client, `FREEBUFF_VAULT_ADDR/TOKEN/KEY`, TTL-кеш токена; VaultClient в diet_platform (127.0.0.1:8400) | HTTP auth | встроенного vault-хранилища нет — внешняя интеграция |
| **Auth (Bearer)** | ✅ | `scripts/mcp_fastapi.py` | `auth_middleware` с `hmac.compare_digest`, Vault→env fallback | /mcp endpoints | |
| **Secret Scanner** | ✅ | `scripts/scanner.py` | сканирует credentials/token/vault-паттерны | CLI | |
| **Encryption** | 🔴 | нет в платформе | (есть Fernet только в `projects/realtor_automation/.../security.py`) | — | |
| **Prompt Injection Protection** | 🟡 | `scripts/verifier.py` + `freebuff_plugin/policy/` | shell-guard (v5.25.0), capability-политики | verifier | системного PII-фильтра нет |
| **Plugin Trust** | 🔴 | нет | — | — | манифесты не подписываются |
| **Documentation** | ✅ | `docs/` (core: 20+ спецификаций, plugin, vision, audits) + README + SPEC.md + BUFFY.md + BUFFY_PROJECT.md + CHANGELOG | docs/core/*_SPECIFICATION.md (EventBus, Memory, Context, Knowledge, Graph, Orchestrator, ModelGateway, PluginAPI, ToolRuntime, NodeMesh, SessionMesh...) | — | |
...
| Файл | Класс | Роль |
|---|---|---|
| `core/router.py` | `SmartRouter`, `ModelCatalog` | capability-выбор модели |
| `freebuff_plugin/router.py` | `IntentRouter` | маршрут «простое→local, сложное→freebuff» |
| `scripts/sdk_bridge.py` | `SmartRouterAdapter` | адаптер роутинга для SDK |

**Рекомендация:** единый `Router` в `core/`, где IntentRouter и SmartRouterAdapter — тонкие адаптеры. Дублируется сама идея «выбери, что выполнит запрос».

### 4.2. Telegram (3 реализации) — КАНОНИЗИРОВАТЬ

- `scripts/telegram_bot.py` — session-бот (ContextManager, чекпоинты).
- `freebuff_plugin/tgbot.py` — scenario-бот (inline keyboards, /scenarios).
- `plugins/tg_messenger/` — plugin (форвардинг событий, очередь).
...
- `scripts/mcp_server.py` — основной инструментальный сервер (stdio, 20 tools; часть — с потерянными бэкендами до Этапа 0).
- `freebuff_plugin/mcp_server.py` — plugin-сервер сценариев.
- `scripts/mcp_fastapi.py` — HTTP-слой (с auth).
...
- `scripts/event_bus.py` — in-process шина.
- `freebuff_plugin/event/store.py` — персистентный EventStore + Replay + Timeline.

Это layering, а не дубль: шина — транспорт, store — история. Но `freebuff_plugin/event/pulse.py` (существующий) и `scripts/project_pulse.py` (утерянный) пересекаются по концепции ленты событий — **объединить при восстановлении**.
...
| **Структурированная Prompt Library** | pompts/ — файлы без метаданных/версий | P2 |
| **Connectors: GitHub/Email/Drive/CRM/ERP/n8n/Make** | нет файлов | P2 |
| **Encryption (платформенный)** | только в projects/realtor_automation | P1 |
...
- метаданные workspace в `data/workspace.db` (имя, настройки, users, roles, connected resources),
...
- Единый Router в core/ (IntentRouter/SmartRouterAdapter — адаптеры).
...
- Prompt Library = структурированная версия `pompts/` (метаданные, версии, категории) с MCP tools `prompt_get/search`.
