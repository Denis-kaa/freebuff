# Отчёт: потеря 7 модулей и восстановление из байткода (Этап 0 из promt23.md)

**Дата:** 2026-07-31
**Автор:** Buffy (сессия восстановления)
**Версия проекта:** v5.25.x (HEAD `753e3f4` после восстановления)
**Файл отчёта:** `docs/audits/RECOVERY_REPORT_7_MODULES_2026-07-31.md`
...
При выполнении Этапа 0 из `pompts/promt23.md` (подготовка Architecture Reality Check №2 на полной кодовой базе) обнаружено, что 7 модулей `scripts/` отсутствуют в рабочем дереве и в git-истории:
...
Особенность: **файлы никогда не существовали в git** (untracked-файлы рабочего дерева, жили только на диске). Сопутствующая потеря — 7 соответствующих файлов тестов в `tests/` (тоже untracked). Остались только скомпилированные тест-файлы: `tests/__pycache__/test_<module>.cpython-314-pytest-9.1.1.pyc` (48 файлов).

Единственный источник истины о поведении модулей — **байткод тестов** (`co_consts`, `dis`-потоки, сигнатуры вызовов, литералы). Из них реконструированы все 7 модулей + добавлена MCP-интеграция (27 инструментов) в `scripts/mcp_server.py`. Коммит `753e3f4` — 6149 вставок (из них ~687 — mcp_server.py, остальное — 7 модулей и .gitignore).

Верификация: **385/385 тест-pycs проходят** (полный лог — `docs/audits/evidence/RECOVERY_7_MODULES_TEST_RUN_2026-07-31.txt`). Дополнительно: on-disk `test_mcp_server.py` + `test_mcp_fastapi.py` — 236 passed / 2 failed (2 фейла — pre-existing в `test_bootstrap_engine.py`, не связаны с восстановлением).
...
Аналогично инциденту с `scripts/metrics.py` (см. `RECOVERY_REPORT_2026-07-31.md`): при коммите security-работы (v5.25.0/v5.25.1) был выполнен `git stash push -u` + преждевременный `git stash drop`. Untracked-часть stash (второй parent-коммит) при ручном пофайловом восстановлении не восстанавливается автоматически. В результате 7 модулей и их тестов исчезли с диска.
...
В `scripts/__pycache__/` есть pyc для всех 7 модулей, но их таймстампы — 2026-07-31 16:35:46–16:35:53, **позже** реконструкции (16:10–16:23). Это перекомпиляция моих правок при импорте, а не оригинальный байткод. Оригинальными являются только **тест-pycs** (2026-07-28…07-30).
...
- 10 dangling-объектов найдены; проверка `git ls-tree -r <object> --name-only` по каждому на наличие `scripts/roles.py` и пр. — **0 совпадений**.
...
4. **Прогон тест-pycs через специальный харнесс** (`scripts/_run_test_pycs.py`, временный) — 385 тестов, итеративные фиксы до 0 падений.

### 4.1 Харнесс тест-pycs

Временный скрипт `scripts/_run_test_pycs.py` (удалён после прогона; см. ограничения в разделе 8):
...
Добавлено в `scripts/mcp_server.py` (паттерн — как у существующих `_get_bootstrap_engine`/`_get_metrics`):
...
```bash
python3 scripts/_run_test_pycs.py   # временный харнесс, удалён после прогона
```
...
Полный лог: `docs/audits/evidence/RECOVERY_7_MODULES_TEST_RUN_2026-07-31.txt` (42 346 байт).

### 7.2 On-disk тесты (MCP-слой)

```bash
python3 -m pytest tests/test_mcp_server.py tests/test_mcp_fastapi.py tests/test_bootstrap_engine.py -q --tb=short
```
...
- воспроизводятся **изолированно** (`pytest tests/test_bootstrap_engine.py` → 2 failed / 59 passed);
- `git diff` для `freebuff_plugin/bootstrap/engine.py`, `scripts/event_bus.py`, `tests/test_bootstrap_engine.py` — **пустой** (изменений нет);
...
```bash
for f in notification roles presence collaboration distributed_agents rag_engine project_pulse mcp_server; do python3 -m py_compile scripts/$f.py; done
```
...
1. **Харнесс `_run_test_pycs.py` удалён** после прогона (по плану Этапа 0 — временный инструмент). Воспроизведение 385/385 требует его пересоздания; evidence-лог сохранён в репо. Рекомендация на будущее — коммитить харнесс в `scripts/` как `_verify_recovery_pycs.py`.
...
3. **On-disk тесты 7 модулей не существуют** (`tests/test_notification.py` и пр. потеряны вместе с модулями). Для них есть только pyc. Восстановление тест-исходников — отдельная задача (не Этап 0).
...
2. **Тест-pycs — контрактный источник.** При потере исходников тестов байткод `tests/__pycache__/*.pytest-*.pyc` содержит полные контракты (литералы, сигнатуры, предикаты).
...
| Файл | Содержимое |
|------|------------|
| `docs/audits/evidence/RECOVERY_7_MODULES_TEST_RUN_2026-07-31.txt` | Полный лог прогона 385 тест-pycs (SUMMARY: 385 PASS / 0 FAIL) |
| `tests/__pycache__/test_{notification,roles,presence,collaboration,distributed_agents,rag_engine,project_pulse***REMOVED***.cpython-314-pytest-9.1.1.pyc` | Оригинальные тест-байткоды (источник контрактов) |
...
753e3f4 feat(recovery): restore 7 lost modules from pyc + MCP integration (promt23 Etap 0)
  ...
  scripts/notification.py, roles.py, presence.py, collaboration.py,
...
```
 m projects/diet_platform          (submodule, pre-existing)
?? pompts/promt22.md, promt23.md, promt24.md   (пользовательские промты)
```
