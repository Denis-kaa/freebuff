...
**Дата:** 2026-07-31
**Источник задачи:** `pompts/promt24.md`
...
Исследование №1 (`docs/audits/ARCHITECTURE_WORKSPACE_OS_2026-07-31.md`, promt23) проводилось на **неполной** кодовой базе: 7 модулей CoWork/Roles/RAG/Pulse/Notification + metrics были утеряны в инциденте `git stash drop` и помечались статусом 🔶 УТЕРЯН.
...
| MCP-интеграция сломана (`distributed_*`/`pulse_*`/`roles_*` registered but unavailable) | ✅ 27 инструментов работают (51 всего в `scripts/mcp_server.py`) |
| metrics без тестов | ✅ восстановлен (`4f91c29`), 57/57 тестов |
| 385/385 pyc-тестов — план | ✅ 385/385 PASS (evidence: `docs/audits/evidence/RECOVERY_7_MODULES_TEST_RUN_2026-07-31.txt`) |

**Новое открытие №2 (в ходе сверки):** 3 плагина (`knowledge_sync`, `system_monitor`, `tg_messenger`) **тоже утеряны** в том же инциденте — на диске только `hello_world/` (единственный закоммиченный), остальные существуют только как pyc в `plugins/*/__pycache__/__init__.cpython-314.pyc` (проверено: загружаются, экспортируют `KnowledgeSyncPlugin`, `SystemMonitorPlugin`, `TelegramMessengerPlugin`). Восстановимы той же методологией. Также: `_main_with_notification()` из CHANGELOG v5.24.0 в `freebuff_cli.py` **отсутствует** (grep → 0) — интеграция Notification ↔ CLI потеряна. Это учтено в разделах 3, 5, 8.
...
- `scripts/` — 44 файла, 25 541 строка; `freebuff_plugin/` — bootstrap/event/policy/runtime/scenarios + api/bridge/router/wrapper; `core/` — interfaces + router; `plugins/` — 1 на диске (hello_world) + 3 pyc-only (knowledge_sync/system_monitor/tg_messenger).
- 7 SQLite БД в `data/`: context, verifier, metrics, roles, presence, collaboration, project_pulse.
- 2 MCP-сервера (`scripts/mcp_server.py` — 51 инструмент; `freebuff_plugin/mcp_server.py` — event/scenario tools), 1 HTTP-слой (`mcp_fastapi.py`), 1 REST API (`freebuff_plugin/api.py`).
...
Workspace (корень) — 🟡 нет класса Workspace, есть WORKSPACE-константы в модулях;
                     ближайшее: freebuff_cli + bootstrap + data/*.db (единая точка — workspace root)
│
...
│
├── Documentation     — ✅ docs/ (20+ core-спецификаций, audits, ops, plugin, vision) + pompts/ (29 файлов)
│
...
|---|---|---|---|---|
| **Session** | `scripts/context_manager.py` | `ContextManager`, `SessionSnapshot`, `SessionStatus`, `CheckpointType` | сессии/сообщения/чекпоинты, auto-checkpoint | ✅ |
| **MemoryEntry** | `scripts/memory_engine.py` | `MemoryEntry`, `MemoryLevel` (5), `ContentType` | build_context, токен-бюджет | ✅ |
| **KnowledgeDocument** | `scripts/knowledge_engine.py` | `Document`, `FtsIndex`, `TfidfIndex`, `SemanticIndex` | FTS5+TF-IDF+LSA | ✅ |
| **GraphNode/Edge** | `scripts/graph_index.py` | `Node`, `Edge`, `PathResult`, `GraphStats` | auto_discover | ✅ |
| **RAGResult/Report** | `scripts/rag_engine.py` | `RAGResult`, `RAGReport`, `FeatureVector`, `RAGEngine` | rrf_merge, rerank, 5 режимов поиска | ✅ (восстановлен) |
| **Project** | `scripts/scan_projects.py` | `scan_projects()`, `detect_git_remote()`, `detect_language()` | таблица projects в context.db | 🟡 |
| **Scenario** | `freebuff_plugin/scenario_engine.py` | `Scenario`, `ScenarioEngine` | 12 сценариев, load/search/apply | ✅ |
| **Task/Workflow/Step** | `scripts/orchestrator.py` | `Workflow`, `Step`, `DefaultPlanner`, `ToolExecutor` | DAG, retry, параллельность | ✅ |
| **VerificationRule/Result** | `scripts/verifier.py` | `VerificationRule`, `VerificationResult`, `Verifier` | 6 типов проверок, DEFAULT_RULES | ✅ |
| **MetricResult/Report** | `scripts/metrics.py` | `MetricResult`, `MetricsReport`, `MetricsEngine` | VCR/SRG/CpVO/RRR/TTD, Health Score | ✅ (восстановлен) |
| **Runtime** | `freebuff_plugin/runtime/` | `RuntimeAdapter(ABC)`, `StdioMCPAdapter`, `HTTPMCPAdapter`, `RuntimeRegistry` | providers YAML | ✅ |
| **Model** | `scripts/model_gateway.py` | `ModelGateway`, `BaseProvider`, `OpenAICompatibleProvider`, `GeminiProvider`, `OllamaProvider` | capability-роутинг | ✅ |
| **Plugin** | `scripts/plugin_api.py` | `PluginManifest`, `PluginRegistry`, `BasePlugin` | 1 плагин на диске (`hello_world`); 3 плагина 🔶 утеряны (pyc: `KnowledgeSyncPlugin`, `SystemMonitorPlugin`, `TelegramMessengerPlugin` — загружаются) | 🟡 (API ✅, 3/4 плагина 🔶) |
| **Event** | `scripts/event_bus.py` | `Event`, `EventBus`, `Subscription` | 25+ типов | ✅ |
| **EventEntry/Timeline/Audit** | `freebuff_plugin/event/` | `EventStore`, `EventReplay`, `TimelineEngine`, `AuditEngine`, `PulseEngine` | schema.sql, replay | ✅ |
| **Role** | `scripts/roles.py` | `RoleDefinition`, `AgentRole`, `RoleEngine` | 6 стандартных ролей | ✅ (восстановлен) |
| **AgentPresence** | `scripts/presence.py` | `AgentPresence`, `PresenceHistoryEntry`, `PresenceEngine` | heartbeat, prune | ✅ (восстановлен) |
| **CollabSession/Message** | `scripts/collaboration.py` | `CollaborationSession`, `CollabMessage`, `Participant`, `CollaborationEngine` | 6 типов сообщений (text/system/task/file/decision/code) | ✅ (восстановлен) |
| **AgentNode/AgentTask** | `scripts/distributed_agents.py` | `AgentNode`, `AgentTask`, `AgentMesh`, `TaskDistributor`, `DistributedCoordinator` | 3 стратегии, DAG-workflow | ✅ (восстановлен) |
| **PulseEntry** | `scripts/project_pulse.py` | `PulseEntry`, `ProjectPulse` | 15+ типов событий, git-scan | ✅ (восстановлен) |
| **Notification** | `scripts/notification.py` | `notify()`, `notify_task_complete()`, `notify_error()` | 3-канальный каскад; ⚠ потребителей вне тестов нет, CLI-обёртка `_main_with_notification` потеряна | 🟡 (модуль ✅, интеграция 🔴) |
...
| **User** | `grep class User` в scripts/freebuff_plugin → 0 | нет (только участники collab-сессий) |
...
| **Runtime ↔ Policy** | `RuntimeCapabilityRegistry` грузит scores из provider manifests, `PolicyEngine` выбирает | `runtime/registry.py:657`, `policy/engine.py` |
| **Runtime ↔ MCP** | 5 MCP tools: runtime_list/connect/disconnect/select/generate | `mcp_server.py:357` |
| **Scenario ↔ REST/MCP/TG** | `api.py` /scenarios*, `mcp_server.py` apply_scenario, `tgbot.py` /scenarios | `freebuff_plugin/api.py:224` |
...
| Collab → sessions/messages/participants | `collab_sessions`, `collab_messages`, `collab_participants` | `collaboration.db` |
...
|---|---|---|
| **Роутеры (3)** | `core/router.py` (SmartRouter), `freebuff_plugin/router.py` (IntentRouter), `scripts/sdk_bridge.py` (SmartRouterAdapter) | единый `core/Router`; остальные — адаптеры |
| **Telegram (3)** | `scripts/telegram_bot.py`, `freebuff_plugin/tgbot.py`, `plugins/tg_messenger/` | один канонический TelegramConnector |
| **MCP-точки входа (3)** | `scripts/mcp_server.py`, `freebuff_plugin/mcp_server.py`, `scripts/mcp_fastapi.py` | зафиксировать единый ToolRegistry; разграничить ответственность |
| **Event-ленты (2)** | `freebuff_plugin/event/pulse.py` (PulseEngine) vs `scripts/project_pulse.py` (ProjectPulse) | объединить: ProjectPulse — доменная лента, PulseEngine — подписчик |
...
|---|---|---|
| Auth (Bearer) | `scripts/mcp_fastapi.py` — `verify_bearer_token`, hmac.compare_digest, Vault-first, TTL-кеш | ✅ |
| Secrets Vault | hvac client, `FREEBUFF_VAULT_ADDR/TOKEN/KEY`, VaultClient в diet_platform | 🟡 |
| Secret Scanner | `scripts/scanner.py` — credentials/token/vault-паттерны | ✅ |
| Shell-guard | `verifier.py` v5.25.0 — `_check_pytest` argv без shell=True | ✅ |
| RBAC/ACL | нет | 🔴 |
| Encryption | нет (только Fernet в projects/realtor_automation) | 🔴 |
| Plugin Trust | нет | 🔴 |
...
|---|---|---|
| EventBus (in-process) | `scripts/event_bus.py` — publish/subscribe/unsubscribe, wildcard, Event/Subscription | ✅ |
| EventStore (persistent) | `freebuff_plugin/event/store.py` — schema.sql | ✅ |
| Replay | `freebuff_plugin/event/replay.py` — EventReplay | ✅ |
| Timeline | `freebuff_plugin/event/timeline.py` — TimelineEngine | ✅ |
| Audit | `freebuff_plugin/event/audit.py` — AuditEngine (actions/decisions/config) | ✅ |
| Pulse (persistent) | `freebuff_plugin/event/pulse.py` — PulseEngine | ✅ |
| Domain Pulse | `scripts/project_pulse.py` — ProjectPulse (git+file+events) | ✅ (восстановлен) |
| MCP event tools | `freebuff_plugin/mcp_server.py` — event_audit/replay/timeline/search/pulse | ✅ |
...
| PolicyEngine (capability) | `freebuff_plugin/policy/engine.py` | ✅ |
...
- ✅ `freebuff_plugin/scenario_engine.py` — 12 сценариев (9 freelance + agent_setup + task_framework + telegram_health_monitor), категории freelancing/agent/templates, REST + MCP + TG.

### 8.6. Plugins

- ✅ `scripts/plugin_api.py` — PluginManifest/Registry/Loader/lifecycle/EventBus — API работает.
...
- **Prompt Library** 🟡: `pompts/` — 29 файлов без метаданных; MCP prompts: context_resume, knowledge_search, task_start.
...
- единый `core/Router` (IntentRouter/SmartRouterAdapter — адаптеры);
...
`SkillRegistry` (manifest: triggers, uses_scenarios, uses_verifier); Prompt Library = структурированная версия `pompts/` с MCP tools `prompt_get/search`.
...
7. `SkillRegistry` + Prompt Library — структурирование pompts/;
