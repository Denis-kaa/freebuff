...
**Document:** `docs/audits/ARCHITECTURAL_AUDIT_PROMT31_2026-07-31.md`  
**Date:** 2026-07-31  
...
| Core scripts | 40+ modules in `scripts/` |
| Plugins | 4 in `plugins/`, 3 recovered from bytecode |
| Documentation files | 78+ markdown files in `docs/` |
| MCP tools | 12+ tools + runtime/bootstrap extensions |
| Runtime providers | 3 YAML manifests in `runtime/providers/` |

### Recent Milestones & Crises

- **2026-07-28:** Genesis of FreeBuff 2.0 (`core/router.py`, `ContextManager`, `MemoryEngine`, 500+ tests) and integration with MCP & Cloudflare Tunnels.
...
- **2026-07-31 (The Crucible):** Two major security audits led to removal of `shell=True`/`exec` and Bearer Auth for MCP. Simultaneously, uncommitted (`untracked`) files including `scripts/metrics.py` and 10 other modules were lost, forcing recovery from compiled `.pyc` bytecode.
...
│   ├── ContextManager (SQLite sessions/checkpoints)                   │
...
│   └── Engineering Memory (scripts/engineering_memory.py)             │
├─────────────────────────────────────────────────────────────────────┤
│  Documentation Layer                                                │
│   ├── docs/engineering-memory/PROJECT_BOOK.md                       │
│   ├── docs/engineering-memory/ARCHITECTURE.md                       │
│   ├── docs/core/SYSTEM_INVENTORY.md                                 │
│   ├── docs/core/PROJECT_REGISTRY.md                                 │
│   ├── docs/decisions/DECISIONS.md                                   │
│   └── docs/audits/DRIFT_REPORT.md                                   │
├─────────────────────────────────────────────────────────────────────┤
│  External Interfaces                                                │
│   ├── MCP Server / FastAPI / Cloudflare Tunnel                     │
│   ├── Runtime Abstraction Layer (freebuff_plugin/runtime/)           │
...
|---|-----------|--------|---------------------|-----------------|----------------------|------|
| 1 | **Knowledge Management System** | Partial | `KnowledgeEngine`, `RAG`, `Vector Memory`, `docs/engineering-memory/` | Overlap between `KnowledgeEngine`, `RAG_engine`, `Vector Memory` and `Engineering Memory` | **Reuse/Extend:** Consolidate indexing around `engineering_memory.py`. No new engine. | Low |
| 2 | **Project Book** | Exists | `docs/engineering-memory/PROJECT_BOOK.md` | Updated manually or via massive EM prompts | **Reuse:** Keep as main narrative. Automate appending via EM triggers. | Low |
| 3 | **Project Retrospective System** | Partial | `docs/engineering-memory/retrospectives/`, EM templates | Missing automatic triggers on `task.completed` / significant events | **Extend:** Connect `EventBus` to EM Orchestrator to draft retros automatically. | Medium |
| 4 | **Decision Log** | Exists | `docs/decisions/DECISIONS.md`, EM `decisions/` templates | Duplication between core `DECISIONS.md` and EM decision journals | **Merge:** Unify under `docs/engineering-memory/decisions/`. | Low |
| 5 | **Architectural Debt** | Missing | `drift_check.py` touches drift, `ARCHITECTURE_REVIEW.md` | No formal debt log tied to drift findings | **Create:** Add `docs/core/ARCHITECTURAL_DEBT.md` linked to `drift_check.py` outputs. | Low |
| 6 | **Module Registry** | Partial | `docs/core/SYSTEM_INVENTORY.md`, `docs/core/PROJECT_REGISTRY.md`, `FILE_REGISTRY.md` | Gets outdated quickly (untracked-modules crisis) | **Extend:** Automate updates via `scripts/doctor.py` or a new registry generator. | Medium |
| 7 | **Agent Registry** | Partial | `AGENTS.md`, `scripts/roles.py`, `scripts/presence.py`, `freebuff_plugin/acp_protocol.py` | `AGENTS.md` is session checkpoint, not formal capability registry | **Extend:** Formalize `docs/core/AGENT_REGISTRY.md` synced with `roles.py` / `AgentMesh`. | Low |
| 8 | **Integration Registry** | Missing | Dispersed in MCP plugins, `runtime/providers/`, `MARKETPLACE.md` | No unified view of external integrations | **Create:** Implement `docs/core/INTEGRATION_REGISTRY.md`. | Low |
...
| 10 | **Notification System** | Exists | `scripts/notification.py`, visual summary | Needs alignment with Promt27 (progress, long ops, stages) | **Extend:** Add `ProgressTracker` and EventBus hooks. | Medium |
...
| Integration Registry | Scattered across plugins/providers | Centralized registry of all external integrations | Yes | No | Medium | Low |
...
1. **Notification System (Promt 28):** Extend `scripts/notification.py` to listen to `EventBus` for long-running operations, task stage changes, and completion/failure. Add progress heartbeat and stage-based notifications.
2. **Registry Consolidation (Promt 29/31):** Merge `docs/decisions/DECISIONS.md` into the EM structure under `docs/engineering-memory/decisions/`. Create `docs/core/MODULE_REGISTRY.md` and `docs/core/AGENT_REGISTRY.md` using `scripts/doctor.py` and existing metadata as the data source.

### Short-term Priority (Knowledge & Automation)

3. **Automated KMS Triggers (Promt 27/29):** Update `scripts/engineering_memory.py` to subscribe to `task.completed`, `task.failed`, `git.merge`, and `system.error` via `EventBus`. Auto-draft Task Retrospectives and Incident Reports based on thresholds (LOC, duration, severity).
4. **Integration Layer Audit (Promt 29):** Create `docs/core/INTEGRATION_REGISTRY.md` mapping all MCP tools, plugins, runtime providers, and external API gateways. Do not build new integrations until this map is approved.
5. **Architectural Debt Tracker:** Add a hook in `scripts/drift_check.py` to log structural violations and outdated docs to `docs/core/ARCHITECTURAL_DEBT.md`.
...
- `pompts/promt27.md` — Project Book / StoryBook evolution
- `pompts/promt28.md` — Notification framework
- `pompts/promt29.md` — Integration research and registry
- `pompts/promt30.md` — Knowledge Management System design
- `pompts/promt31.md` — Architectural audit mandate
- `docs/engineering-memory/PROJECT_BOOK.md`
- `docs/engineering-memory/ARCHITECTURE.md`
- `docs/core/SYSTEM_INVENTORY.md`
- `docs/core/PROJECT_REGISTRY.md`
