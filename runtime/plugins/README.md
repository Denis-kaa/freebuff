...
`runtime/plugins/` содержит опциональные Python-модули для Runtime,
...
```python
# runtime/plugins/my_runtime/__init__.py
...
И в манифесте `runtime/providers/my_runtime.yaml`:
...
При запуске `RuntimeRegistry` сканирует `runtime/plugins/` и:
...
Структура `runtime/plugins/` — часть Marketplace-архитектуры:

- **providers/** = каталог Runtime
- **plugins/** = расширения системы
