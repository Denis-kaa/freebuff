...
`runtime/providers/` содержит YAML-манифесты для каждого AI Runtime.
...
```bash
# 1. Создай YAML-файл
cp runtime/providers/_template.yaml runtime/providers/my_runtime.yaml

# 2. Заполни поля
nano runtime/providers/my_runtime.yaml

# 3. (Опционально) создай адаптер, если MCP не подходит
# freebuff_plugin/runtime/adapters/my_runtime.py
...
`RuntimeRegistry.discover()` сканирует `runtime/providers/*.yaml` и:
...
5. Сохраняет состояние в `data/runtime_registry.json`
...
Для других типов нужен Python-адаптер в `freebuff_plugin/runtime/adapters/`.

## Marketplace-ready

Эта структура закладывает фундамент для будущего Marketplace:

- **runtime/providers/** — каталог провайдеров (YAML)
- **runtime/plugins/** — опциональные Python-плагины для сложных адаптеров
- **runtime/recipes/** — человекочитаемые инструкции по установке

См. `runtime/MARKETPLACE.md` для полной архитектуры.
