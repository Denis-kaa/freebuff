...
from freebuff_plugin.runtime.registry import (
    RuntimeRegistry,
    RuntimeCapabilityRegistry,
)
from freebuff_plugin.runtime.adapters import (
    FreebuffAdapter,
    ClaudeCodeAdapter,
)
...
class TestStdioMCPAdapter:
    ...
    def test_connect_with_mock(self, mock_mcp_client):
        ...
        with patch("freebuff_plugin.runtime.adapter.StdioMCPClient", return_value=mock_mcp_client):
            ok = adapter.connect()
            assert ok is True
...
class TestRuntimeRegistry:
    ...
    def test_connect_disconnect(self, tmp_storage: Path):
        ...
        with patch("freebuff_plugin.runtime.registry.shutil.which", return_value=None):
            with patch("freebuff_plugin.runtime.registry.Path.exists", return_value=False):
                with patch("freebuff_plugin.runtime.registry.subprocess.run") as mock_run:
...
class TestClaudeCodeAdapter:
    ...
    def test_find_claude_fallback(self):
        ...
        with patch("shutil.which", return_value=None):
            with patch("freebuff_plugin.runtime.adapters.claude.subprocess.run") as mock_run:
