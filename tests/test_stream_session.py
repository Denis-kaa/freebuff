"""Tests for scripts/stream_session.py.
...
from scripts.stream_session import (
    BackgroundWriter,
    start_session,
    resume_session,
    attach_session,
...
class TestCounterCache:
    ...
    def test_get_counter_persisted(self, tmp_path):
        ...
        from scripts.stream_session import _counter_cache
...
class TestStartSession:
    def test_start_creates_dirs_and_files(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_session.STREAMS_DIR", tmp_path)
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", mock_current)
        monkeypatch.setattr("scripts.stream_session.cm", None)
        from scripts.stream_session import _counter_cache
    ...
    def test_start_creates_session_id(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_session.STREAMS_DIR", tmp_path)
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_session.cm", None)
    ...
    def test_start_with_context_manager(self, tmp_path, monkeypatch, context_manager):
        ...
        monkeypatch.setattr("scripts.stream_session.STREAMS_DIR", tmp_path)
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_session.cm", context_manager)
...
class TestLogMessage:
    ...
    def test_log_increments_counter(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_session.cm", None)
        monkeypatch.setattr("scripts.stream_session.BG_WRITER", MagicMock())
...
class TestPruneStreams:
    def test_prune_old_dirs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_session.STREAMS_DIR", tmp_path)
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_session.MAX_STREAM_DIRS", 3)
    ...
    def test_prune_keeps_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_session.STREAMS_DIR", tmp_path)
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_session.MAX_STREAM_DIRS", 1)
...
class TestIntegration:
    ...
    def test_prune_with_active_session(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_session.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_session.cm", None)
        monkeypatch.setattr("scripts.stream_session.MAX_STREAM_DIRS", 2)
