"""Tests for scripts/auto_conspect.py CLI behavior."""
import os
import subprocess
import sys
***REMOVED***
...
class TestAutoConspectCLI:
    ...
    def _run_auto_conspect(self, project_root: Path, workspace: Path, *args: str) -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            [sys.executable, str(project_root / "scripts" / "auto_conspect.py"), *args***REMOVED***,
            capture_output=True,
            text=True,
    ...
    def test_processes_active_session(self, project_root, tmp_path):
        ...
        for sub in ["data", "sessions", "context/checkpoints", "context/summaries"***REMOVED***:
        ...
        summaries = list((tmp_path / "context" / "summaries").glob("*.md"))
        assert len(summaries) >= 1

    def test_demo_flag_runs_demo(self, project_root, tmp_path):
        env = os.environ.copy()
        env["FREEBUFF_ROOT"***REMOVED*** = str(tmp_path)
        result = subprocess.run(
            [sys.executable, str(project_root / "scripts" / "demo_auto_conspect.py")***REMOVED***,
            capture_output=True,
            text=True,
            cwd=str(project_root),
            env=env,
