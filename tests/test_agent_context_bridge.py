"""Tests for scripts/agent_context_bridge.py."""
...
from scripts.agent_context_bridge import AgentContextBridge
...
def _make_bridge(tmp_path) -> AgentContextBridge:
    ...
    os.makedirs(os.path.join(workspace, "context", "summaries"), exist_ok=True)
    os.makedirs(os.path.join(workspace, "context", "checkpoints"), exist_ok=True)
