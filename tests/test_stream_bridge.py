"""Tests for scripts/stream_bridge.py.
...
class TestStreamBridgeInit:
    ...
    def test_init_defaults(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge.WORKSPACE", tmp_path)
        monkeypatch.setattr("scripts.stream_bridge._prune_all", MagicMock(return_value={***REMOVED***))
    ...
    def test_init_runs_gc(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge._prune_all", mock_prune)
        monkeypatch.setattr("scripts.stream_bridge.WORKSPACE", tmp_path)
    ...
    def test_gc_error_doesnt_crash(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge.WORKSPACE", tmp_path)
        monkeypatch.setattr("scripts.stream_bridge._prune_all", MagicMock(side_effect=Exception("GC failed")))
...
class TestStreamBridgeStartSession:
    ...
    def test_start_creates_session(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge.WORKSPACE", tmp_path)
        monkeypatch.setattr("scripts.stream_bridge._prune_all", MagicMock())
        monkeypatch.setattr("scripts.stream_bridge.STREAMS_DIR", tmp_path)
        monkeypatch.setattr("scripts.stream_bridge.CURRENT_FILE", tmp_path / ".current")
        monkeypatch.setattr("scripts.stream_bridge._start_session", MagicMock(return_value=tmp_path / "test_session"))
    ...
    def test_start_session_sets_sid(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge._start_session", MagicMock(return_value=session_dir))
...
class TestStreamBridgeLogging:
    ...
    def test_log_user(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge._log_message", mock_log)
...
class TestStreamBridgeCheckpoint:
    ...
    def test_checkpoint_without_sid(self, tmp_path, monkeypatch):
        ...
        mock_save = MagicMock()
        monkeypatch.setattr("scripts.stream_bridge.ContextManager", MagicMock())
...
class TestStreamBridgeGetContextResume:
    ...
    def test_get_context_resume_with_summary(self, tmp_path, monkeypatch):
        ...
        summaries_dir = tmp_path / "context" / "summaries"
...
class TestStreamBridgeAutoBootstrap:
    ...
    def test_auto_bootstrap_with_active_session(self, tmp_path, monkeypatch):
        ...
        ***REMOVED***
        ...
        monkeypatch.setattr("scripts.stream_bridge._attach_session", mock_attach)
        monkeypatch.setattr("scripts.stream_bridge._prune_all", MagicMock())
...
class TestStreamBridgeFullLifecycle:
    ...
    def test_full_lifecycle_with_mocks(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("scripts.stream_bridge._prune_all", MagicMock())
        monkeypatch.setattr("scripts.stream_bridge._start_session", MagicMock(return_value=mock_session_dir))
        monkeypatch.setattr("scripts.stream_bridge._log_message", mock_log)
