...
Функции:
  ...
  - start_bot / stop_bot: управление фоновым bot listener (scripts/telegram_bot.py)
...
class TelegramMessengerPlugin(BasePlugin):
    ...
    def do_start_bot(self) -> dict:
        ...
        Использует scripts/telegram_bot.py как подпроцесс.
        """
        if self._running:
            return {"success": True, "data": "Bot already running"***REMOVED***
        if not self._bot_token:
            return {"success": False, "error": "TELEGRAM_BOT_TOKEN not set"***REMOVED***
        try:
            ...
            self._running = True
            return {"success": True, "data": "Bot started in background"***REMOVED***
        except Exception as e:
            return {"success": False, "error": str(e)***REMOVED***
    ...
    def do_stop_bot(self) -> dict:
        ...
        self._running = False
        self._bot_thread = None
        return {"success": True, "data": "Bot stopped"***REMOVED***
    ...
    def _run_bot_process(self):
        ...
        try:
            ...
            bot_script = str(Path(WORKSPACE) / "scripts" / "telegram_bot.py")
            env = dict(os.environ)
    ...
    def on_event(self, event):
        ...
        event_data = getattr(event, "data", {***REMOVED***) or {***REMOVED***
