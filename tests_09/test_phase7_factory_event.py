...
def _mock_forge_facade(monkeypatch: pytest.MonkeyPatch, result: Any) -> List[str***REMOVED***:
    ...
    fake_module.ForgeFacade = _FakeForgeFacade  # type: ignore[attr-defined***REMOVED***
...
def test_opportunity_schema_all_fields_roundtrip(tmp_path: Path):
    ...
    opp = _make_opp(
        priority=7,
        scenario={"scenario_id": "blueprint_v3", "role_id": "architect", "score": 0.9***REMOVED***,
        roles=[{"role_id": "architect"***REMOVED***, {"role_id": "developer"***REMOVED******REMOVED***,
        artifacts=[{"raw": {"overall": "ok"***REMOVED******REMOVED******REMOVED***,
        source_path="/tmp/x",
        evidence_path="/tmp/y",
        related_decisions=["d1"***REMOVED***,
        related_whims=["w1"***REMOVED***,
    )
...
def test_opportunity_schema_canonical_field_set():
    ...
    for f in ("id", "project_id", "source", "description", "status", "provenance",
        ...
        assert f in fields, f"canonical поле {f***REMOVED*** отсутствует"
    ...
    for f in ("priority", "scenario", "roles", "artifacts", "source_path",
              ...
              "related_whims", "title"):
        assert f in fields, f"runtime поле {f***REMOVED*** отсутствует"
...
def test_derive_capability_from_provenance():
    ...
    opp = _make_opp()
    opp.provenance["capability"***REMOVED*** = "review"
...
def test_derive_capability_from_scenario():
    ...
    opp = _make_opp(scenario={"capability": "architecture"***REMOVED***)
...
def test_select_factory_forge_routes_by_capability():
    ...
    assert reg.calls == ["review"***REMOVED***
...
def test_execute_records_factory_selection_and_runs_chain(monkeypatch, tmp_path):
    ...
    sel = opp.provenance.get("factory_selection") or {***REMOVED***
...
def test_execute_passes_project_object_not_string(monkeypatch, tmp_path):
    ...
    captured: Dict[str, Any***REMOVED*** = {***REMOVED***

    class _CapturingFacade:
        @staticmethod
        def run_chain(*args: Any, **kwargs: Any) -> Any:
            captured["project"***REMOVED*** = args[0***REMOVED*** if args else None
            return _FakeChainRun(overall="ok")

    fake_module = types.ModuleType("core_02.forge_facade")
    fake_module.ForgeFacade = _CapturingFacade  # type: ignore[attr-defined***REMOVED***
    monkeypatch.setitem(sys.modules, "core_02.forge_facade", fake_module)
    ...
    assert captured["project"***REMOVED***.root == proj_root
...
def test_execute_emits_execution_events(monkeypatch, tmp_path):
    ...
    types_ = [e["type"***REMOVED*** for e in bus.events***REMOVED***
    assert "execution.started" in types_
    assert "execution.completed" in types_
    started = next(e for e in bus.events if e["type"***REMOVED*** == "execution.started")
    assert started["data"***REMOVED***["opportunity_id"***REMOVED*** == opp.id
    assert started["data"***REMOVED***["project_id"***REMOVED*** == "proj-p7"
    assert started["source"***REMOVED*** == "opportunity_engine"
...
def test_execute_emits_execution_failed_on_exception(monkeypatch, tmp_path):
    ...
    failed = next(e for e in bus.events if e["type"***REMOVED*** == "execution.failed")
    assert "forge boom" in failed["data"***REMOVED***["reason"***REMOVED***
...
def test_advance_emits_lifecycle_events():
    ...
    assert any(e["type"***REMOVED*** == "opportunity.deferred" for e in bus.events)
    deferred = next(e for e in bus.events if e["type"***REMOVED*** == "opportunity.deferred")
    assert deferred["data"***REMOVED***["reason"***REMOVED*** == "busy"
    assert deferred["data"***REMOVED***["previous_status"***REMOVED*** == "ACTIVE"

    advance(opp, "REACTIVATED", event_bus=bus)
    assert any(e["type"***REMOVED*** == "opportunity.reactivated" for e in bus.events)

    advance(opp, "READY", event_bus=bus)  # READY не эмитится (не в §J списке)
    advance(opp, "COMPLETED", reason="done", event_bus=bus)
    assert any(e["type"***REMOVED*** == "opportunity.completed" for e in bus.events)
...
def test_propose_emits_scenario_selected(monkeypatch):
    ...
    # Hermetic: подменяем sys.modules['core_02.scenario_registry'***REMOVED*** на фейк-модуль
    ...
    fake_module.ScenarioRegistry = _FakeScenarioRegistry  # type: ignore[attr-defined***REMOVED***
    ...
    assert any(e["type"***REMOVED*** == "scenario.selected" for e in bus.events)
    ev = next(e for e in bus.events if e["type"***REMOVED*** == "scenario.selected")
    assert ev["data"***REMOVED***["scenario_id"***REMOVED*** == "blueprint_v3"
    assert ev["data"***REMOVED***["role_id"***REMOVED*** == "architect"
...
def test_whim_capture_emits_captured(tmp_path):
    ...
    assert any(e["type"***REMOVED*** == "whim.captured" for e in bus.events)
    ev = next(e for e in bus.events if e["type"***REMOVED*** == "whim.captured")
    assert ev["data"***REMOVED***["whim_id"***REMOVED*** == w.id
    assert ev["data"***REMOVED***["whim_source"***REMOVED*** == "cli"
...
def test_whim_promote_emits_promoted(tmp_path, monkeypatch):
    ...
    assert any(e["type"***REMOVED*** == "whim.promoted" for e in bus.events)
    ev = next(e for e in bus.events if e["type"***REMOVED*** == "whim.promoted")
    assert ev["data"***REMOVED***["opportunity_id"***REMOVED*** == w.related_opportunity_id
...
def test_lifecycle_transitions_with_events():
    ...
    assert opp.status == "COMPLETED"
    types_ = {e["type"***REMOVED*** for e in bus.events***REMOVED***
    assert {"opportunity.deferred", "opportunity.reactivated", "opportunity.completed"***REMOVED*** <= types_
...
def test_execute_completed_is_noop(monkeypatch, tmp_path):
    ...
    assert opp.status == "COMPLETED"
    assert calls == [***REMOVED***, "повторный execute на COMPLETED не должен запускать chain"
...
def test_execute_degrade_path_emits_execution_failed(monkeypatch, tmp_path):
    ...
    failed = next(e for e in bus.events if e["type"***REMOVED*** == "execution.failed")
    assert "cannot complete from status" in failed["data"***REMOVED***["reason"***REMOVED***
...
def test_emit_event_real_eventbus_roundtrip(tmp_path):
    ...
    _emit_event(bus, "execution.started", source="opportunity_engine",
                opportunity_id="opp-1", project_id="proj-p7", role_ids=["r1"***REMOVED***)
    entries = bus.get_events(event_type="execution.started", limit=10)
    assert len(entries) == 1
    assert entries[0***REMOVED***.source == "opportunity_engine"
    data = json.loads(entries[0***REMOVED***.data_json)
    assert data["opportunity_id"***REMOVED*** == "opp-1"
    assert data["project_id"***REMOVED*** == "proj-p7"
    assert data["role_ids"***REMOVED*** == ["r1"***REMOVED***
...
def test_execute_with_real_eventbus(tmp_path, monkeypatch):
    ...
    opp = execute(opp, dry_run=False, memory_store=MemoryStore(tmp_path / "mem_real.db"),
                  event_bus=bus)
    assert opp.status == "COMPLETED"
    types_ = {e.event_type for e in bus.get_events(limit=50)***REMOVED***
    assert {"execution.started", "execution.completed", "opportunity.completed"***REMOVED*** <= types_
...
def test_persistence_roundtrip_with_factory_selection(tmp_path):
    ...
    ***REMOVED***
    ...
    loaded = store.get(opp.id)
    assert loaded is not None
    assert loaded.provenance["factory_selection"***REMOVED***["forge_id"***REMOVED*** == "review"
...
def test_discover_candidates_still_works(tmp_path):
    ...
    cands = discover_candidates(
        ...
        source_paths={"whims": tmp_path / "no_whims.yaml",
                      ...
                      "memory": tmp_path / "no_mem.db"***REMOVED***,
    )
