...
class TestAutoConspectCLI:
    ...
    def _run_auto_conspect(self, project_root: Path, workspace: Path, *args: str) -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            [sys.executable, str(project_root / "scripts_01" / "auto_conspect.py"), *args***REMOVED***,
            capture_output=True,
            text=True,
    ...
    def test_processes_active_session(self, project_root, tmp_path):
        ...
        for sub in ["data", "sessions_15", "context_12/checkpoints", "context_12/summaries"***REMOVED***:
    ...
    def test_demo_flag_runs_demo(self, project_root, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, str(project_root / "scripts_01" / "demo_auto_conspect.py")***REMOVED***,
            capture_output=True,
            text=True,
            cwd=str(project_root),
            env=env,
