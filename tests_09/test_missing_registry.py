...
def test_list_by_status(registry):
    ...
    assert [i.item_id for i in registry.list_by_status(REGISTERED)***REMOVED*** == ["a"***REMOVED***
    assert [i.item_id for i in registry.list_by_status(IMPLEMENTED)***REMOVED*** == ["b"***REMOVED***


def test_list_by_factory(registry):
    registry.register_missing("research_web", kind="tool", factory="research")
    registry.register_missing("x", kind="tool", factory="code")
    assert [i.item_id for i in registry.list_by_factory("research")***REMOVED*** == ["research_web"***REMOVED***
...
def test_validate_schema_bad_kind(registry):
    ...
    reg._data = {"x": {"item_id": "x", "kind": "bogus", "status": REGISTERED***REMOVED******REMOVED***
...
def test_validate_schema_implemented_requires_impl(registry):
    ...
    reg._load_error = None
    reg._data = {"x": {"item_id": "x", "kind": "tool", "status": IMPLEMENTED***REMOVED******REMOVED***
...
def test_validate_schema_prompt_requires_path(registry):
    reg = MissingRegistry.__new__(MissingRegistry)
    reg._load_error = None
    reg._data = {"x": {"item_id": "x", "kind": "tool", "status": PROMPT_WRITTEN***REMOVED******REMOVED***
...
def test_missing_item_to_dict():
    ...
    d = item.to_dict()
    assert d["item_id"***REMOVED*** == "research_web"
    assert d["status"***REMOVED*** == REGISTERED
...
def test_cli_seed(tmp_path):
    path = tmp_path / "missing.yaml"
    assert main(["--path", str(path), "seed"***REMOVED***) == 0
    assert main(["--path", str(path), "seed"***REMOVED***) == 0  # идемпотентно
...
def test_cli_register_and_list(tmp_path, capsys):
    ...
    assert main(["--path", str(path), "register", "my_tool", "--kind", "tool",
                 "--factory", "code", "--description", "недостающий тул"***REMOVED***) == 0
    assert main(["--path", str(path), "list"***REMOVED***) == 0
...
def test_cli_list_json(tmp_path, capsys):
    path = tmp_path / "missing.yaml"
    main(["--path", str(path), "register", "x", "--kind", "engine"***REMOVED***)
    capsys.readouterr()  # сбросить вывод register
    assert main(["--path", str(path), "list", "--json"***REMOVED***) == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload[0***REMOVED***["item_id"***REMOVED*** == "x"
    assert payload[0***REMOVED***["status"***REMOVED*** == REGISTERED
...
def test_cli_mark_implemented_unknown_item_clean_exit(tmp_path, capsys):
    ...
    code = main(["--path", str(path), "mark-implemented", "ghost_item",
                 "--implementation", "scripts_01/x.py"***REMOVED***)
...
def test_cli_mark_prompt_written_unknown_item_clean_exit(tmp_path, capsys):
    ...
    code = main(["--path", str(path), "mark-prompt-written", "ghost_item",
                 "--prompt", "pompts_11/promt99.md"***REMOVED***)
    captured = capsys.readouterr()
    assert code == 1
    assert "ghost_item" in captured.err
    assert "Traceback" not in captured.out and "Traceback" not in captured.err
...
def test_cli_mark_implemented(tmp_path, capsys):
    path = tmp_path / "missing.yaml"
    main(["--path", str(path), "register", "research_web", "--kind", "tool"***REMOVED***)
    assert main(["--path", str(path), "mark-implemented", "research_web",
                 "--implementation", "scripts_01/research_web.py"***REMOVED***) == 0
    reg = MissingRegistry(path)
...
def test_cli_check_valid(tmp_path, capsys):
    path = tmp_path / "missing.yaml"
    main(["--path", str(path), "seed"***REMOVED***)
    assert main(["--path", str(path), "check"***REMOVED***) == 0
...
def test_cli_check_invalid(tmp_path, capsys):
    ...
    path = tmp_path / "missing.yaml"
    main(["--path", str(path), "register", "x", "--kind", "tool"***REMOVED***)
    reg = MissingRegistry(path)
    reg._data["x"***REMOVED***["status"***REMOVED*** = IMPLEMENTED  # вручную ломаем инвариант
    reg._save()
    assert main(["--path", str(path), "check"***REMOVED***) == 1
...
def test_related_prompts_roundtrip(registry):
    ...
    registry.register_missing(
        ...
        related_prompts=["pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***,
    )
    item = registry.get("intelligence_integration")
    assert item.prompt_path == "pompts_11/085_19_close_intelligence_loop.md"
    assert item.related_prompts == ["pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***
    d = item.to_dict()
    assert d["related_prompts"***REMOVED*** == ["pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***
...
def test_validate_schema_related_prompts_not_list(registry):
    ...
    reg._data = {"x": {"item_id": "x", "kind": "tool", "status": REGISTERED,
                        "related_prompts": "not-a-list"***REMOVED******REMOVED***
...
def test_validate_schema_related_prompts_empty_string(registry):
    ...
    reg._load_error = None
    reg._data = {"x": {"item_id": "x", "kind": "tool", "status": REGISTERED,
                        "related_prompts": [""***REMOVED******REMOVED******REMOVED***
...
def test_register_with_backfill_true(registry):
    ...
    d = item.to_dict()
    assert d["backfill"***REMOVED*** is True
    restored = MissingItem.from_dict(d)
...
def test_validate_schema_backfill_true_implemented_clean(registry):
    ...
    reg._data = {"x": {"item_id": "x", "kind": "tool", "status": IMPLEMENTED,
                        "implementation": "scripts_01/x.py",
                        "backfill": True***REMOVED******REMOVED***
    assert reg.validate_schema() == [***REMOVED***


def test_from_dict_backfill_default_false():
    item = MissingItem.from_dict({"item_id": "x", "kind": "tool", "status": REGISTERED***REMOVED***)
...
def test_cli_register_backfill(tmp_path):
    ...
    assert main(["--path", str(path), "register", "role_executor", "--kind", "module",
                 "--implementation", "core_02/role_executor.py",
                 "--status", "implemented", "--backfill"***REMOVED***) == 0
    reg = MissingRegistry(path)
...
def test_cli_add_related_prompt(tmp_path, capsys):
    ...
    assert main(["--path", str(path), "add-related-prompt", "x",
                 "--prompt", "pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***) == 0
    reg = MissingRegistry(path)
    assert reg.get("x").related_prompts == ["pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***
...
def test_cli_mark_implemented_related_prompt(tmp_path, capsys):
    ...
    assert main(["--path", str(path), "mark-implemented", "x",
                 ...
                 "--related-prompt", "pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***) == 0
    reg = MissingRegistry(path)
    item = reg.get("x")
    assert item.status == IMPLEMENTED
...
def test_cli_list_json_includes_related_prompts(tmp_path, capsys):
    ...
    main(["--path", str(path), "add-related-prompt", "x",
          "--prompt", "pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***)
    capsys.readouterr()
    assert main(["--path", str(path), "list", "--json"***REMOVED***) == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload[0***REMOVED***["related_prompts"***REMOVED*** == ["pompts_11/084_19_intelligence_integration_forensics.md"***REMOVED***
