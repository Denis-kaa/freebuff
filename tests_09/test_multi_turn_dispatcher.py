...
def _make_pending_result(pending_text: str, success: bool = True) -> Dict[str, Any***REMOVED***:
    """Fake `.freebuff_result` JSON с pending_task. Wrapper returns this as
    result['result'***REMOVED*** (raw JSON text), which `_extract_pending_task` parses.
    """
    return {
        "success": success,
        "output": f"Output for {pending_text[:20***REMOVED******REMOVED***",
        "result": json.dumps({
            "status": "ok",
...
def _make_done_result() -> Dict[str, Any***REMOVED***:
...
def _sequence_launcher(responses: List[Dict[str, Any***REMOVED******REMOVED***) -> Callable[[str, str, int, str***REMOVED***, Dict[str, Any***REMOVED******REMOVED***:
    """Fake launcher returning different responses per call (для cycle-tests)."""
    state = {"call_idx": 0, "calls": [***REMOVED******REMOVED***

    def launcher(prompt: str, cwd: str, timeout: int, model: str = "auto") -> Dict[str, Any***REMOVED***:
        idx = state["call_idx"***REMOVED***
        state["call_idx"***REMOVED*** += 1
        if idx >= len(responses):
            # Default: terminal done (для overshoot)
            resp = _make_done_result()
        else:
            resp = responses[idx***REMOVED***
        state["calls"***REMOVED***.append(
            {"prompt_len": len(prompt), "model": model, "response": resp***REMOVED***
        )
        return resp
...
def test_extract_pending_task_parses_string_field() -> None:
    raw = json.dumps({"status": "ok", "pending_task": "Какой порт нужен?"***REMOVED***)
    result = {"success": True, "result": raw, "output": "", "duration": 1.0***REMOVED***
...
def test_extract_pending_task_parses_discriminated_dict() -> None:
    # Task 2 (promt 61): new dict format {type, text***REMOVED*** routes kind explicitly.
    raw = json.dumps(
        {"status": "ok", "pending_task": {"type": "clarification", "text": "Уточни порт?"***REMOVED******REMOVED***
    )
    result = {"success": True, "result": raw, "output": "", "duration": 1.0***REMOVED***
...
def test_extract_pending_task_dict_work_kind_and_invalid_kind() -> None:
    # Task 2 (promt 61): dict {type, text***REMOVED*** также поддерживает kind="work"
    # и отклоняет неизвестные kinds (guard `kind in ("work", "clarification")`).
    work_raw = json.dumps(
        {"status": "ok", "pending_task": {"type": "work", "text": "Продолжи работу"***REMOVED******REMOVED***
    )
    assert _extract_pending_task(
        {"success": True, "result": work_raw, "output": ""***REMOVED***
    ) == ("work", "Продолжи работу")

    bad_raw = json.dumps(
        {"status": "ok", "pending_task": {"type": "bogus", "text": "x"***REMOVED******REMOVED***
    )
    assert _extract_pending_task(
        {"success": True, "result": bad_raw, "output": ""***REMOVED***
    ) is None


def test_extract_pending_task_returns_none_when_field_missing() -> None:
    raw = json.dumps({"status": "ok", "message": "done"***REMOVED***)
    result = {"success": True, "result": raw, "output": "", "duration": 1.0***REMOVED***
    assert _extract_pending_task(result) is None


def test_extract_pending_task_handles_malformed_json() -> None:
    result = {"success": True, "result": "not json {{{", "output": ""***REMOVED***
    assert _extract_pending_task(result) is None


def test_extract_pending_task_empty_string_is_ignored() -> None:
    raw = json.dumps({"status": "ok", "pending_task": "   "***REMOVED***)
    result = {"success": True, "result": raw, "output": ""***REMOVED***
    assert _extract_pending_task(result) is None


def test_extract_pending_task_non_string_value_ignored() -> None:
    raw = json.dumps({"status": "ok", "pending_task": ["list", "not", "str"***REMOVED******REMOVED***)
    result = {"success": True, "result": raw, "output": ""***REMOVED***
    assert _extract_pending_task(result) is None


def test_extract_pending_task_result_field_missing_is_none() -> None:
    result = {"success": True, "output": "", "duration": 1.0***REMOVED***
    assert _extract_pending_task(result) is None
...
def test_scan_resumable_finds_running_pending_in_running_dir(
    ...
    assert len(found) == 1
    assert found[0***REMOVED***.task_id
...
def test_first_dispatch_with_pending_task_keeps_file_in_running(
    ...
    ***REMOVED***)
    ...
    assert result["status"***REMOVED*** == "multi-turn-pending"
    assert result["iteration"***REMOVED*** == 1
    assert result["max_iterations"***REMOVED*** == 3
    assert result["pending_task"***REMOVED*** == "Какой порт?"

    # File should now live in running/ (not user/, not done/).
    targets = [
        f for f in (prompts_dir() / "running").glob("*.md")
    ***REMOVED***
    assert len(targets) == 1, f"expected exactly 1 file in running/, got {targets***REMOVED***"
    text = targets[0***REMOVED***.read_text(encoding="utf-8")
    assert "**Status:** running-pending" in text
    assert "**Iteration:** 2" in text
...
def test_second_dispatch_continues_multi_turn_cycle(
    ...
    ***REMOVED***)
    ...
    assert r1["status"***REMOVED*** == "multi-turn-pending"
    assert r1["iteration"***REMOVED*** == 1

    # Iter 2 (no user/ but has running-pending)
    r2 = dispatch_one(launcher=launcher, timeout=60, send_tg=False)
    assert r2["status"***REMOVED*** == "multi-turn-pending"
    assert r2["iteration"***REMOVED*** == 2
...
def test_max_iterations_reached_forces_failed_on_first_iter(
    ...
    ***REMOVED***)
    ...
    assert r["status"***REMOVED*** == "failed-multi-turn-max"
    assert r["reason"***REMOVED*** == "max_iterations_reached"
...
def test_max_iterations_reached_after_iter_3(
    ...
    ***REMOVED***)
    ...
    assert r3["status"***REMOVED*** == "failed-multi-turn-max"
...
def test_no_pending_task_first_dispatch_terminates_to_done(
    ...
    launcher, _ = _sequence_launcher([_make_done_result()***REMOVED***)
...
def test_launcher_failure_first_dispatch_terminates_to_failed(
    ...
    def fail_invoke(prompt: str, cwd: str, timeout: int) -> Dict[str, Any***REMOVED***:
        return {"success": False, "error": "codebuff OOM", "output": ""***REMOVED***

    write_user_prompt("bad task", chat_id=42)
    r = dispatch_one(launcher=fail_invoke, timeout=60, send_tg=False)
    assert r["status"***REMOVED*** == "failed"
    assert len(list((prompts_dir() / "failed").glob("*.md"))) == 1


def test_dispatch_with_empty_queue_returns_noop(ws_root: Path) -> None:
    """Ни user/, ни running/ → noop."""
    r = dispatch_one(launcher=lambda p, c, t, m="auto": {***REMOVED***, timeout=60, send_tg=False)
    assert r["handled"***REMOVED*** is False
