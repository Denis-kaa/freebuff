#!/usr/bin/env python3
"""
Tests for scripts_01/seed_knowledge.py — seeding the Knowledge Memory layer.
"""
...
from scripts_01.seed_knowledge import seed, _safe_key, _collect_doc_sources
...
class TestSeedKnowledge:
    def test_seed_creates_memory_entries(self, tmp_path: Path):
        ...
        (tmp_path / "docs_10" / "AGENTS.md").write_text("# AGENTS\nAgent guide.")
...
class TestCollectDocSources:
    def test_core_manifests_plus_discovered_docs(self, tmp_path: Path):
        """_collect_doc_sources includes core manifests and docs_10/*.md."""
        (tmp_path / "README.md").write_text("# README")
        (tmp_path / "BUFFY.md").write_text("# BUFFY")
        (tmp_path / "docs_10").mkdir()
        (tmp_path / "docs_10" / "AGENTS.md").write_text("# AGENTS")
        (tmp_path / "docs_10" / "RULES.md").write_text("# RULES")

        sources = _collect_doc_sources(tmp_path)
        assert "README.md" in sources
        assert "BUFFY.md" in sources
        assert "docs_10/AGENTS.md" in sources
    ...
    def test_excluded_patterns_are_skipped(self, tmp_path: Path):
        ...
        (tmp_path / "README.md").write_text("# README")
        (tmp_path / "docs_10").mkdir()
        (tmp_path / "docs_10" / "ops").mkdir()
        (tmp_path / "docs_10" / "audits").mkdir()
        (tmp_path / "docs_10" / "ops" / "AGENTS.md").write_text("# AGENTS")
        (tmp_path / "docs_10" / "audits" / "AUDIT_2026-07-28.md").write_text("# AUDIT")
        (tmp_path / "docs_10" / "ops" / "TASK_TEMPLATE.md").write_text("# TEMPLATE")

        sources = _collect_doc_sources(tmp_path)
        assert "docs_10/ops/AGENTS.md" in sources
        assert "docs_10/audits/AUDIT_2026-07-28.md" not in sources
        assert "docs_10/ops/TASK_TEMPLATE.md" not in sources
