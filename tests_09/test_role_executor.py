...
class TestRoleExecutorRegistry:
    def test_register_and_get(self):
        reg = RoleExecutorRegistry([LisaExecutor()***REMOVED***)
        assert "lisa" in reg
        assert isinstance(reg.get("lisa"), LisaExecutor)
        assert reg.role_ids() == ["lisa"***REMOVED***
        assert len(reg) == 1
...
class TestLisaExecutor:
    def test_generates_lisa_report_from_brief(self, project):
        ...
        assert created == ["lisa_report.md"***REMOVED***
...
class TestRunChainGenerate:
    def test_generate_mode_materializes_missing_lisa(self, tmp_path, project,
        ...
        _write_yaml_registry(rp, [{"id": "lisa", "outputs": ["lisa_report.md"***REMOVED******REMOVED******REMOVED***)
    ...
    def test_generate_mode_executor_failure_marks_gen_failed(self, tmp_path,
        ...
        result = facade.run_chain(
            ...
            registry_path=rp,
            light_mode="generate",
            executor_registry=RoleExecutorRegistry([BoomExecutor()***REMOVED***),
        )
        stage = result.chain[0***REMOVED***
        assert stage.mode == "generate"
...
class _FakeGateway:
    ...
    def __init__(self, content: str = "", error: Exception | None = None):
        self.content = content
        self.error = error
        self.calls: list[dict***REMOVED*** = [***REMOVED***

    def generate_by_capabilities(self, capabilities, messages, **kwargs):
        self.calls.append({"capabilities": capabilities, "messages": messages***REMOVED***)
...
class _FakeBlueprint:
    ...
    def __init__(self, sections: dict | None = None):
        self.sections = sections or {***REMOVED***
...
class _FakeCorpus:
    ...
    def __init__(self, sections: dict | None = None, hint: list | None = None):
        ...
        ***REMOVED***
        self.hint = hint or ["summarize"***REMOVED***
...
class TestLlmRoleExecutor:
    def test_generates_files_from_file_blocks(self, project):
        ...
        assert sorted(created) == ["brief.md", "parsed_requirements.md"***REMOVED***
    ...
    def test_passes_blueprint_prompt_and_capabilities(self, project):
        ...
        corpus = _FakeCorpus(hint=["summarize", "explain"***REMOVED***)
        ex = _explainer_executor(gw, corpus=corpus)
        ex.execute(project, "explainer")
        assert len(gw.calls) == 1
        call = gw.calls[0***REMOVED***
        assert call["capabilities"***REMOVED*** == ["summarize", "explain"***REMOVED***
        system, user = call["messages"***REMOVED***
        assert system["role"***REMOVED*** == "system"
        assert "do the thing" in system["content"***REMOVED***
        assert user["role"***REMOVED*** == "user"
        assert "@@FILE" in user["content"***REMOVED***
        assert "brief.md" in user["content"***REMOVED***
    ...
    def test_ignore_unauthorized_files(self, project):
        ...
        ex = _explainer_executor(gw)
        created = ex.execute(project, "explainer")
        assert created == ["brief.md"***REMOVED***
    ...
    def test_fail_safe_on_gateway_error_returns_empty(self, project):
        ...
        ex = _explainer_executor(gw)
        assert ex.execute(project, "explainer") == [***REMOVED***
    ...
    def test_single_output_fallback_writes_raw_content(self, project):
        ...
        assert created == ["risk_matrix.md"***REMOVED***
    ...
    def test_glob_output_matches_concrete_filename(self, project):
        ...
        assert created == ["adr/ADR-001.md"***REMOVED***
    ...
    def test_run_chain_generate_uses_llm_executor(self, tmp_path, project, registry):
        ...
        ***REMOVED***)
        ...
        result = facade.run_chain(
            ...
            registry_path=rp,
            light_mode="generate",
            executor_registry=RoleExecutorRegistry([ex***REMOVED***),
        )
        stage = result.chain[0***REMOVED***
