"""
Tests for scripts_01/engineering_memory.py — Engineering Memory Engine.
"""
...
from scripts_01.engineering_memory import EMEngine, DraftNotFoundError
...
def em(tmp_path):
    ...
    templates_dir = project_root / "docs_10" / "engineering-memory" / "templates"
...
class TestEMFinalize:
    ...
    def test_finalize_creates_decision_subdir(self, em):
        ...
        path = em.finalize_draft(draft_id)
        assert "docs_10/engineering-memory/decisions" in str(path)
    ...
    def test_finalize_creates_incident_subdir(self, em):
        ...
        path = em.finalize_draft(draft_id)
        assert "docs_10/engineering-memory/incidents" in str(path)
...
class TestEMDraftManagement:
    ...
    def test_discard_non_em_key_fails(self, em):
        em._memory.store(MemoryLevel.PROJECT, "regular_key", "data_13")
...
class TestEMDecisionIndex:
    ...
    def test_finalize_decision_updates_index(self, em):
        ...
        index_path = em._root / "docs_10" / "decisions" / "DECISIONS.md"
    ...
    def test_index_contains_existing_and_new_adrs(self, em):
        # Ручной ADR
        decisions_dir = em._root / "docs_10" / "engineering-memory" / "decisions"
