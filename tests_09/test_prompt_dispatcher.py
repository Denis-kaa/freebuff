...
def _ok_launcher(prompt: str, cwd: str, timeout: int, model: str = "auto") -> dict:
    return {"success": True, "output": "Готово", "duration": 1.2***REMOVED***


def _fail_launcher(prompt: str, cwd: str, timeout: int, model: str = "auto") -> dict:
    return {"success": False, "output": "", "error": "boom", "duration": 0.5***REMOVED***
...
def test_dispatch_one_noop_when_empty(queue_root):
    ...
    assert result["status"***REMOVED*** == "noop"


def test_dispatch_one_success_moves_to_done(queue_root):
    write_user_prompt("Задача A", chat_id=111, priority=5)
    result = dispatch_one(launcher=_ok_launcher, send_tg=False)
    assert result["handled"***REMOVED*** is True
    assert result["status"***REMOVED*** == "done"
    counts = queue_counts()
    assert counts["pending"***REMOVED*** == 0
    assert counts["done"***REMOVED*** == 1
    done_meta = parse_prompt(queue_dir("done") / result["path"***REMOVED***.rsplit("/", 1)[-1***REMOVED***)
...
def test_dispatch_one_failure_moves_to_failed(queue_root):
    ...
    assert result["handled"***REMOVED*** is True
    assert result["status"***REMOVED*** == "failed"
    assert "boom" in result["report"***REMOVED***
    counts = queue_counts()
    assert counts["failed"***REMOVED*** == 1
    assert counts["pending"***REMOVED*** == 0
...
def test_dispatch_multi_turn_defers_on_single_instance_blocker(queue_root):
    ...
    assert result["handled"***REMOVED*** is True
    assert result["status"***REMOVED*** == "deferred_single_instance"
    counts = queue_counts()
    assert counts["running"***REMOVED*** == 1  # вернулся в running/, не failed
    assert counts["failed"***REMOVED*** == 0
    ...
    assert result2["status"***REMOVED*** == "done"
    assert queue_counts()["done"***REMOVED*** == 1
...
def test_dispatch_one_defers_on_single_instance_blocker(queue_root):
    ...
    assert result["status"***REMOVED*** == "deferred_single_instance"
    counts = queue_counts()
    assert counts["pending"***REMOVED*** == 1  # вернулась в user/
    assert counts["failed"***REMOVED*** == 0   # не ложный failed
...
def test_dispatch_one_launcher_exception_treated_as_failed(queue_root):
    ...
    assert result["status"***REMOVED*** == "failed"
    assert "launcher crashed" in result["report"***REMOVED***
    assert queue_counts()["failed"***REMOVED*** == 1
...
def test_dispatch_all_processes_everything(queue_root):
    ...
    assert all(r["status"***REMOVED*** == "done" for r in results)
    counts = queue_counts()
    assert counts["pending"***REMOVED*** == 0
    assert counts["done"***REMOVED*** == 2
...
def test_dispatch_all_respects_max_tasks(queue_root):
    ...
    assert len(results) == 2
    assert queue_counts()["pending"***REMOVED*** == 1
...
def test_format_report_ok_and_fail(queue_root):
    ...
    ok = _format_report(
        ok_meta,
        {"success": True, "output": "вывод", "duration": 3.0***REMOVED***,
    )
    ...
    bad = _format_report(
        bad_meta,
        {"success": False, "output": "", "error": "ошибка"***REMOVED***,
    )
...
def test_default_launcher_uses_phase_based_launch_and_wait(monkeypatch):
    ...
    def _fake(prompt, cwd, timeout, model="auto"):
        ...
        return {"success": True, "result": "OK", "duration": 1.0***REMOVED***
...
def test_dispatch_one_forwards_model_to_launcher(queue_root):
    ...
    def _capture(prompt: str, cwd: str, timeout: int, model: str = "auto") -> dict:
        ...
        return {"success": True, "output": "OK", "duration": 0.5***REMOVED***
...
def test_dispatch_one_backoff_skips_spawn_when_instance_busy(queue_root, monkeypatch):
    ...
    assert result["handled"***REMOVED*** is False
    assert result["status"***REMOVED*** == "deferred_single_instance_backoff"
    counts = queue_counts()
    assert counts["pending"***REMOVED*** == 1  # задача осталась в user/, не потеряна
    assert counts["failed"***REMOVED*** == 0   # не ложный failed
...
def test_dispatch_all_breaks_on_backoff(queue_root, monkeypatch):
    ...
    assert results[0***REMOVED***["status"***REMOVED*** == "deferred_single_instance_backoff"
    counts = queue_counts()
    assert counts["pending"***REMOVED*** == 2  # обе задачи целы
    assert counts["failed"***REMOVED*** == 0
...
def test_deferred_at_stamp_on_blocker(queue_root):
    ...
    pending = scan_pending()[0***REMOVED***
...
def test_dispatch_all_breaks_on_wrapper_blocker(queue_root):
    ...
    results = dispatch_all(launcher=_launcher, send_tg=False)
    assert len(results) == 1
    assert results[0***REMOVED***["status"***REMOVED*** == "deferred_single_instance"
    assert len(calls) == 1  # вторая задача НЕ запускается — инстанс занят
    counts = queue_counts()
    assert counts["pending"***REMOVED*** == 2  # обе задачи целы (1 вернулась, 2 не тронута)
    assert counts["failed"***REMOVED*** == 0
...
def test_backoff_streak_increments_and_notifies_once(queue_root, monkeypatch):
    ...
    for _ in range(2):
        ...
        assert r["status"***REMOVED*** == "deferred_single_instance_backoff"
    assert len(tg_calls) == 0
    meta = scan_pending()[0***REMOVED***
    ...
    assert scan_pending()[0***REMOVED***.backoff_streak == 5
...
def test_backoff_notify_disabled_when_zero(queue_root, monkeypatch):
    ...
    assert len(tg_calls) == 0
    assert scan_pending()[0***REMOVED***.backoff_streak == 10
    assert scan_pending()[0***REMOVED***.backoff_notified is False
...
def _clarifying_launcher_factory(text: str = "\u0443\u0442\u043e\u0447\u043d\u0438 \u0432\u0432\u043e\u0434\u043d\u044b\u0435"):
    ...
    def _launcher(prompt, cwd, timeout, model="auto"):
        return {
            "success": True,
            "result": _json.dumps({"pending_task": {"type": "clarification", "text": text***REMOVED******REMOVED***),
...
def _legacy_string_pending_launcher_factory(text: str = "legacy bare string"):
    ...
    def _launcher(prompt, cwd, timeout, model="auto"):
        return {
            "success": True,
            "result": _json.dumps({"pending_task": text***REMOVED***),
...
def test_clarification_budget_doesnt_consume_work_budget(queue_root):
    ...
    assert r1["handled"***REMOVED*** is True
    assert r1["status"***REMOVED*** == "multi-turn-pending-clarification"
    assert r1["clarification_count"***REMOVED*** == 1
    assert r1["iteration"***REMOVED*** == 1  # work iter UNCHANGED
    assert r1["max_clarifications"***REMOVED*** == 10
    assert r1["max_iterations"***REMOVED*** == 3

    # \u0421\u043b\u0435\u0434\u0443\u044e\u0449\u0438\u0435 5 \u0442\u0438\u043a\u043e\u0432: 1\u21922\u21923\u21924\u21925\u21926
    for expected_cc in range(2, 7):
        r = dispatch_one(
            launcher=_clarifying_launcher_factory(f"Q{expected_cc***REMOVED***"), send_tg=False
        )
        assert r["status"***REMOVED*** == "multi-turn-pending-clarification", (
            f"cc={expected_cc***REMOVED***: expected multi-turn-pending-clarification, got {r['status'***REMOVED******REMOVED***"
        )
        assert r["clarification_count"***REMOVED*** == expected_cc, f"cc={expected_cc***REMOVED***: bump failed"
        assert r["iteration"***REMOVED*** == 1, f"cc={expected_cc***REMOVED***: work iter changed (budget leak!)"

    # \u0424\u0430\u0439\u043b \u0432 running/: iteration \u0432 \u0448\u0430\u043f\u043a\u0435 \u043e\u0441\u0442\u0430\u0451\u0442\u0441\u044f 1/3
    running_files = list(queue_dir("running").glob("*.md"))
    assert len(running_files) == 1, f"running should hold 1 file, got {len(running_files)***REMOVED***"
    running_meta = parse_prompt(running_files[0***REMOVED***)
...
def test_clarification_budget_exhausted_force_fails_without_touching_work_iter(queue_root):
    ...
    assert final["handled"***REMOVED*** is True
    assert final["status"***REMOVED*** == "failed-multi-turn-max-clarification"
    assert final["reason"***REMOVED*** == "max_clarifications_reached"
    assert final["clarification_count"***REMOVED*** == 10
    assert final["iteration"***REMOVED*** == 1, "work iter was modified despite clarification-only failure"
    assert final["max_iterations"***REMOVED*** == 3

    counts = queue_counts()
    assert counts["failed"***REMOVED*** == 1, "file should be in failed/"
    assert counts["running"***REMOVED*** == 0, "file should NOT be in running/ (force-failed)"
...
def test_legacy_bare_string_pending_task_routes_as_work(queue_root):
    ...
    r1 = dispatch_one(launcher=launcher, send_tg=False)
    assert r1["handled"***REMOVED*** is True
    assert r1["status"***REMOVED*** == "multi-turn-pending"  # legacy \u2192 work routing
...
def test_clarification_sets_running_resumable_status(queue_root):
    ...
    m = parse_prompt(running_files[0***REMOVED***)
    assert m.status == "running-resumable", f"\u043e\u0436\u0438\u0434\u0430\u043b\u043e\u0441\u044c running-resumable, \u043f\u043e\u043b\u0443\u0447\u0438\u043b\u0438 {m.status***REMOVED***"
    ...
    assert len(resumable_list) == 1, f"scan_resumable \u0434\u043e\u043b\u0436\u0435\u043d \u0432\u0435\u0440\u043d\u0443\u0442\u044c \u043e\u0434\u0438\u043d \u0444\u0430\u0439\u043b, got {len(resumable_list)***REMOVED***"
...
def test_process_answer_resumes_running_resumable_task(queue_root):
    ...
    r1 = dispatch_one(launcher=launcher, send_tg=False)
    assert r1["status"***REMOVED*** == "multi-turn-pending-clarification"
    task_id = r1.get("path", "").split("/")[-1***REMOVED*** if False else None  # extract from meta
    running_files = list(queue_dir("running").glob("*.md"))
    assert len(running_files) == 1
    meta_before = parse_prompt(running_files[0***REMOVED***)
    ...
    assert result["task_id"***REMOVED*** == task_id
    assert result["old_status"***REMOVED*** == "running-resumable"
    assert result["new_status"***REMOVED*** == "running-pending"
    assert result["old_iteration"***REMOVED*** == 1
    assert result["new_iteration"***REMOVED*** == 2

    # \u0424\u0430\u0439\u043b \u0432 running/: status \u0441\u0431\u0440\u043e\u0448\u0435\u043d, iter +1, cc reset
    running_files_after = list(queue_dir("running").glob("*.md"))
    meta_after = parse_prompt(running_files_after[0***REMOVED***)
    assert meta_after.status == "running-pending", f"\u043e\u0436\u0438\u0434\u0430\u043b\u043e\u0441\u044c running-pending, got {meta_after.status***REMOVED***"
    ...
    full_text = running_files_after[0***REMOVED***.read_text(encoding="utf-8")
...
def test_process_answer_rejects_non_resumable_status(queue_root):
    ...
    assert r1["status"***REMOVED*** == "multi-turn-pending", f"unexpected init status: {r1***REMOVED***"
    work_meta = parse_prompt(list(queue_dir("running").glob("*.md"))[0***REMOVED***)
    ...
    assert "not awaiting answer" in result["error"***REMOVED***, (
        f"ожидалось 'not awaiting answer', got {result***REMOVED***"
    )
    assert result["current_status"***REMOVED*** == "running-pending"
...
def test_process_answer_returns_not_found_for_user_dir_task(queue_root):
    ...
    assert result["ok"***REMOVED*** is False
    assert "not found in running/" in result["error"***REMOVED***
