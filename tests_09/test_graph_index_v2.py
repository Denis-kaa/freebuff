...
def load_graph_index_v2():
    ...
    sys.modules["graph_index_v2"] = mod
...
class TestGraphIndexV2(unittest.TestCase):
    ...
    def test_link_artifact_to_kg_and_retrieve(self):
        ...
        source_ids = {row[0] for row in related}
        self.assertIn(f"artifact:{artifact}", source_ids)
    ...
    def test_link_artifact_to_kg_idempotent(self):
        ...
        node = self.gi.get_node(f"artifact:{artifact}")
    ...
    def test_interlink_vkusvill_research_emits_links(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for fname in ["01_business_scale.md", "02_supply_chain_economics.md"]:
                with open(os.path.join(tmpdir, "subproj", fname), "w") as f:
                    f.write(f"# sample {fname}\n")
            n = self.gi.interlink(tmpdir, file_extensions={".md"})
    ...
    def test_version_chain_detection(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for fname in ["DOC_v1.0.md", "DOC_v1.1.md"]:
                with open(os.path.join(tmpdir, fname), "w") as f:
                    f.write(f"# {fname}\n")
            n = self.gi.interlink(tmpdir, file_extensions={".md"})
