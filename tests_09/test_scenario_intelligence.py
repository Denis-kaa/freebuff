...
def test_13_routing_hard_gate_for_code_opp(tmp_path: Path) -> None:
    """PHASE 12 G-11.6 (CANONICAL_ENGINE_ROUTING_V1.md): capability-match hard gate.

    Opportunity with ``capability='code'`` MUST route through the scenario whose
    declared capability IS ``code`` (canonical Phase 11 TestFactory route — domain
    developer/tester/verifier). Cross-domain scenarios (e.g. ``article_generation``)
    are filtered BEFORE ranking — even if their relevance score is higher — because
    a code opportunity cannot truthfully execute through a content-writing scenario.

    Without this gate (Phase 11 limitation, test_13b lenient xfail), SI returned
    the highest-relevance scenario regardless of capability-match — wrong-scenario
    routing risk. After Phase 12 G-11.6 the SI ranking is capability-correct.
    """
    factory_registry = _FakeFactoryRegistry({
        "code": [("test_factory", "verifier_forge")***REMOVED***,
        "article_generation": [("articles_factory", "article_forge")***REMOVED***,
    ***REMOVED***)
    scenarios = [
        (_FakeScenario("scenario_content", ["article_generation"***REMOVED***), _FakeRole("writer", ["article_generation"***REMOVED***), 0.9),
        (_FakeScenario("scenario_code", ["code"***REMOVED***), _FakeRole("developer", ["code"***REMOVED***), 0.6),
    ***REMOVED***
    registry = _FakeScenarioRegistry(scenarios)
    ...
    si = ScenarioIntelligence(
        ...
        memory_store=None,  # history=neutral
        history_store=DecisionHistoryStore(tmp_path / "history_g116.yaml"),
    )
    # Opportunity with capability=code via the canonical _make_opp(scenario_cap=...) helper.
    opp_code = _make_opp(scenario_cap="code")
    decision = si.select(opp_code, persist=False, available_only=True)

    assert decision.status == "selected", (
        f"G-11.6 hard-gate failure: opp(capability=code) returned status={decision.status!r***REMOVED*** "
        f"instead of 'selected'. Candidates were filtered out as infeasible — but "
        f"scenario_code with capability=code MUST be available. Reasons: {decision.reasons***REMOVED***"
    )
    assert decision.selected_scenario_id == "scenario_code", (
        f"G-11.6 violation — opp(capability=code) routed to {decision.selected_scenario_id!r***REMOVED*** "
        f"instead of canonical 'scenario_code'. Cross-domain hard-gate failed. "
        f"Reasons: {decision.reasons***REMOVED***, Evidence: {decision.evidence***REMOVED***"
    )
    assert decision.capability == "code"
    assert decision.factory_id == "test_factory"
    assert decision.forge_id == "verifier_forge"

    # Also assert that the cross-domain 'scenario_content' was correctly excluded
    # (its feasibility=0.0 per HARD GATE).
    evidence = decision.evidence
    assert evidence.get("opp_capability") == "code"


def test_13c_multi_cap_set_membership_positive(tmp_path: Path) -> None:
    """PHASE 13 G-11.6 (CANONICAL_ENGINE_ROUTING_V1.md §8 D-2): scenario with
    multi-cap tuple ``[code, refactor***REMOVED***`` MUST accept ``opp(capability=refactor)``
    via set-membership semantics.

    Pre-refactor (v5.189.30, first-element): ``scenario_fullstack.capabilities``
    was effectively truncated to ``[code***REMOVED***`` (first element only). ``opp(refactor)``
    would have DOMAIN_MISMATCH (FALSE NEGATIVE). Phase 13 set-membership
    semantics accept ANY scenario_caps element matching opp.capability.
    """
    factory_registry = _FakeFactoryRegistry({
        "code": [("test_factory", "verifier_forge")***REMOVED***,
        "refactor": [("test_factory", "refactor_forge")***REMOVED***,
    ***REMOVED***)
    scenarios = [
        (_FakeScenario("scenario_fullstack", ["code", "refactor"***REMOVED***),
         _FakeRole("fullstack_dev", ["code", "refactor"***REMOVED***), 0.7),
    ***REMOVED***
    si = ScenarioIntelligence(
        registry=_FakeScenarioRegistry(scenarios),
        factory_registry=factory_registry,
        memory_store=None,
        history_store=DecisionHistoryStore(tmp_path / "history_g116c.yaml"),
    )

    # opp with capability=refactor (NOT first-element "code") MUST be feasible.
    opp_refactor = _make_opp(scenario_cap="refactor")
    decision = si.select(opp_refactor, persist=False, available_only=True)

    assert decision.status == "selected", (
        f"Phase 13 G-11.6 set-membership failure: scenario_fullstack should "
        f"accept opp(capability=refactor) but got status={decision.status!r***REMOVED***. "
        f"Reasons: {decision.reasons***REMOVED***"
    )
    assert decision.selected_scenario_id == "scenario_fullstack"
    assert decision.factory_id == "test_factory"
    assert decision.capability in ("code", "refactor")  # either is a valid full-set match


def test_13d_multi_cap_cross_domain_rejected(tmp_path: Path) -> None:
    """PHASE 13 G-11.6 (CANONICAL_ENGINE_ROUTING_V1.md §8 D-2): scenarios with
    ``[code, refactor***REMOVED***`` MUST STILL reject ``opp(capability=image_generation)``
    — cross-domain filter preserved (I-6 invariant intact).
    """
    factory_registry = _FakeFactoryRegistry({
        "code": [("test_factory", "verifier_forge")***REMOVED***,
        "refactor": [("test_factory", "refactor_forge")***REMOVED***,
    ...
    ***REMOVED***
    si = ScenarioIntelligence(
        ...
        factory_registry=factory_registry,
        memory_store=None,
        history_store=DecisionHistoryStore(tmp_path / "history_g116d.yaml"),
    )

    # opp with capability=image_generation (NOT in scenario_caps) MUST be infeasible.
    opp_img = _make_opp(scenario_cap="image_generation")
    decision = si.select(opp_img, persist=False, available_only=True)

    assert decision.status == "unavailable", (
        f"Phase 13 G-11.6 cross-domain filter broken: scenario_fullstack should "
        f"NOT accept opp(capability=image_generation) but got status={decision.status!r***REMOVED***. "
        f"Reasons: {decision.reasons***REMOVED***"
    )
