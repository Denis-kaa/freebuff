...
def test_13_routing_hard_gate_for_code_opp(tmp_path: Path) -> None:
    ...
    assert decision.status == "selected", (
        f"G-11.6 hard-gate failure: opp(capability=code) returned status={decision.status!r} "
        f"instead of 'selected'. Candidates were filtered out as infeasible — but "
        f"scenario_code with capability=code MUST be available. Reasons: {decision.reasons}"
    )
    assert decision.selected_scenario_id == "scenario_code", (
        f"G-11.6 violation — opp(capability=code) routed to {decision.selected_scenario_id!r} "
        f"instead of canonical 'scenario_code'. Cross-domain hard-gate failed. "
        f"Reasons: {decision.reasons}, Evidence: {decision.evidence}"
    )
...
def test_13c_multi_cap_set_membership_positive(tmp_path: Path) -> None:
    ...
    multi-cap tuple ``[code, refactor]`` MUST accept ``opp(capability=refactor)``
    via set-membership semantics.

    Pre-refactor (v5.189.30, first-element): ``scenario_fullstack.capabilities``
    was effectively truncated to ``[code]`` (first element only). ``opp(refactor)``
    ...
    assert decision.status == "selected", (
        ...
        f"accept opp(capability=refactor) but got status={decision.status!r}. "
...
def test_13d_multi_cap_cross_domain_rejected(tmp_path: Path) -> None:
    ...
    ``[code, refactor]`` MUST STILL reject ``opp(capability=image_generation)``
    ...
    assert decision.status == "unavailable", (
        ...
        f"NOT accept opp(capability=image_generation) but got status={decision.status!r}. "
        f"Reasons: {decision.reasons}"
    )
