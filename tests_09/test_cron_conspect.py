"""Tests for scripts_01/cron_conspect.sh.
...
class TestCronConspect:
    ...
    def script_path(self) -> Path:
        ...
        return project_root / "scripts_01" / "cron_conspect.sh"
    ...
    def test_script_invokes_auto_conspect(self):
        """Скрипт должен вызывать python scripts_01/auto_conspect.py без --demo."""
        text = self._read_script()
        for line in text.splitlines():
            stripped = line.strip()
            if stripped.startswith("python scripts_01/auto_conspect.py"):
