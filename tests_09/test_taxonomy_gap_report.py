...
KINDS ∈ {tool, module, role, engine***REMOVED***) AND Section A/B categorization
...
class TestCuratedGateway:
    def test_returns_18_capabilities_from_curated_default(self):
        ...
        parsed = _parse_llm_response(gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content)
        assert len(parsed) == 18, f"expected 18 capabilities, got {len(parsed)***REMOVED***"

    def test_explicit_count_is_8(self):
        gw = CuratedLlmGateway()
        parsed = _parse_llm_response(gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content)
        explicit = [item for item in parsed if item.get("explicit")***REMOVED***
        assert len(explicit) == 8

    def test_inferred_count_is_10(self):
        gw = CuratedLlmGateway()
        parsed = _parse_llm_response(gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content)
        inferred = [item for item in parsed if not item.get("explicit")***REMOVED***
        assert len(inferred) == 10

    def test_all_kinds_are_valid_closed_set(self):
        gw = CuratedLlmGateway()
        parsed = _parse_llm_response(gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content)
        valid_kinds = {"tool", "module", "role", "engine"***REMOVED***
        invalid = [item for item in parsed if item["kind"***REMOVED*** not in valid_kinds***REMOVED***
        assert not invalid, f"invalid kinds: {[(i['item_id'***REMOVED***, i['kind'***REMOVED***) for i in invalid***REMOVED******REMOVED***"
    ...
    def test_provenance_field_present_per_item(self):
        ...
        for item in raw:
            assert "_provenance" in item, (
                f"missing _provenance: {item['item_id'***REMOVED******REMOVED***"
            )

    def test_call_count_increments(self):
        gw = CuratedLlmGateway()
        gw.generate_by_capabilities(["plan"***REMOVED***, [{"role": "user", "content": "x"***REMOVED******REMOVED***)
        gw.generate_by_capabilities(["plan"***REMOVED***, [{"role": "user", "content": "y"***REMOVED******REMOVED***)
        assert gw.call_count == 2

    def test_last_messages_captured_for_inspection(self):
        gw = CuratedLlmGateway()
        msgs = [
            {"role": "system", "content": "You are ..."***REMOVED***,
            {"role": "user", "content": "extract capabilities"***REMOVED***,
        ***REMOVED***
        gw.generate_by_capabilities(["plan"***REMOVED***, msgs)
...
class TestBuildGapAnalysis:
    def test_sec_a_categorizes_in_taxonomy_not_keyword_matched(self):
        ...
        gw = CuratedLlmGateway()
        curated = gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content
        ...
        ***REMOVED***
        assert analysis["sec_a_in_taxonomy_not_matched"***REMOVED*** == sec_a_expected
    ...
    def test_sec_b_categorizes_not_in_taxonomy(self):
        ...
        ***REMOVED***
        assert analysis["sec_b_not_in_taxonomy"***REMOVED*** == sec_b_expected

    def test_inferred_gap_at_least_n(self):
        """Curated mock must yield ≥8 INFERRED (10 expected) — sanity check."""
        gw = CuratedLlmGateway()
        analysis = build_gap_analysis("", gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content)
        assert len(analysis["inferred_gaps"***REMOVED***) >= 8, (
            f"curated mock insufficient: only {len(analysis['inferred_gaps'***REMOVED***)***REMOVED*** inferred gaps"
        )
    ...
    def test_deterministic_8_on_vocal_fragment(self):
        ...
        vocal_text = (
            ...
            "claim source tracker [fact***REMOVED*** vs observation vs hypothesis; "
        ...
        analysis = build_gap_analysis(vocal_text, gw.generate_by_capabilities([***REMOVED***, [***REMOVED***).content)
        # 8 deterministic (matches v5.189.61 contract).
        assert len(analysis["deterministic_ids"***REMOVED***) == 8
...
class TestEndToEnd:
    def test_cli_on_real_vocal_project_produces_report(self, tmp_path):
        ...
        vocal_task = (
            ...
            "## 3. Claim source tracker [fact***REMOVED***\n"
        ...
        ***REMOVED***
        ...
        assert result.returncode == 0, f"CLI failed: {result.stderr***REMOVED***"
