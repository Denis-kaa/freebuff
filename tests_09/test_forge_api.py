...
2. /api/v1/projects/{slug} returns 404 for unknown slug.
3. CORS preflight OPTIONS returns Access-Control-Allow-Origin.
4. /api/v1/projects/{slug}/chain includes _mock:False/True correctly.
5. /static/{app.js,style.css,index.html} returns 200 + correct Content-Type.
...
@pytest.fixture(scope="module")
def registered_projects(client: TestClient) -> list[dict]:
    """Snapshot of /api/v1/projects entries — re-used across tests for stability.

    If registry is empty (rare), returns []. Per-project get tests then skip
...
class TestAllEndpoints:
    """8 GET endpoints under v5.181.0 surface (8 routes incl. /static/{path])."""
    ...
    def test_root_returns_200_with_platform_info(self, client: TestClient) -> None:
        ...
        for k in ("name", "version", "platform", "endpoints"):
            assert k in d, f"missing key {k!r} in root payload"
        assert d["name"] == "Freebuff Forge API"
        assert d["version"] == APP_VERSION
        assert d["pipeline_chain_source"] == "core_02.forge_facade.PIPELINE_CHAIN"
        assert d["pipeline_chain_role_count"] == 14
    ...
    def test_health_returns_200_with_liveness_fields(self, client: TestClient) -> None:
        ...
        assert isinstance(d["registry_present"], bool)
        assert isinstance(d["registry_violations"], int)
        assert d["registry_violations"] >= 0
    ...
    def test_api_v1_projects_returns_200_with_projects_list(
        self, client: TestClient, registered_projects: list[dict]
    ) -> None:
        r = client.get("/api/v1/projects")
        assert r.status_code == 200
        d = r.json()
        assert d["count"] == len(registered_projects)
        assert isinstance(d["projects"], list)
        assert isinstance(d["schema_violations"], list)
        ...
        for p in registered_projects:
            for k in ("project_id", "name", "root", "status", "last_run_at"):
                assert k in p, f"missing {k!r} in projects[{p.get('project_id')!r}]"
    ...
    def test_api_v1_projects_detail_returns_200_for_registered(
        ...
        slug = registered_projects[0]["project_id"]
        r = client.get(f"/api/v1/projects/{slug}")
        # Real registry OR synthetic mock fallback — both return 200
        assert r.status_code == 200
        d = r.json()
        assert "_mock" in d  # mock flag discipline (v5.181.0 contract)
        assert d["project_id"] in (slug, slug.replace("-", "_"))
        # Detail contract
    ...
    def test_api_v1_projects_chain_returns_200_with_chain_payload(
        ...
        slug = registered_projects[0]["project_id"]
        r = client.get(f"/api/v1/projects/{slug}/chain")
        assert r.status_code == 200
        d = r.json()
        ...
        for k in ("_mock", "project_id", "project_root", "stage_count",
            ...
            assert k in d, f"missing {k!r} in chain payload"
        assert d["stage_count"] == 14
        assert len(d["chain"]) == d["stage_count"]
        # Each chain stage has 5 fields
        for s in d["chain"]:
            for k in ("role_id", "mode", "status", "details", "duration_s"):
                assert k in s, f"missing {k!r} in chain[{s.get('role_id')!r}]"
...
PARTIAL_CHAIN_SLUGS = frozenset({"smoke"})
...
class TestProjectNotFound:
    ...
    def test_detail_returns_404_for_unknown_slug(self, client: TestClient) -> None:
        r = client.get(f"/api/v1/projects/{UNKNOWN_SLUG}")
        assert r.status_code == 404
        d = r.json()
        # FastAPI HTTPException detail shape
        assert "detail" in d
        assert UNKNOWN_SLUG in d["detail"]
    ...
    def test_detail_returns_404_for_unknown_slug_with_special_chars(self, client: TestClient) -> None:
        ...
        for slug in ("definitely-not-real-xyz", "also_not_real_xyz"):
            r = client.get(f"/api/v1/projects/{slug}")
            assert r.status_code == 404, f"expected 404 for {slug!r}, got {r.status_code}"
...
class TestCORSPreflight:
    """Starlette CORSMiddleware (allow_origins=['*']) should respond 200 + ACAO."""
...
class TestChainStageCount:
    ...
    def test_chain_includes_mock_flag_key(self, client: TestClient) -> None:
        ...
        r = client.get(f"/api/v1/projects/{UNKNOWN_SLUG}/chain")
        assert r.status_code == 200
        d = r.json()
    ...
    def test_chain_for_unknown_slug_returns_mock_true(
        ...
        if not d.get("_error"):
            assert d["_mock"] is True, "unknown slug should yield _mock=True"
    ...
    def test_chain_for_registered_project_has_canonical_14_stages(
        ...
        for p in registered_projects:
            ...
            assert d["stage_count"] == 14, f"{slug!r} chain length mismatch"
            assert len(d["chain"]) == 14
            # Project_id in payload matches slug canonical form
            assert d["project_id"] in (slug, slug.replace("-", "_"), slug.replace("_", "-"))
    ...
    def test_partial_chain_smoke_has_1_stage_lisa_only(
        ...
        records stage_count=1 + chain=[{role_id: 'lisa', mode: 'generate'}]).
        ...
        assert d["stage_count"] == 1, (
            f"'smoke' expected partial chain stage_count=1, got {d['stage_count']!r}"
        )
        assert len(d["chain"]) == 1
        assert d["chain"][0]["role_id"] == "lisa"
        assert d["chain"][0]["mode"] == "generate"
    ...
    def test_chain_includes_validation_registry_status(
        ...
        assert d["validation_registry_status"] in ("loaded", "missing", "unreadable")
...
class TestStaticMounts:
    ...
    def test_static_app_js_returns_200_javascript(self, client: TestClient) -> None:
        ...
        ), f"unexpected content-type for app.js: {ct!r}"
    ...
    def test_static_style_css_returns_200_css(self, client: TestClient) -> None:
        ...
        ), f"unexpected content-type for style.css: {ct!r}"
