"""
Tests for Plugin API — plugin_api.py + plugins_04/ directory.
...
class TestPluginLoader:
    def test_discover_plugins(self):
        """Проверяет, что PluginLoader находит плагины в plugins_04/"""
    ...
    def test_load_existing_plugin(self):
        registry = PluginRegistry()
        loader = PluginLoader(registry)
        plugin_path = WORKSPACE / "plugins_04" / "hello_world"
...
""")
    ...
    def test_create_plugin_registrar(self, event_bus):
        ...
        from scripts_01.tool_runtime import ToolRegistry

        tool_registry = ToolRegistry()
        loader, registry = create_plugin_registrar(
            event_bus=event_bus,
            tool_registry=tool_registry,
            plugins_dir=str(WORKSPACE / "plugins_04"),
        )
...
# ═══════════════════════════════════════════════════════════════
# Регрессия: `python -m scripts_01.plugin_api` (runpy __main__)
# ═══════════════════════════════════════════════════════════════


class TestPythonMRun:
    """Регрессия: CLI через `python -m scripts_01.plugin_api` грузит плагины.

    При запуске через `-m` модуль исполняется как `__main__` и до завершения
    НЕ зарегистрирован в sys.modules под каноническим именем. Плагины делают
    `from scripts_01.plugin_api import BasePlugin` — без фикса это порождало
    ВТОРУЮ копию модуля (и класса BasePlugin), из-за чего
    isinstance(plugin, BasePlugin) падал: "'plugin' is not a BasePlugin instance".
    Фикс: sys.modules.setdefault(...) в блоке __main__ регистрирует себя
    под каноническим именем до загрузки плагинов.
    """

    def test_python_m_loads_plugins(self):
        ...
        assert "not a BasePlugin instance" not in out
