...
def config_ws_dir(tmp_path: Path) -> Path:
    ...
    for proj in ("proj_a", "proj_b"):
        ...
        (pd / "project.yaml").write_text(
            yaml.dump({"name": f"Project {proj[-1***REMOVED***.upper()***REMOVED***", "type": "python"***REMOVED***),
            encoding="utf-8",
        )
...
def config_ws_with_projects(tmp_path: Path) -> Path:
    ...
    for proj in ("p1", "p2"):
        (root / proj).mkdir()
        (root / proj / "README.md").write_text(f"# {proj***REMOVED***")

    (root / "workspace.yaml").write_text(
        yaml.dump({
            "name": "Explicit WS",
            "projects": ["p1", "p2"***REMOVED***,
        ***REMOVED***),
        encoding="utf-8",
    )
...
class TestSyncReport:
    ...
    def test_defaults_are_empty(self) -> None:
        r = SyncReport()
        assert r.created_workspaces == [***REMOVED***
        assert r.created_projects == [***REMOVED***
        assert r.skipped_workspaces == [***REMOVED***
        assert r.skipped_projects == [***REMOVED***
        assert r.conflicts == [***REMOVED***
...
class TestSyncFromConfig:
    ...
    def test_initial_sync_creates_workspace_and_projects(
        ...
        assert len(wss) == 1
        assert wss[0***REMOVED***.slug == slug
        assert wss[0***REMOVED***.name == "Test Workspace"

        bound = [Path(p).name for p in wss[0***REMOVED***.project_paths***REMOVED***
        assert sorted(bound) == ["proj_a", "proj_b"***REMOVED***
    ...
    def test_sync_does_not_overwrite_owner_or_status(
        ...
        ws = tmp_registry.create_workspace(
            "Test Workspace",
            project_paths=[str(config_ws_dir / "proj_a")***REMOVED***,
            owner_chat_id=42,
        )
        ...
        assert updated[0***REMOVED***.owner_chat_id == 42
        assert updated[0***REMOVED***.status == "active"
    ...
    def test_privacy_conflict_when_path_in_other_workspace(
        ...
        conflict_path, current_slug, expected_slug = report.conflicts[0***REMOVED***
        ...
        bound = [Path(p).name for p in report.created_projects***REMOVED***
    ...
    def test_no_deletion_absent_project_in_yaml_keeps_db_row(
        ...
        cfg = yaml.safe_load((config_ws_dir / "workspace.yaml").read_text()) or {***REMOVED***
        cfg["projects"***REMOVED*** = ["proj_b"***REMOVED***  # proj_a excluded
        ...
        slug = _slugify_name("Test Workspace")
        bound = [Path(p).name for p in tmp_registry.list_projects(slug)***REMOVED***
    ...
    def test_sync_with_explicit_project_list(
        ...
        assert len(report.created_projects) == 2
        bound = [Path(p).name for p in report.created_projects***REMOVED***
        assert sorted(bound) == ["p1", "p2"***REMOVED***
