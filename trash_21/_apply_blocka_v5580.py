...
Replaces the parents[1***REMOVED*** sys.path block in BOTH canonical sibling-project
...
def replace_block(path: Path, old: str, new: str, label: str) -> bool:
    ...
    if not path.exists():
        print(f"  [{label***REMOVED******REMOVED*** ERROR: {path***REMOVED*** not found")
        return False
    text = path.read_text()
    if "from _freebuff_locator ***REMOVED***solve_freebuff_root" in text:
        print(f"  [{label***REMOVED******REMOVED*** already-applied (locator import present) — SKIP")
        return True
    if old not in text:
        print(f"  [{label***REMOVED******REMOVED*** ERROR: pattern not found in {path***REMOVED***")
        return False
    path.write_text(text.replace(old, new, 1))
    print(f"  [{label***REMOVED******REMOVED*** OK — {path***REMOVED***")
    return True
...
E2E_NEW = (
    ...
    "# Resolves Freebuff root so `core_02/` is importable WITHOUT PYTHONPATH env.\n"
    "# Resolution: $FREEBUFF_ROOT > canonical hardcode (`/storage/.../freebuff`).\n"
    "# Note: ROOT is now the Freebuff root (not `parents[1***REMOVED***`); downstream refs\n"
