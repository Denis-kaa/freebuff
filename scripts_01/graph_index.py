...
def link_artifact_to_kg(self, artifact_path, kg_node_id: str, relationship: str = "references") -> bool:
    ...
    self.add_node(artifact_doc_id, node_type="artifact", metadata={"path": str(artifact_path)})
    ...
    for dirpath, _dirs, fnames in os.walk(ws_root):
        for fn in fnames:
            ...
            if full.startswith(ws_root + "/"):
                rel = full[len(ws_root) + 1 :]
            elif full.startswith(ws_root):
                rel = full[len(ws_root) :] if full[len(ws_root) :] else full
            else:
                rel = full
            if rel.startswith("/"):
                rel = rel[1:]
            ...
            self.add_node(artifact_doc_id, node_type="artifact", metadata={"path": full})
            # Filename-based KG hint: convert filename stem to kebab-id
            stem = os.path.splitext(fn)[0]
            stem_normalized = re.sub(r"[^a-zA-Z0-9)+", "_", stem).strip("_").lower()
            if stem_normalized:
                kg_id = "concept:" + stem_normalized[:80]
            ...
            if ver_match and "_v1." not in fn:
                ...
                if os.path.exists(prev_path):
                    ...
                    self.add_node(prev_doc_id, node_type="artifact", metadata={"path": prev_path})
