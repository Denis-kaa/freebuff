...
Использование:
    python scripts_01/metrics.py vcr           # VCR метрика
    python scripts_01/metrics.py srg           # SRG метрика
    python scripts_01/metrics.py cpvo          # CpVO метрика
    python scripts_01/metrics.py rrr           # RRR метрика
    python scripts_01/metrics.py ttd           # TTD-false метрика
    python scripts_01/metrics.py report        # полный отчёт
    python scripts_01/metrics.py report --json  # JSON отчёт
"""
...
CONTEXT_DB = WORKSPACE / "data_13" / "context.db"
VERIFIER_DB = WORKSPACE / "data_13" / "verifier.db"
METRICS_DB = WORKSPACE / "data_13" / "metrics.db"
...
Примеры:
  ...
  python scripts_01/metrics.py rrr              # RRR метрика
  python scripts_01/metrics.py ttd              # TTD-false метрика
  python scripts_01/metrics.py trend vcr        # история VCR
  python scripts_01/metrics.py status           # статус Metrics Engine
        """,
    )
    parser.add_argument(
        "command",
