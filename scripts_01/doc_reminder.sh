...
#
# Использование:
#   bash scripts_01/doc_reminder.sh                  — проверить сейчас
#   bash scripts_01/doc_reminder.sh tmux             — проверить в tmux-попапе
#   bash scripts_01/doc_reminder.sh daemon           — запустить фоновую проверку (каждый час)
#   bash scripts_01/doc_reminder.sh view             — открыть TROUBLESHOOTING.md
# ============================================================
...
check_docs() {
    ...
    for f in TROUBLESHOOTING.md DECISIONS.md REFERENCES.md OVERLAY_IMPLEMENTATION.md; do
        if [ -f "$FREEBUFF/docs_10/$f" ***REMOVED***; then
            age=$((($(date +%s) - $(stat -c %Y "$FREEBUFF/docs_10/$f")) / 86400))
            echo "  ✅ docs_10/$f ($age дн.)"
        else
            echo "  ❌ docs_10/$f ОТСУТСТВУЕТ"
            issues=$((issues + 1))
        fi
    done
    ...
    if [ $issues -eq 0 ***REMOVED***; then
        ...
        if command -v termux-notification &>/dev/null; then
            termux-notification \
                ...
                --action "bash $FREEBUFF/scripts_01/doc_reminder.sh view"
        else
...
case "$CMD" in
    ...
    tmux)
        ...
        tmux display-popup -w 60% -h 50% -E "bash $FREEBUFF/scripts_01/doc_reminder.sh check; echo ''; echo 'Нажми Enter для закрытия...'; read"
        ;;
    daemon)
        ...
        tmux new-session -d -s "$SESS" \
            "while true; do
                ...
                cd $FREEBUFF
                bash scripts_01/doc_reminder.sh check
                sleep 3600
            done"
        ...
        ;;
    view)
        if [ -f "$FREEBUFF/docs_10/ops/TROUBLESHOOTING.md" ***REMOVED***; then
            if command -v less &>/dev/null; then
                less "$FREEBUFF/docs_10/ops/TROUBLESHOOTING.md"
            else
                cat "$FREEBUFF/docs_10/ops/TROUBLESHOOTING.md"
            fi
        else
