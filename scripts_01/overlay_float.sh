...
#
# Использование:
#   bash scripts_01/overlay_float.sh              # авто-выбор
#   bash scripts_01/overlay_float.sh float        # termux-float
#   bash scripts_01/overlay_float.sh tmux         # tmux
#   bash scripts_01/overlay_float.sh direct       # прямой запуск
#
# Горячие клавиши:
...
if [ "$MODE" = "float" ***REMOVED*** || [ "$MODE" = "auto" ***REMOVED***; then
    if command -v termux-float &>/dev/null; then
        ...
        termux-float python scripts_01/overlay_server.py
        exit $?
...
if [ "$MODE" = "tmux" ***REMOVED*** || [ "$MODE" = "auto" ***REMOVED***; then
    if command -v tmux &>/dev/null; then
        ...
        if tmux has-session 2>/dev/null; then
            tmux split-window -v -l 12 "python scripts_01/overlay_server.py"
        else
            tmux new-session -s freebuff-overlay "python scripts_01/overlay_server.py"
        fi
        exit $?
