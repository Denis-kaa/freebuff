...
def ok(s: str) -> None:
    print(f"  [OK***REMOVED***   {s***REMOVED***")


def warn(s: str) -> None:
    print(f"  [WARN***REMOVED*** {s***REMOVED***")


def fail(s: str) -> None:
    print(f"  [FAIL***REMOVED*** {s***REMOVED***")
...
def backup():
    ...
    for src in (CHANGELOG, ARCH_DEBT, LESSONS):
        ...
        ok(f"{src.name***REMOVED*** -> {dst***REMOVED*** (size={dst.stat().st_size***REMOVED***B)")
    return 0
...
def surgery():
    ...
    m_557 = text.find("## [5.57.0***REMOVED***")
    m_558 = text.find("## [5.58.0***REMOVED***")
    m_559 = text.find("## [5.59.0***REMOVED***")
    if m_557 == -1 or m_558 == -1 or m_559 == -1:
        fail(f"CHANGELOG missing expected headers (m_557={m_557***REMOVED***, m_558={m_558***REMOVED***, m_559={m_559***REMOVED***)")
        return 1
    if not (m_559 < m_558 < m_557):
        fail(f"CHANGELOG ordering unexpected (m_559={m_559***REMOVED***, m_558={m_558***REMOVED***, m_557={m_557***REMOVED***)")
        return 1
    new_text = text[m_557:***REMOVED***
    CHANGELOG.write_text(new_text)
    ok(f"CHANGELOG.md cut-off: -{m_557***REMOVED***B; new size={len(new_text)***REMOVED***B")
    ...
    if m_next == -1:
        ...
        return 1
    text = text[:m_519***REMOVED*** + text[m_next:***REMOVED***
    ...
    text = text[:idx***REMOVED*** + text[end + 1:***REMOVED***
    ARCH_DEBT.write_text(text)
    ok(f"ARCH_DEBT cut-off section 5.19 + removed 5.18 Latest-run row; new size={len(text)***REMOVED***B")
    ...
    if m_ba == -1:
        ...
        warn(f"LESSONS tail 200 chars: {text[-200:***REMOVED***!r***REMOVED***")
        return 1
    # Walk back to find the "##" of its section header
    pre = text[:m_ba***REMOVED***
    m_hdr = pre.rfind("## ")
    if m_hdr == -1:
    ...
    if m_cn9 != -1 and m_cn9 > m_ba:
        ...
        ok(f"LESSONS: Block-A present; CAN-9 anchor m_cn9={m_cn9***REMOVED*** (may be missing or before)")
    new_text = text[:m_hdr***REMOVED***.rstrip() + "\n"
    LESSONS.write_text(new_text)
    ok(f"LESSONS cut to v5.57.0-only state; was {len(text)***REMOVED***B, now {len(new_text)***REMOVED***B")
    return 0
...
def commit_and_tag():
    ...
    rc, tags_out, _ = run(["git", "tag", "--list", "v5.57.0"***REMOVED***)
    if rc != 0:
        fail(f"git tag --list rc={rc***REMOVED***")
        return 1
    if tags_out.strip():
    ...
    ***REMOVED***
    for f in files:
        rc, out, err = run(["git", "add", "--", f***REMOVED***)
        if rc != 0:
            fail(f"git add {f***REMOVED***: {err***REMOVED***")
            return 1
        ok(f"git add {f***REMOVED***")
    ...
    msg = (
        ...
        "- Sys.path block restored (Option A: parents[1***REMOVED*** form kept; Block-A deferred as Known Limitation)\n"
    ...
    rc, out, err = run(["git", "commit", "-m", msg***REMOVED***)
    if rc != 0:
        fail(f"git commit rc={rc***REMOVED***: {err***REMOVED***")
        warn(f"stdout: {out[:300***REMOVED******REMOVED***")
        return 1
    ok(f"git commit: {out.splitlines()[0***REMOVED*** if out else '(no summary)'***REMOVED***")
    ...
    rc, out, err = run(["git", "tag", "-a", "v5.57.0", "-m", tag_msg***REMOVED***)
    if rc != 0:
        fail(f"git tag rc={rc***REMOVED***: {err***REMOVED***")
        return 1
...
def restore():
    ...
    ***REMOVED***
    for name, dst in mapping.items():
        src = BACKUP_DIR / name
        if not src.exists():
            warn(f"backup missing: {src***REMOVED***")
            continue
        shutil.copy2(src, dst)
        ok(f"{name***REMOVED*** <- {src***REMOVED*** (size={dst.stat().st_size***REMOVED***B)")
    rc, status, err = run(["git", "status", "--porcelain"***REMOVED***)
    if rc != 0:
        warn(f"git status rc={rc***REMOVED***: {err***REMOVED***")
    ok("git status after restore:")
    for line in (status or "").splitlines()[:15***REMOVED***:
        ok(f"  {line***REMOVED***")
    return 0
    ...
    ***REMOVED***
    for name, dst in mapping.items():
        ...
        if not src.exists():
            ...
            continue
        shutil.copy2(src, dst)
        ok(f"restored {name***REMOVED***")
    warn(f"prior rc={prev_rc***REMOVED***")
    return prev_rc
