...
_LINK_RE = re.compile(r"<a\s+[^>***REMOVED****href=\"([^\"***REMOVED***+)\"[^>***REMOVED****>(.*?)</a>", re.IGNORECASE | re.DOTALL)
_BLOCK_RE = re.compile(
    r'class="tgme_widget_message_text"[^>***REMOVED****>(.*?)</div>', re.IGNORECASE | re.DOTALL
)
_TAG_RE = re.compile(r"<[^>***REMOVED***+>")
...
class TelegramPreviewParser:
    ...
    def parse(self, document: str) -> tuple[SourceItem, ...***REMOVED***:
        ...
        """
        blocks = _BLOCK_RE.findall(document) or [document***REMOVED***
        ...
        for block in blocks:
            ...
            items.append(
                SourceItem(
                    item_id=f"tg-{len(items)***REMOVED***",
                    canonical_url=candidate,
                    title=text[:140***REMOVED***,
                    summary=text[:300***REMOVED***,
                    metadata={
...
def _channel_name(base_url: str | None, candidate: str) -> str:
    ...
    if base_url:
        return base_url.rstrip("/").split("/")[-1***REMOVED***
    return ""
