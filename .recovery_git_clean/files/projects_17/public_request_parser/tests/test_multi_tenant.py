...
def test_schema_v2_initialized(storage: SqliteStorage) -> None:
    ...
    ***REMOVED***
    assert {"profiles", "feedback"***REMOVED*** <= tables
...
def test_profile_isolation_between_owners(storage: SqliteStorage) -> None:
    ...
    assert [p.profile_id for p in storage.list_profiles("alice")***REMOVED*** == ["profile-2"***REMOVED***
...
def test_feedback_record_and_stats(storage: SqliteStorage) -> None:
    ...
    assert storage.feedback_stats("operator") == {"relevant": 1***REMOVED***
    assert storage.feedback_stats("anonymous") == {"irrelevant": 1***REMOVED***
...
def test_v1_database_migrates_to_v2_preserving_data(tmp_path: Path) -> None:
    ...
    conn.executescript(
        ...
        CREATE TABLE publications (
            ...
            metadata_json TEXT NOT NULL DEFAULT '{***REMOVED***', status TEXT NOT NULL DEFAULT 'new',
