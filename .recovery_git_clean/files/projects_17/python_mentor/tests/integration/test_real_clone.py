...
def test_real_canary_ingest(tmp_path: Path, real_clone: Path) -> None:
    ...
    for group in ("concept", "practice"):
        ...
        subsets.append((group, sorted(p.name for p in d.iterdir() if p.is_dir())[:10***REMOVED***))
    ...
    conn = open_corpus(db)
    n = conn.execute("SELECT COUNT(*) FROM exercises").fetchone()[0***REMOVED***
    conn.close()
