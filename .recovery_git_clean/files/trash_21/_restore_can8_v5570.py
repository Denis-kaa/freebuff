...
edits but REMOVED the sys.path block (`parents[1***REMOVED***` form) without a replacement.
...
- Re-insert `parents[1***REMOVED***` sys.path block BEFORE the `# --- Workspace locator ---`
  comment, restoring pre-apply state for that block.
- OUT OF SCOPE: Block-A recovery (= replace `parents[1***REMOVED***` with
  `from _freebuff_locator ***REMOVED***solve_freebuff_root` import for hot-freebuff
...
RESTORED_MARK = "ROOT = Path(__file__).resolve().parents[1***REMOVED***\n"

# Block to insert before the canonical Workspace-locator anchor. Includes a
# lead-in comment explaining why we're keeping the `parents[1***REMOVED***` form (so future
# devs don't "fix" it without context).
PRE_INLINER_BLOCK = (
    "# --- Restore CON-17 sys.path block (Option A: keep parents[1***REMOVED*** form for CAN-8 closure) ---\n"
    "# NOTE: `parents[1***REMOVED***` (= parent of scripts/ = interior_planner/) does NOT contain\n"
    ...
    "# which is a SEPARATE debt; CAN-8 closure kept parents[1***REMOVED*** form per scope.\n"
...
def main() -> int:
    ...
    if ANCHOR not in txt:
        print(f"  ✗ {REG.name***REMOVED***: anchor '{ANCHOR.strip()***REMOVED***' not found — manual review")
        return 1
    if txt.count(ANCHOR) > 1:
        print(f"  ✗ {REG.name***REMOVED***: anchor appears {txt.count(ANCHOR)***REMOVED*** times — ambiguous")
        return 1

    # === Idempotent insertion ===
    if RESTORED_MARK in txt:
        print(f"  - {REG.name***REMOVED***: sys.path block already restored (idempotent no-op)")
    else:
        new_txt = txt.replace(ANCHOR, PRE_INLINER_BLOCK, 1)
        REG.write_text(new_txt)
        print(f"  ✓ {REG.name***REMOVED***: inserted 5-line sys.path block before Workspace locator")

    # === Drop helper (idempotent) ===
    print()
    if HELPER.exists():
        HELPER.unlink()
        print(f"  ✓ dropped helper: {HELPER.name***REMOVED***")
    else:
        print(f"  - helper already absent: {HELPER.name***REMOVED***")
