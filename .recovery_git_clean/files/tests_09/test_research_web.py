...
from scripts_01 ***REMOVED***search_web as rw
    ...
    def _stub_persist(url, source, *, title=None, metadata=None, root=None):
        ...
        return cp_mod.PersistResult(
            entry=cp_mod.CorpusEntry(
                ...
                title=title, metadata=dict(metadata or {***REMOVED***),
            ),
            is_duplicate=False,
        )
...
def test_research_web_no_save_dry_run(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        rw, "search_web",
        lambda query, max_sources=10, timeout=10.0: [_make_source("https://example.com/a", "Alpha")***REMOVED***,
    )
...
def test_json_schema_keys(monkeypatch) -> None:
    """DoD §5.2: JSON contains query, sources[***REMOVED***, synthesis, evidence_checked, degraded."""
    monkeypatch.setattr(
        rw, "search_web",
        lambda query, max_sources=10, timeout=10.0: [
    ...
    assert isinstance(payload["sources"***REMOVED***, list)
    assert payload["sources"***REMOVED***[0***REMOVED***["url"***REMOVED***.startswith("https://")
...
def test_cli_json_stdout(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(rw, "fetch_page", lambda url, timeout=10.0: "<p>c</p>")
    monkeypatch.setattr(sys, "argv", ["research_web", "Workspace OS", "--json", "--no-save"***REMOVED***)

    assert rw.main() == 0
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert payload["query"***REMOVED*** == "Workspace OS"
    assert payload["sources_checked"***REMOVED*** == 1
...
def test_fail_safe_broken_source(monkeypatch) -> None:
    ...
    verified_urls = [s.url for s in report.sources if s.verified***REMOVED***
...
def test_parse_ddg_results_extracts_sources() -> None:
    ...
    assert sources[0***REMOVED***.url == "https://example.com/doc"
    assert sources[0***REMOVED***.title == "Example Doc"
    assert sources[0***REMOVED***.snippet
...
class TestCorpusPersistenceIntegration:
    ...
    - source='research_web', title=src.title, metadata={'status': 200, 'query': ...***REMOVED***
    ...
    def test_persist_called_per_successful_fetch(self, monkeypatch, tmp_path):
        ...
        def _recorder(url, source, *, title=None, metadata=None, root=None):
            calls.append({
                ...
                "metadata": dict(metadata or {***REMOVED***), "root": root,
            ***REMOVED***)
        ...
        assert len(calls) == 2
        for c in calls:
            assert c["source"***REMOVED*** == "research_web"
            assert c["root"***REMOVED*** == tmp_path
            assert c["metadata"***REMOVED***["status"***REMOVED*** == 200
            assert c["metadata"***REMOVED***["query"***REMOVED*** == "test query"
        urls = {c["url"***REMOVED*** for c in calls***REMOVED***
        assert urls == {"https://alpha.example.com/", "https://beta.example.com/"***REMOVED***
        # Title contract: search-result title propagated to corpus (per code-reviewer v5.189.56).
        title_by_url = {c["url"***REMOVED***: c["title"***REMOVED*** for c in calls***REMOVED***
        assert title_by_url["https://alpha.example.com/"***REMOVED*** == "Alpha source"
        assert title_by_url["https://beta.example.com/"***REMOVED*** == "Beta source"
    ...
    def test_persist_not_called_on_fetch_failure(self, monkeypatch, tmp_path):
        ...
        def _recorder(url, source, *, title=None, metadata=None, root=None):
            calls.append({"url": url, "source": source, "root": root***REMOVED***)
            return real_persist(url, source, title=title, metadata=metadata, root=root)
        ...
        assert len(calls) == 1
        assert calls[0***REMOVED***["url"***REMOVED*** == "https://ok.example.com/"
    ...
    def test_persist_failure_does_not_break_research_web(self, monkeypatch, tmp_path):
        ...
        assert not report.degraded
        assert report.sources_checked == 2
        corpus_warnings = [w for w in report.warnings if "corpus_persistence error" in w***REMOVED***
    ...
    def test_persist_corpus_false_skips_persist(self, monkeypatch, tmp_path):
        ...
        def _recorder(*args, **kwargs):
            calls.append({"called": True***REMOVED***)
            return cp.PersistResult(
                entry=cp.CorpusEntry(
                    url=args[0***REMOVED***, source=args[1***REMOVED***, timestamp=cp._now_iso(),
                ),
                is_duplicate=False,
            )
    ...
    def test_corpus_dir_writes_real_entries(self, monkeypatch, tmp_path):
        ...
        urls = {e.url for e in entries***REMOVED***
        assert urls == {"https://real1.example.com/", "https://real2.example.com/"***REMOVED***
        for e in entries:
    ...
    def test_no_corpus_cli_flag_disables_persist(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            rw, "search_web",
            lambda q, max_sources=10, timeout=10.0: [_make_source("https://cli.example.com/", "CLI")***REMOVED***,
        )
        monkeypatch.setattr(rw, "fetch_page", lambda url, timeout=10.0: "<p>ok</p>")
        monkeypatch.setattr(
            sys, "argv",
            ["research_web", "test", "--no-save", "--no-corpus", "--json"***REMOVED***,
        )
    ...
    def test_persist_raises_unexpected_exception_still_completes(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            rw, "search_web",
            lambda q, **k: [_make_source("https://x.example.com/", "X")***REMOVED***,
        )
