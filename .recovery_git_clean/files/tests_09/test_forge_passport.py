...
@pytest.fixture
def minimal_manifest_dict() -> dict[str, Any***REMOVED***:
...
class TestForgePassportFromYaml:
    ...
    def test_happy_path_from_yaml_minimal(self, tmp_path: Path, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        ...
        assert pp.metadata == {"prompt_path": "prompts_11/_test.md"***REMOVED***
        assert pp.mission == minimal_manifest_dict["mission"***REMOVED***
    ...
    def test_happy_path_full_manifest(self, tmp_path: Path) -> None:
        ...
        data = {
            ...
            "inputs": ["architectural_problem", "architecture", "models"***REMOVED***,
            "production_workflow": ["problem_validation", "context_analysis", "verdict_generation"***REMOVED***,
            "engines": ["@entity blueprint.v3"***REMOVED***,
            "quality_gates": ["evidence_complete", "alternatives_considered"***REMOVED***,
            "outputs": ["review_verdict", "review_report"***REMOVED***,
            "artifacts": ["projects_17/<slug>/forge/review.yaml"***REMOVED***,
            "interfaces": ["receives: architecture, models", "produces: review_result"***REMOVED***,
            "memory": ["past_verdicts", "adr"***REMOVED***,
            "knowledge": ["patterns: arch_patterns_v1"***REMOVED***,
        ***REMOVED***
        ...
        assert pp.validate() == [***REMOVED***  # all token ⊆ KNOWN_CAPABILITIES
...
class TestForgePassportRequiredFields:
    ...
    def test_empty_forge_id_raises(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["forge_id"***REMOVED*** = ""
        with pytest.raises(ValueError, match="forge_id — обязательное поле"):
            ForgePassport._from_dict(minimal_manifest_dict, source="<test>")

    def test_missing_forge_id_raises(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        del minimal_manifest_dict["forge_id"***REMOVED***
        with pytest.raises(ValueError, match="forge_id — обязательное поле"):
            ForgePassport._from_dict(minimal_manifest_dict, source="<test>")

    def test_empty_factory_id_raises(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["factory_id"***REMOVED*** = ""
        with pytest.raises(ValueError, match="factory_id — обязательное поле"):
            ForgePassport._from_dict(minimal_manifest_dict, source="<test>")

    def test_invalid_status_raises(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["status"***REMOVED*** = "PRODUCTION"  # uppercase = invalid
        with pytest.raises(ValueError, match=r"должен быть ∈"):
            ForgePassport._from_dict(minimal_manifest_dict, source="<test>")

    def test_forge_id_not_lowercase_slug_raises(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["forge_id"***REMOVED*** = "Review"  # capital R fails
        with pytest.raises(ValueError, match=r"lowercase, начинается с буквы"):
            ForgePassport._from_dict(minimal_manifest_dict, source="<test>")

    def test_forge_id_starts_with_digit_raises(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["forge_id"***REMOVED*** = "1review"  # digit-prefix fails
        with pytest.raises(ValueError, match=r"lowercase, начинается с буквы"):
            ForgePassport._from_dict(minimal_manifest_dict, source="<test>")

    def test_factory_id_post_init_pattern_defense(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        """Last-resort guard in __post_init__ (after field assignment)."""
        minimal_manifest_dict["factory_id"***REMOVED*** = "BadFactoryId"
...
class TestForgePassportValidate:
    ...
    def test_valid_passport_returns_no_violations(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        assert pp.validate() == [***REMOVED***

    def test_empty_mission_violation(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["mission"***REMOVED*** = ""
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        violations = pp.validate()
        assert any("mission must be non-empty" in v for v in violations), violations

    def test_empty_outputs_violation(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["outputs"***REMOVED*** = [***REMOVED***
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        violations = pp.validate()
        assert any("outputs must be non-empty" in v for v in violations), violations

    def test_unknown_capability_violation(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        minimal_manifest_dict["capabilities"***REMOVED*** = ["explain", "qa"***REMOVED***  # "qa" not in KNOWN_CAPABILITIES
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        violations = pp.validate()
        assert any("unknown tokens" in v and "'qa'" in v for v in violations), violations

    def test_known_capability_only_passes(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        # All four are ⊆ KNOWN_CAPABILITIES.
        minimal_manifest_dict["capabilities"***REMOVED*** = ["explain", "review", "architecture", "report"***REMOVED***
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        assert pp.validate() == [***REMOVED***, (
            "All tokens should pass KNOWN_CAPABILITIES check"
        )

    def test_status_out_of_set_violation(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
...
class TestForgePassportRoundTrip:
    ...
    def test_round_trip_preserves_all_fields(self, tmp_path: Path) -> None:
        data = {
            ...
            "display_name": "Architecture Review Forge",
            "capabilities": ["review", "architecture", "explain"***REMOVED***,
            "metadata": {"prompt_path": "prompts_11/_test.md", "extra": 42***REMOVED***,
            "mission": "Проверить архитектурное решение",
            "inputs": ["in1", "in2"***REMOVED***,
            "production_workflow": ["step1", "step2"***REMOVED***,
            "engines": ["@entity blueprint.v3"***REMOVED***,
            "quality_gates": ["gate1"***REMOVED***,
            "outputs": ["out1", "out2"***REMOVED***,
            "artifacts": ["file1", "file2"***REMOVED***,
            "interfaces": ["i1", "i2"***REMOVED***,
            "memory": ["m1"***REMOVED***,
            "knowledge": ["k1"***REMOVED***,
        ***REMOVED***
...
class TestForgePassportSafetyHelpers:
    ...
    def test_as_tuple_accepts_list(self) -> None:
        assert _as_tuple(["a", "b"***REMOVED***, field_name="x") == ("a", "b")
    ...
    def test_as_tuple_dict_raises(self) -> None:
        with pytest.raises(ValueError, match="НЕ-scalar → НЕ тихая потеря"):
            _as_tuple({"key": "value"***REMOVED***, field_name="x")
...
class TestForgePassportToDict:
    ...
    def test_to_dict_has_lists_not_tuples(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        d = pp.to_dict()
        ...
        for field_name in ("capabilities", "inputs", "production_workflow", "engines",
                           ...
                           "memory", "knowledge"):
            assert isinstance(d[field_name***REMOVED***, list), field_name + " should be list in JSON"
        assert isinstance(d["forge_id"***REMOVED***, str)
        assert isinstance(d["metadata"***REMOVED***, dict)
...
class TestForgePassportContract:
    ...
    def test_required_fields_canonical_order(self) -> None:
        assert REQUIRED_FIELDS[:5***REMOVED*** == ("forge_id", "factory_id", "version", "status", "display_name")
        assert "mission" in REQUIRED_FIELDS
        assert "outputs" in REQUIRED_FIELDS

    def test_dataclass_is_frozen(self, minimal_manifest_dict: dict[str, Any***REMOVED***) -> None:
        from dataclasses import FrozenInstanceError
        pp = ForgePassport._from_dict(minimal_manifest_dict, source="<test>")
        with pytest.raises(FrozenInstanceError):
            pp.mission = "tampered"  # type: ignore[misc***REMOVED***
    ...
    def test_slug_regex_pattern(self) -> None:
        ...
        assert _SLUG_RE.pattern == r"^[a-z***REMOVED***[a-z0-9_***REMOVED***{1,30***REMOVED***$"
