...
def _dump_yaml(path: Path, data: dict[str, Any***REMOVED***) -> Path:
...
def _make_factory_dir(
    root: Path,
    factory_id: str,
    forge_ids: list[str***REMOVED***,
    *,
    factory_meta: dict[str, Any***REMOVED*** | None = None,
) -> Path:
    """Create factory_dir/<factory_id>/{factory.yaml, <forge>.yaml...***REMOVED***."""
    fdir = root / factory_id
    fdir.mkdir(parents=True, exist_ok=True)
    # factory.yaml metadata (defaults if not provided)
    meta = factory_meta or {
        "factory_id": factory_id,
        "display_name": f"{factory_id.capitalize()***REMOVED*** Factory",
        "version": "1.0.0",
        "status": "production",
        "description": f"{factory_id***REMOVED*** factory (test fixture).",
    ***REMOVED***
    ...
    for forge_id in forge_ids:
        data = {
            ...
            "version": "1.0.0",
            "status": "material",
            "display_name": f"{forge_id.capitalize()***REMOVED*** Forge",
            "capabilities": ["explain", "validate"***REMOVED***,
            "metadata": {"prompt_path": "prompts_11/_test.md"***REMOVED***,
            "mission": f"Test mission for {forge_id***REMOVED***",
            "outputs": [f"{forge_id***REMOVED***_verdict"***REMOVED***,
        ***REMOVED***
        _dump_yaml(fdir / f"{forge_id***REMOVED***.yaml", data)
    return fdir
...
class TestFactoryRegistryDiscovery:
    """Auto-discovery of manifests under factories_dir/<factory_id>/{factory.yaml, <forge>.yaml***REMOVED***."""

    def test_discover_single_factory_with_forges(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "research", ["scanner", "synthesizer"***REMOVED***)
        r = FactoryRegistry(tmp_path)

        assert r.list_factories() == ["research"***REMOVED***
        forges = r.list_forges("research")
        assert len(forges) == 2
        assert {f.forge_id for f in forges***REMOVED*** == {"scanner", "synthesizer"***REMOVED***

    def test_discover_multiple_factories(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "alpha", ["forge_a1"***REMOVED***)
        _make_factory_dir(tmp_path, "beta", ["forge_b1", "forge_b2"***REMOVED***)
        _make_factory_dir(tmp_path, "gamma", [***REMOVED***)
        r = FactoryRegistry(tmp_path)
        assert r.list_factories() == ["alpha", "beta", "gamma"***REMOVED***
        assert len(r.list_forges("beta")) == 2
        assert r.list_forges("gamma") == [***REMOVED***
    ...
    def test_factory_metadata_loaded(self, tmp_path: Path) -> None:
        ...
        r = FactoryRegistry(tmp_path)
        meta = r._factory_meta["lab"***REMOVED***
        assert meta["display_name"***REMOVED*** == "Lab Factory"
        assert meta["version"***REMOVED*** == "2.0.0"
        assert meta["description"***REMOVED*** == "Экспериментальная фабрика."

    def test_warnings_empty_on_clean_load(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "clean", ["happy_path"***REMOVED***)
        r = FactoryRegistry(tmp_path)
        assert r.warnings() == [***REMOVED***
        assert r.validate_all() == [***REMOVED***
...
class TestFactoryRegistryQuery:
    ...
    def test_list_factories_sorted(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "zeta", [***REMOVED***)
        _make_factory_dir(tmp_path, "alpha", [***REMOVED***)
        _make_factory_dir(tmp_path, "middle", [***REMOVED***)
        r = FactoryRegistry(tmp_path)
        assert r.list_factories() == ["alpha", "middle", "zeta"***REMOVED***
    ...
    def test_get_forge_missing_returns_none(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "research", ["scanner"***REMOVED***)
        r = FactoryRegistry(tmp_path)
    ...
    def test_find_by_capability_crosses_factories(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "alpha", ["forge_a1", "forge_a2"***REMOVED***)
        _make_factory_dir(tmp_path, "beta", ["forge_b1"***REMOVED***)
        # All default manifests use ["explain", "validate"***REMOVED***; both should match.
        r = FactoryRegistry(tmp_path)
    ...
    def test_find_by_capability_no_match_returns_empty(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "alpha", ["forge_a1"***REMOVED***)
        r = FactoryRegistry(tmp_path)
        assert r.find_by_capability("research") == [***REMOVED***  # not in default caps

    def test_all_forges_returns_sorted(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "alpha", ["z_forge", "a_forge"***REMOVED***)
        _make_factory_dir(tmp_path, "beta", ["middle"***REMOVED***)
        r = FactoryRegistry(tmp_path)
        all_f = r.all_forges()
        assert len(all_f) == 3
        # Sort key: (factory_id, forge_id)
        ids = [(f.factory_id, f.forge_id) for f in all_f***REMOVED***
        assert ids == sorted(ids), f"All-forges must be sorted: {ids***REMOVED***"
...
class TestFactoryRegistryFailSafe:
    ...
    def test_missing_factories_dir_warns(self, tmp_path: Path) -> None:
        ...
        assert r.list_factories() == [***REMOVED***
        assert r.list_forges("anything") == [***REMOVED***
        warnings = r.warnings()
    ...
    def test_factory_yaml_missing_warns(self, tmp_path: Path) -> None:
        ...
        r = FactoryRegistry(tmp_path)
        assert r.list_factories() == ["no_meta"***REMOVED***  # discovered but meta missing
        warnings = r.warnings()
    ...
    def test_corrupt_manifest_warns_and_skips(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "good", ["good_forge"***REMOVED***)
        ...
        _dump_yaml(
            tmp_path / "bad" / "bad.yaml",
            {
                ...
                "outputs": [***REMOVED***,  # пустой outputs → ValueError в _from_dict
            ***REMOVED***,
        )
        r = FactoryRegistry(tmp_path)
        ...
        assert r.list_forges("bad") == [***REMOVED***
    ...
    def test_validate_all_returns_aggregated_violations(self, tmp_path: Path) -> None:
        ...
        _dump_yaml(
            valid_dir / "good.yaml",
            {
                ...
                "capabilities": ["explain"***REMOVED***, "metadata": {***REMOVED***, "mission": "m",
                "outputs": ["v"***REMOVED***,
            ***REMOVED***,
        )
        ...
        _dump_yaml(
            invalid_dir / "bad.yaml",
            {
                ...
                "capabilities": ["unknown_token"***REMOVED***,  # ← vocab violation
                "metadata": {***REMOVED***, "mission": "m",
                "outputs": ["v"***REMOVED***,
            ***REMOVED***,
        )
        ...
        assert any("invalid_factory/bad" in v and "unknown_token" in v or "unknown tokens" in v for v in violations), (
            f"Expected vocab violation; got: {violations***REMOVED***"
        )
        assert not any("valid_factory/good" in v for v in violations), (
            f"Valid passport should not appear in violations; got: {violations***REMOVED***"
        )
...
class TestFactoryRegistryEnvAndReload:
    ...
    def test_from_env_no_var(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        _make_factory_dir(tmp_path, "no_env_factory", ["forge"***REMOVED***)
    ...
    def test_from_env_with_var(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        _make_factory_dir(env_dir, "env_factory", ["forge"***REMOVED***)
    ...
    def test_reload_rediscovers(self, tmp_path: Path) -> None:
        _make_factory_dir(tmp_path, "before_reload", ["forge_v1"***REMOVED***)
        r = FactoryRegistry(tmp_path)
        assert "before_reload" in r.list_factories()
        # Add new factory AFTER construction.
        _make_factory_dir(tmp_path, "after_reload", ["forge_v2"***REMOVED***)
        ...
        assert len(forges) == 1
        assert forges[0***REMOVED***.forge_id == "forge_v2"
...
class TestFactoryRegistryCapabilityCatalog:
    ...
    def _make_cap_factory(
        ...
        factory_id: str,
        status: str,
        factory_caps: list[str***REMOVED***,
        forge_ids: list[str***REMOVED***,
        forge_caps: list[str***REMOVED***,
    ) -> None:
        fdir = root / factory_id
        fdir.mkdir(parents=True, exist_ok=True)
        _dump_yaml(fdir / "factory.yaml", {
            "factory_id": factory_id,
            "display_name": f"{factory_id***REMOVED*** Factory",
            "version": "1.0.0",
            "status": status,
            "description": f"{factory_id***REMOVED*** factory.",
            "capabilities": factory_caps,
        ***REMOVED***)
        for fid in forge_ids:
            _dump_yaml(fdir / f"{fid***REMOVED***.yaml", {
                "forge_id": fid,
                "factory_id": factory_id,
                "version": "1.0.0",
                "status": "material",
                "display_name": f"{fid***REMOVED*** Forge",
                "capabilities": forge_caps,
                "metadata": {***REMOVED***,
                "mission": f"mission {fid***REMOVED***",
                "outputs": [f"{fid***REMOVED***_verdict"***REMOVED***,
            ***REMOVED***)

    def test_get_factory_returns_passport(self, tmp_path: Path) -> None:
        self._make_cap_factory(
            tmp_path, "arch", "production",
            ["architecture", "review"***REMOVED***, ["review"***REMOVED***, ["review", "explain"***REMOVED***,
        )
        r = FactoryRegistry(tmp_path)
    ...
    def test_factory_capabilities_union(self, tmp_path: Path) -> None:
        ...
        self._make_cap_factory(
            tmp_path, "arch", "production",
            ["architecture"***REMOVED***, ["review", "governance"***REMOVED***, ["explain", "validate"***REMOVED***,
        )
        r = FactoryRegistry(tmp_path)
    ...
    def test_find_factories_by_capability(self, tmp_path: Path) -> None:
        self._make_cap_factory(tmp_path, "alpha", "production", ["architecture"***REMOVED***, [***REMOVED***, [***REMOVED***)
        self._make_cap_factory(tmp_path, "beta", "production", ["research"***REMOVED***, [***REMOVED***, [***REMOVED***)
        r = FactoryRegistry(tmp_path)
        matches = r.find_factories_by_capability("architecture")
        assert [fp.factory_id for fp in matches***REMOVED*** == ["alpha"***REMOVED***
        assert r.find_factories_by_capability("nope") == [***REMOVED***
    ...
    def test_find_factories_by_capability_uses_union(self, tmp_path: Path) -> None:
        ...
        self._make_cap_factory(
            tmp_path, "arch", "production",
            ["architecture"***REMOVED***, ["review"***REMOVED***, ["validate"***REMOVED***,
        )
        r = FactoryRegistry(tmp_path)
        matches = r.find_factories_by_capability("validate")
        assert [fp.factory_id for fp in matches***REMOVED*** == ["arch"***REMOVED***

    def test_select_forge_status_priority(self, tmp_path: Path) -> None:
        # production factory beats design factory for same capability.
        self._make_cap_factory(tmp_path, "prod_factory", "production", ["review"***REMOVED***, ["review"***REMOVED***, ["review"***REMOVED***)
        self._make_cap_factory(tmp_path, "design_factory", "design", ["review"***REMOVED***, ["review"***REMOVED***, ["review"***REMOVED***)
        r = FactoryRegistry(tmp_path)
    ...
    def test_select_forge_no_match_returns_none(self, tmp_path: Path) -> None:
        self._make_cap_factory(tmp_path, "arch", "production", ["architecture"***REMOVED***, ["review"***REMOVED***, ["review"***REMOVED***)
        r = FactoryRegistry(tmp_path)
        assert r.select_forge("research") is None

    def test_capability_catalog(self, tmp_path: Path) -> None:
        self._make_cap_factory(tmp_path, "alpha", "production", ["architecture", "review"***REMOVED***, [***REMOVED***, [***REMOVED***)
        self._make_cap_factory(tmp_path, "beta", "production", ["review"***REMOVED***, [***REMOVED***, [***REMOVED***)
        r = FactoryRegistry(tmp_path)
        catalog = r.capability_catalog()
        assert catalog["architecture"***REMOVED*** == ["alpha"***REMOVED***
        assert catalog["review"***REMOVED*** == ["alpha", "beta"***REMOVED***
