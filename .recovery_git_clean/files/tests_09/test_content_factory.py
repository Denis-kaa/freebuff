...
class _FakeScenario:
    def __init__(self, scenario_id: str, capabilities):
        ...
        # via _candidate_capability(scenario, role) → scenario.capabilities[0***REMOVED***).
...
def _make_opp(opp_id="opp-test1", project_id="proj-test", capability="article_generation",
    ...
    return type("Opp", (), {
        ...
        "status": status,
        "priority": 5,
        "provenance": {"source": "whim", "source_id": "whim-1", "capability": capability***REMOVED***,
        "scenario": {"capability": capability***REMOVED***,
        "source_path": "data_13/whims.yaml",
        "evidence_path": "",
        "related_whims": ["whim-1"***REMOVED***,
    ***REMOVED***)()
...
def test_2_resolve_content_capability_via_fake_registry():
    ...
    ***REMOVED***))
    ...
    assert pair[0***REMOVED***.factory_id == "content"
    assert pair[1***REMOVED***.forge_id == "writing"
...
def test_5_normalize_input_fields():
    ...
    assert inp["title"***REMOVED*** == "Создать статью по Workspace OS"
    assert inp["source"***REMOVED*** == "whim"
    assert inp["provenance"***REMOVED***["capability"***REMOVED*** == "article_generation"
    assert inp["related_whims"***REMOVED*** == ["whim-1"***REMOVED***
...
def test_6_build_execution_request():
    ...
    ***REMOVED***))
    ...
    assert req.inputs["title"***REMOVED***.startswith("Создать")
...
def test_7_build_request_missing_factory_returns_none():
    cf = ContentFactory(factory_registry=_FakeFactoryRegistry({***REMOVED***))
    opp = _make_opp()
...
def test_9_execute_full_slice_artifact_and_feedback():
    ...
    cf._resolve_project = lambda opp, project_root=None: type(
        "Proj", (), {"root": Path("/tmp/fake_project")***REMOVED***)()
    result = cf.execute(opp)
    assert result["ok"***REMOVED*** is True
    art = result["artifact"***REMOVED***
    assert art["kind"***REMOVED*** == "content_artifact"
    assert art["factory_id"***REMOVED*** == "content"
    assert art["forge_id"***REMOVED*** == "writing"
    assert art["overall"***REMOVED*** == "ok"
    assert facade.calls and facade.calls[0***REMOVED***["role_ids"***REMOVED*** == CONTENT_ROLE_IDS
    assert memory.kos and memory.kos[0***REMOVED***["kind"***REMOVED*** == "candidate"
    assert "content_factory" in memory.kos[0***REMOVED***["tags"***REMOVED***
...
def test_11_execute_no_capability_returns_error():
    ...
    opp.provenance = {"source": "whim"***REMOVED***
    opp.scenario = None
    result = cf.execute(opp)
...
def test_13_domain_isolation_si_agnostic(tmp_path):
    ...
    ***REMOVED***)
    ...
    assert factory_registry.select_forge("article_generation")[1***REMOVED***.forge_id == "writing"
    assert factory_registry.select_forge("code")[0***REMOVED***.factory_id == "test"
    assert factory_registry.select_forge("code")[1***REMOVED***.forge_id == "code_forge"
...
def test_13b_si_routes_code_opp_to_test_factory(tmp_path):
    ...
    ***REMOVED***)
    scenarios = [
        (_FakeScenario("scenario_content", ["article_generation"***REMOVED***), _FakeRole("writer"), 0.9),
        (_FakeScenario("scenario_code", ["code"***REMOVED***), _FakeRole("developer"), 0.6),  # ниже скора content
    ***REMOVED***
...
def test_14_real_factory_registry_resolves_content_manifests():
    ...
    for cap in CONTENT_CAPABILITIES:
        assert reg.select_forge(cap) is not None, f"{cap***REMOVED*** не разрешается"
...
def test_15_per_instance_warnings_no_cross_pollution():
    ...
    try:
        ...
        assert inst1._import_warnings[0***REMOVED***.startswith("factory_registry:"), (
            f"warning text mismatch: {inst1._import_warnings!r***REMOVED***"
        )
        ...
        assert inst2._import_warnings == ["factory_registry: unavailable"***REMOVED***, (
            f"inst2 should have 1 warning after forced failure: {inst2._import_warnings!r***REMOVED***"
        )
        assert inst1._import_warnings == inst1_warnings_snapshot, (
            f"inst1 warnings DRIFTED after inst2 lazy load — cross-pollution: "
            f"before={inst1_warnings_snapshot!r***REMOVED*** after={inst1._import_warnings!r***REMOVED***"
        )
        ...
        assert depr_singleton_after == depr_singleton_before, (
            ...
            f"per-instance lazy methods (G-13.1). before={depr_singleton_before!r***REMOVED*** "
            f"after={depr_singleton_after!r***REMOVED***"
        )
    finally:
