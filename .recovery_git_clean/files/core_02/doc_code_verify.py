...
ANCHOR_NAMESPACES: tuple[str, ...***REMOVED*** = (
...
_ENTITY_HEADER_RE = re.compile(
    r"^###\s+@entity[:\s***REMOVED***+(.+)$"
)

# Lifecycle state machine (Anti-5 minimum useful set).
CLASSIFICATIONS: tuple[str, ...***REMOVED*** = (
...
def extract_claims(doc_path: Path) -> list[Claim***REMOVED***:
    ...
    - Empty / missing doc → [***REMOVED*** (no exception).
    - ReadError (encoding) → [***REMOVED***.
    """
    ...
    claims: list[Claim***REMOVED*** = [***REMOVED***
    in_fence = False
...
def load_code_map(workspace: Path) -> dict[str, dict[str, Any***REMOVED******REMOVED***:
    ...
    Returns: dict[entity_id***REMOVED*** -> {type, file, symbol***REMOVED***

    Edge cases:
    - File missing → {***REMOVED*** (silently — caller handles NONE-of-map case via DOC_ONLY).
    - ReadError → {***REMOVED***.
    ...
    result: dict[str, dict[str, Any***REMOVED******REMOVED*** = {***REMOVED***
    current: Optional[str***REMOVED*** = None
    ...
    for line in text.splitlines():
        ...
        if s.startswith("### @entity"):
            ...
            if m:
                ...
                if candidate and candidate not in result:
                    result[candidate***REMOVED*** = {"type": "", "file": "", "symbol": ""***REMOVED***
                    current = candidate
                else:
        ...
        if s.startswith("- **type:**"):
            result[current***REMOVED***["type"***REMOVED*** = _strip_md_marker(s.split(":", 1)[1***REMOVED***)
        elif s.startswith("- **file:**"):
            result[current***REMOVED***["file"***REMOVED*** = _extract_first_backtick(s.split(":", 1)[1***REMOVED***)
        elif s.startswith("- **symbol:**"):
            val = _extract_first_backtick(s.split(":", 1)[1***REMOVED***)
            if val and not result[current***REMOVED***["symbol"***REMOVED***:
                result[current***REMOVED***["symbol"***REMOVED*** = val
...
def verify_claim(
    claim: Claim,
    code_map: dict[str, dict[str, Any***REMOVED******REMOVED***,
    workspace: Path,
) -> VerificationResult:
    ...
    mapped = code_map[claim.target***REMOVED***
    ...
    if check_symbol_exists(workspace, file_rel, symbol):
        return VerificationResult(
            ...
            mapped_file=file_rel,
            mapped_symbol=symbol,
            evidence=f"{file_rel***REMOVED***::{symbol***REMOVED***",
        )
    ...
    return VerificationResult(
        ...
        mapped_file=file_rel,
        mapped_symbol=symbol,
        evidence=f"{file_rel***REMOVED***::{symbol***REMOVED*** not found via AST",
    )
...
def run_verification(
    ...
    *,
    ...
    docs_checked = 0
    findings: list[dict[str, Any***REMOVED******REMOVED*** = [***REMOVED***
    by_classification: dict[str, int***REMOVED*** = {c: 0 for c in CLASSIFICATIONS***REMOVED***

    code_map = load_code_map(workspace)

    if target_path.is_file():
        docs = [target_path***REMOVED***
    elif target_path.is_dir():
        docs = sorted(target_path.rglob("*.md"))
    else:
        return {
            "error": f"not found: {target_path***REMOVED***",
            "docs_checked": 0,
            "total_claims": 0,
    ...
    for doc in docs:
        ...
        for claim in extract_claims(doc):
            ...
            by_classification[result.classification***REMOVED*** += 1
            try:
    ...
    strict_exit_code = 1 if (
        strict
        and (by_classification["STALE"***REMOVED*** > 0 or by_classification["DOC_ONLY"***REMOVED*** > 0)
    ) else 0
...
def main(argv: Optional[list[str***REMOVED******REMOVED*** = None) -> int:
    ...
    if args.json:
        ...
        bc = summary["by_classification"***REMOVED***
        print(f"Docs checked  : {summary['docs_checked'***REMOVED******REMOVED***")
        print(f"Total claims  : {summary['total_claims'***REMOVED******REMOVED***")
        for c in CLASSIFICATIONS:
            print(f"  {c:<11***REMOVED***  : {bc[c***REMOVED******REMOVED***")
        if summary["findings"***REMOVED***:
            print("\nFindings:")
            for f in summary["findings"***REMOVED***:
                marker = {
                    "CONFIRMED": "[OK***REMOVED***",
                    "STALE":     "[!!***REMOVED***",
                    "DOC_ONLY":  "[??***REMOVED***",
                    "UNKNOWN":   "[??***REMOVED***",
                ***REMOVED***.get(f["classification"***REMOVED***, "[??***REMOVED***")
                line = (
                    f"  {marker***REMOVED*** {f['classification'***REMOVED***:<9***REMOVED*** "
                    f"{f['doc'***REMOVED******REMOVED***:{f['line'***REMOVED******REMOVED*** "
                    f"{f['namespace'***REMOVED******REMOVED*** {f['target'***REMOVED******REMOVED***"
                )
                print(line)
                if f["evidence"***REMOVED***:
                    print(f"           -> {f['evidence'***REMOVED******REMOVED***")

    return int(summary["strict_exit_code"***REMOVED***)
