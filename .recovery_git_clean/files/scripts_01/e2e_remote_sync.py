...
`TGClient.get_messages(chat_id, ids=[msg_id***REMOVED***)` returns non-empty, non-synthetic
...
**CAN-9 / CAN-16 discipline:**
  - Round-trip is verified through `TGClient.get_messages(chat_id, ids=[msg_id***REMOVED***)`
...
if not logger.handlers:
    ...
    _h.setFormatter(
        logging.Formatter("[e2e_remote_sync***REMOVED*** %(asctime)s %(levelname)s %(message)s")
    )
...
def _round_id() -> str:
    ...
    return f"phase_5_3_c_{uuid.uuid4().hex[:8***REMOVED******REMOVED***"


def _truncate(s: Optional[str***REMOVED***, n: int = 80) -> str:
    if s is None:
        return "—"
    return s if len(s) <= n else s[: n - 1***REMOVED*** + "…"


def _table_escape(s: Optional[str***REMOVED***, n: int = 60) -> str:
    ...
    return escaped if len(escaped) <= n else escaped[: n - 1***REMOVED*** + "…"
...
async def stage0_preflight(
    *, skip_tg: bool = False, log_dir: Optional[Path***REMOVED*** = None
    ...
    preflight: Dict[str, Any***REMOVED*** = {
        "tg_session": None,
    ...
    ***REMOVED***
    ...
    try:
        ...
        preflight["core_02_import"***REMOVED*** = bool(_cls is RemoteSyncCoordinatorImpl)
    except Exception as e:
        preflight["core_02_import_error"***REMOVED*** = str(e)

    # log-dir writable check
    try:
        log_dir.mkdir(parents=True, exist_ok=True)
        _tmp = log_dir / f".touch_{uuid.uuid4().hex[:6***REMOVED******REMOVED***"
        _tmp.touch()
        _tmp.unlink()
        preflight["log_dir_writable"***REMOVED*** = True
    except Exception as e:
        preflight["log_dir_writable_error"***REMOVED*** = str(e)
    ...
    try:
        ...
        if not connected:
            preflight["tg_session"***REMOVED*** = False
        else:
            try:
                ...
                preflight["tg_session"***REMOVED*** = tg_user_id or True
            finally:
                try:
        ...
        preflight["tg_session_error"***REMOVED*** = str(e)
...
async def stage2_push_channels(
    ...
    ***REMOVED***
    ...
    if dry_run:
        push["saved_msgs"***REMOVED***["ok"***REMOVED*** = True
        push["saved_msgs"***REMOVED***["msg_id"***REMOVED*** = "DRY_RUN"
        if sync_group_active:
            push["litvinov"***REMOVED*** = {
    ...
    try:
        ...
        push["saved_msgs"***REMOVED***["msg_id"***REMOVED*** = (saved_res.get("msg_ids") or [None***REMOVED***)[0***REMOVED***
        push["saved_msgs"***REMOVED***["ok"***REMOVED*** = bool(saved_res.get("ok"))
        push["saved_msgs"***REMOVED***["correlation_id"***REMOVED*** = saved_res.get("correlation_id")
        push["saved_msgs"***REMOVED***["chunk_count"***REMOVED*** = saved_res.get("chunk_count")
        if not saved_res.get("ok"):
            push["saved_msgs"***REMOVED***["error"***REMOVED*** = saved_res.get("error")
        await coord_saved.shutdown()
    except Exception as e:  # defensive
        push["saved_msgs"***REMOVED***["error"***REMOVED*** = f"Saved push coordinator exception: {e***REMOVED***"
    ...
    if sync_group_active:
        try:
            ...
            push["litvinov"***REMOVED*** = {
                "chat_id": tc.ALEX_LITVINOV_CHAT_ID,
                "msg_id": (lit_res.get("msg_ids") or [None***REMOVED***)[0***REMOVED***,
            ...
            ***REMOVED***
            ...
            push["litvinov"***REMOVED*** = {
                ...
                "msg_id": None,
                "ok": False,
                "error": f"Литвинов push coordinator exception: {e***REMOVED***",
            ***REMOVED***
...
async def stage3_round_trip(
    push_results: Dict[str, Any***REMOVED***, *, dry_run: bool = False
) -> Dict[str, Any***REMOVED***:
    """Verify each push via TGClient.get_messages(chat_id, ids=[msg_id***REMOVED***).
    ...
    try:
        ...
        return {
            "connected": False,
            "error": f"TGClient bootstrap failed: {e***REMOVED***",
            "saved_msg_text_non_empty": False,
            "lit_msg_text_non_empty": False,
        ***REMOVED***

    out: Dict[str, Any***REMOVED*** = {"connected": True***REMOVED***
    try:
        ...
        saved_msg_id = push_results["saved_msgs"***REMOVED***["msg_id"***REMOVED***
        if saved_msg_id is not None and saved_msg_id != "DRY_RUN":
            try:
                ...
                out["saved_msg_id"***REMOVED*** = saved_msg_id
                out["saved_msg_text_non_empty"***REMOVED*** = ok
                if ok:
                    out["saved_msg_text_head"***REMOVED*** = match.text[:100***REMOVED***
                ...
                out["saved_msg_id"***REMOVED*** = saved_msg_id
                out["saved_msg_text_non_empty"***REMOVED*** = None

        # Литвинов round-trip (same limit-scan pattern)
        if (
            push_results.get("litvinov") is not None
            and push_results["litvinov"***REMOVED***.get("msg_id") is not None
            and push_results["litvinov"***REMOVED***["msg_id"***REMOVED*** != "DRY_RUN"
        ):
            lit_msg_id = push_results["litvinov"***REMOVED***["msg_id"***REMOVED***
            try:
                recent = await client.get_messages(
                ...
                out["lit_msg_id"***REMOVED*** = lit_msg_id
                out["lit_msg_text_non_empty"***REMOVED*** = ok
                if ok:
                    out["lit_msg_text_head"***REMOVED*** = match.text[:100***REMOVED***
            except Exception as e:
                logger.warning(
                    "stage3 Литвинов round-trip exception: %s", e,
                )
                out["lit_msg_id"***REMOVED*** = lit_msg_id
                out["lit_msg_text_non_empty"***REMOVED*** = None
    finally:
        try:
            await client.disconnect()
        except Exception:
...
async def run_e2e_pipeline(
    *,
    ...
    skip_tg: bool = False,
    dry_run: bool = False,
    run_tag: Optional[str***REMOVED*** = None,
) -> Dict[str, Any***REMOVED***:
    ...
    if dry_run:
        ...
        summary = (
            f"Saved={push['saved_msgs'***REMOVED***.get('msg_id')***REMOVED*** verified={saved_ok***REMOVED***; "
            + (
                f"Литвинов={push['litvinov'***REMOVED***.get('msg_id')***REMOVED*** verified={lit_ok***REMOVED***"
                if not lit_excluded
...
def write_e2e_log(results: Dict[str, Any***REMOVED***, log_path: Path) -> Dict[str, Any***REMOVED***:
    ...
    lines.append(f"# Phase 5.3-C Remote Sync — End-to-End Run @ {results['started_iso'***REMOVED******REMOVED***")
    lines.append("")
    lines.append(f"- **Run tag**: {results.get('run_tag') or '(default — UTC timestamp)'***REMOVED***")
    lines.append(f"- **Round ID**: `{results.get('round_id')***REMOVED***`")
    lines.append(f"- **Sync Group active**: {results.get('sync_group_active', False)***REMOVED***")
    lines.append(f"- **Dry run**: {results.get('dry_run', False)***REMOVED***")
    lines.append(f"- **Skipped**: {results.get('skipped', False)***REMOVED***")
    status = (
        "✅ PASS"
        if (results.get("ok") or results.get("skipped"))
        else "❌ FAIL"
    )
    lines.append(f"- **Status**: {status***REMOVED***")
    lines.append("")

    # Stage 0 — pre-flight
    pf = results.get("stage0_preflight") or {***REMOVED***
    ...
    lines.append(
        f"| TG session alive | "
        f"{'✅' if tg_session_ok else ('⚠️ skipped' if pf.get('skip_tg_flag') else '❌')***REMOVED*** "
        f"| `{tg_session or 'skipped'***REMOVED***`"
        + (f" — error: `{pf.get('tg_session_error')***REMOVED***`" if pf.get("tg_session_error") else "")
        + " |"
    )
    lines.append(
        f"| core_02.remote_sync importable | "
        f"{'✅' if pf.get('core_02_import') else '❌'***REMOVED*** "
        f"| `RemoteSyncCoordinatorImpl`"
        + (f" — error: `{pf.get('core_02_import_error')***REMOVED***`" if pf.get("core_02_import_error") else "")
        + " |"
    )
    lines.append(
        f"| log-dir writable | "
        f"{'✅' if pf.get('log_dir_writable') else '❌'***REMOVED*** "
        f"| `{DEFAULT_E2E_LOG_DIR***REMOVED***`"
        + (f" — error: `{pf.get('log_dir_writable_error')***REMOVED***`" if pf.get("log_dir_writable_error") else "")
        + " |"
    )
    lines.append("")
    ...
    if results.get("skipped"):
        ...
        lines.append("")
        lines.append(f"- Run {results.get('summary')***REMOVED***.")
        lines.append(f"- Exit code: `{results.get('exit_code')***REMOVED***`")
        lines.append("")
        log_path.write_text("\n".join(lines))
        return {"log_path": str(log_path), "line_count": len(lines)***REMOVED***

    # Stage 1 — planning
    sd = results.get("stage1_delta_summary") or {***REMOVED***
    if sd:
        lines.append("## Stage 1 — Planning (SyncDelta construction)")
        lines.append("")
        lines.append(f"- timestamp_ms: `{sd.get('timestamp_ms')***REMOVED***`")
        lines.append(f"- source_device_id: `{sd.get('source_device_id')***REMOVED***`")
        lines.append(f"- revision: `{sd.get('revision')***REMOVED***`")
        lines.append(
            f"- updated_keys: `{json.dumps(sd.get('updated_keys') or {***REMOVED***, sort_keys=True)***REMOVED***`"
        )
        lines.append("")

    # Stage 2 — push
    sp = results.get("stage2_push") or {***REMOVED***
    saved_data = sp.get("saved_msgs") or {***REMOVED***
    ...
    lines.append(
        f"| Saved Messages | `{saved_data.get('chat_id')***REMOVED***` | "
        f"`{saved_data.get('msg_id') or '—'***REMOVED***` | "
        f"`{saved_data.get('ok')***REMOVED***` | "
        f"`{saved_data.get('chunk_count') or '—'***REMOVED***` | "
        f"`{(saved_data.get('correlation_id') or '—')[:30***REMOVED******REMOVED***` | "
        f"`{saved_error***REMOVED***` |"
    )
    if lit_data:
        lit_error = _table_escape(lit_data.get("error"))
        lines.append(
            f"| Литвинов | `{lit_data.get('chat_id')***REMOVED***` | "
            f"`{lit_data.get('msg_id') or '—'***REMOVED***` | "
            f"`{lit_data.get('ok')***REMOVED***` | "
            f"`{lit_data.get('chunk_count') or '—'***REMOVED***` | "
            f"`{(lit_data.get('correlation_id') or '—')[:30***REMOVED******REMOVED***` | "
            f"`{lit_error***REMOVED***` |"
        )
    lines.append("")

    # Stage 3 — round-trip
    rt = results.get("stage3_round_trip") or {***REMOVED***
    lines.append("## Stage 3 — Round-trip (TGClient.get_messages read-back)")
    lines.append("")
    lines.append(f"- Connected: **{rt.get('connected')***REMOVED*****")
    if rt.get("saved_msg_text_non_empty") is not None:
        is_yes = bool(rt.get("saved_msg_text_non_empty"))
        lines.append(
            f"- Saved msg_id `{rt.get('saved_msg_id')***REMOVED***` text non-empty: "
            f"**{'✅ TRUE' if is_yes else '❌ FALSE'***REMOVED*****"
        )
    if rt.get("saved_msg_text_head"):
        lines.append(f"  - text head: `{rt['saved_msg_text_head'***REMOVED******REMOVED***`")
    if rt.get("lit_msg_text_non_empty") is not None:
        is_yes = bool(rt.get("lit_msg_text_non_empty"))
        lines.append(
            f"- Литвинов msg_id `{rt.get('lit_msg_id')***REMOVED***` text non-empty: "
            f"**{'✅ TRUE' if is_yes else '❌ FALSE'***REMOVED*****"
        )
    if rt.get("lit_msg_text_head"):
        lines.append(f"  - text head: `{rt['lit_msg_text_head'***REMOVED******REMOVED***`")
    if rt.get("error"):
        lines.append(f"- Round-trip error: `{rt['error'***REMOVED******REMOVED***`")
    lines.append("")

    # Summary
    lines.append("## Summary")
    lines.append("")
    lines.append(f"- {results.get('summary', '(no summary)')***REMOVED***")
    lines.append(f"- Exit code: `{results.get('exit_code')***REMOVED***`")
    lines.append("")

    # Bugs encountered
    bugs: List[str***REMOVED*** = [***REMOVED***
    if not rt.get("connected"):
        bugs.append("TGClient not connected during round-trip")
    if rt.get("saved_msg_text_non_empty") is False:
        bugs.append(
            f"Saved msg_id {rt.get('saved_msg_id')***REMOVED*** not round-tripped "
    ...
    if (
        ...
        bugs.append(
            f"Литвинов msg_id {rt.get('lit_msg_id')***REMOVED*** not round-tripped"
        )
    if bugs:
        lines.append("## Bugs encountered")
        lines.append("")
        for bug in bugs:
            lines.append(f"- {bug***REMOVED***")
        lines.append("")
...
def main() -> int:
    ...
    if args.e2e_log is None:
        ...
        log_path = DEFAULT_E2E_LOG_DIR / f"remote_sync_{tag***REMOVED***.md"
    else:
    ...
    if not args.silent:
        ...
        print(
            f"   flags: sync_group={args.sync_group***REMOVED***, "
            f"skip_tg={args.skip_tg***REMOVED***, dry_run={args.dry_run***REMOVED***"
        )
        print(f"   log_path: {log_path***REMOVED***")
        print()
    ...
    results["stage4_log"***REMOVED*** = log_result

    if not args.silent:
        print("=== Run Summary ===")
        print(f"   {results.get('summary')***REMOVED***")
        print(
            f"   log written: {log_result['log_path'***REMOVED******REMOVED*** "
            f"({log_result['line_count'***REMOVED******REMOVED*** lines)"
        )
        print()
