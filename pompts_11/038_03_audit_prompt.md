...
Назначение: Запустить масштабный аудит всего проекта Freebuff с проверкой соответствия 040_13_code_quality_standard.md
...
Стандарт качества: docs_10/core/CODE_QUALITY_STANDARD.md (v2.0.0) — все проверки проводятся по этому документу
...
```bash
python -m pytest tests_09/ -q --tb=short --cov=scripts --cov=freebuff_plugin --cov=core
```
...
· StreamSession — tests_09/test_stream_session.py?
· StreamBridge — tests_09/test_stream_bridge.py?
· ContextManager — tests_09/test_context_manager.py?
· KeyPool — tests_09/test_keypool.py?
· API endpoints — tests_09/test_api.py?
· Phone MCP Server — tests_09/test_phone_mcp_server.py?
...
Проверь документацию (docs_10/core/ARCHITECTURE_3.0.md, docs_10/vision/VISION_3.0.md) против кода:
...
Проверь актуальность docs_10/decisions/DECISIONS.md:
...
🎯 Задание 3: Качество кода по 040_13_code_quality_standard.md (v2.0.0)

Стандарт: docs_10/core/CODE_QUALITY_STANDARD.md — обязателен для всех компонентов.
...
```bash
# Найти shell=True
grep -r "shell=True" scripts_01/ freebuff_plugin_03/ --include="*.py"

# Найти exec/eval
grep -r "exec(" scripts_01/ freebuff_plugin_03/ --include="*.py"
grep -r "eval(" scripts_01/ freebuff_plugin_03/ --include="*.py"

# Найти хардкоженные пути
grep -r "/storage/emulated/0" scripts_01/ freebuff_plugin_03/ --include="*.py"
grep -r "/data/data/com.termux" scripts_01/ freebuff_plugin_03/ --include="*.py"
```
...
```bash
python -m mypy scripts_01/ freebuff_plugin_03/ core_02/ --strict --ignore-missing-imports 2>&1 | tee mypy_report.txt
```
...
scripts_01/orchestrator.py CODE SERVICE Workflow engine ✅ Active
freebuff_plugin_03/... PLUGIN   
...    
...
· AGENTS.md vs docs_10/ops/AGENTS.md vs .freebuff/AGENTS.md
· BUFFY.md vs CLAUDE.md vs CODY.md vs .cursorrules
· scripts_01/mcp_server.py vs freebuff_plugin_03/mcp_server.py
· scripts_01/event_bus.py vs freebuff_plugin_03/event/store.py
· pompts_11/promt*.md — какие актуальны?
...
├── plugins_04/                 # Плагины (freebuff_plugin_03/)
├── cli_07/                     # CLI-инструменты
├── storage/                 # Базы данных
├── scripts_01/                 # Утилиты (админские)
├── tests_09/                   # Тесты
├── docs_10/
│   ├── 01-architecture/
│   ├── 02-specs/
...
Сформируй единый документ docs_10/audits/AUDIT_FULL_[DATE***REMOVED***.md:
...
· Сводная таблица по 14 разделам 040_13_code_quality_standard.md
...
7. Все проверки проводи по 040_13_code_quality_standard.md (v2.0.0)

Все найденные проблемы задокументируй в docs_10/audits/AUDIT_FULL_[DATE***REMOVED***.md
