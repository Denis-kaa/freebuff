...
- Fail-safe: try/except, dict {ok, …***REMOVED*** payload, exit 0/1/2.
...
# self._import_warnings = [***REMOVED***). The module-level singleton is kept as a
...
DEFAULT_CAPABILITIES: Tuple[str, ...***REMOVED*** = ()
DEFAULT_ROLE_IDS: Tuple[str, ...***REMOVED*** = (
    "explainer",
    "documenter",
    "retrospective",
)
...
class ExecutionRequest:
    ...
    factory_id: str
    forge_id: str
    role_ids: Tuple[str, ...***REMOVED***
    inputs: Dict[str, Any***REMOVED***
    output_spec: Dict[str, Any***REMOVED***
...
class BaseFactory:
    ...
    Subclasses ДОЛЖНЫ переопределить:
      - ``CAPABILITIES`` (Tuple[str, ...***REMOVED***): capability-токены домена.
    ...
    CAPABILITIES: Tuple[str, ...***REMOVED*** = DEFAULT_CAPABILITIES
    ROLE_IDS: Tuple[str, ...***REMOVED*** = DEFAULT_ROLE_IDS
    ...
    # get a fresh `[***REMOVED***` via __init__; lazy-import failures append HERE, not the
    # legacy module-level singleton (deprecated since v5.189.32).
    _import_warnings: List[str***REMOVED***  # per-instance attribute (ADR-015, set via __init__)
    ...
    def _lazy_forge_facade(self) -> Any:
        ...
        try:
            ...
            self._import_warnings.append(f"forge_facade: {exc***REMOVED***")
            return None
    ...
    def _lazy_memory_store(self) -> Any:
        ...
        try:
            ...
            # observable behavior (matches ``f"factory_registry: {exc***REMOVED***"`` /
            # ``f"forge_facade: {exc***REMOVED***"`` patterns). Conserves the exception in
    ...
    def normalize_input(self, opp: Any) -> Dict[str, Any***REMOVED***:
        ...
        return {
            ...
            "provenance": dict(getattr(opp, "provenance", {***REMOVED***) or {***REMOVED***),
    ...
    def build_execution_request(
        ...
        opp: Any,
        capability: str,
    ) -> Optional[ExecutionRequest***REMOVED***:
        ...
        return ExecutionRequest(
            ...
            output_spec={
                ...
                "target": f"projects_17/{getattr(opp, 'project_id', '') or '<slug>'***REMOVED***/forge/",
            ***REMOVED***,
        )
    ...
    def execute(
        ...
        *,
        ...
        Возвращает dict {ok, artifact?, request?, error?***REMOVED***. dry_run=True НЕ
        ...
        if not capability:
            return {"ok": False, "error": "no capability token on opportunity"***REMOVED***
        request = self.build_execution_request(opp, capability)
        if request is None:
            return {
                "ok": False,
                "error": f"capability {capability!r***REMOVED*** not offered by any factory/forge",
                "capability": capability,
            ***REMOVED***
        if dry_run:
            return {"ok": True, "dry_run": True, "request": request.to_dict()***REMOVED***

        facade = self._forge_facade or self._lazy_forge_facade()
        if facade is None:
            return {"ok": False, "error": "forge_facade unavailable", "request": request.to_dict()***REMOVED***

        project = self._resolve_project(opp, project_root=project_root)
        if project is None:
            return {
                "ok": False,
                "error": f"project {request.project_id!r***REMOVED*** unresolved (projects_17/<project_id>)",
        ...
        try:
            ...
            return {"ok": False, "error": f"run_chain: {exc***REMOVED***", "request": request.to_dict()***REMOVED***
    ...
    def _accumulate(
        ...
        *,
        ...
        result: Dict[str, Any***REMOVED*** = {"recorded": False, "knowledge_id": None***REMOVED***
        if memory is None:
        ...
        try:
            kid = memory.store_knowledge(
                ...
                title=f"{self.TITLE_PREFIX***REMOVED***:{artifact['id'***REMOVED******REMOVED***",
                summary=(
                    f"opportunity={artifact['opportunity_id'***REMOVED******REMOVED*** "
                    f"capability={artifact['capability'***REMOVED******REMOVED*** "
                    f"forge={artifact['forge_id'***REMOVED******REMOVED*** overall={overall***REMOVED***"
                ),
                tags=[self.TAG_PREFIX, artifact["capability"***REMOVED***, artifact["opportunity_id"***REMOVED******REMOVED***,
        ...
        try:
            eid = memory.record_learning_event(
                trigger_id=f"{self.TITLE_PREFIX***REMOVED***:{artifact['id'***REMOVED******REMOVED***",
                context_snapshot={
                    "opportunity_id": artifact["opportunity_id"***REMOVED***,
                    "capability": artifact["capability"***REMOVED***,
                    "factory_id": artifact["factory_id"***REMOVED***,
                    "forge_id": artifact["forge_id"***REMOVED***,
                    "overall": overall,
                ***REMOVED***,
                outcome=outcome,
                lesson_id=kid,
    ...
    @staticmethod
    def _derive_capability(opp: Any) -> Optional[str***REMOVED***:
        ...
        if helper is not None:
            try:
                ...
                pass
        prov = getattr(opp, "provenance", None) or {***REMOVED***
    ...
    def _resolve_project(self, opp: Any, *, project_root: Optional[Path***REMOVED*** = None) -> Any:
        """Project-объект для ForgeFacade.run_chain (best-effort, fail-safe)."""
        try:
            from core_02.workspace import Project
        except ImportError as exc:
            self._import_warnings.append(f"workspace.Project: {exc***REMOVED***")
            return None
    ...
    def _new_id(self) -> str:
        return f"{self.ID_PREFIX***REMOVED***-{uuid.uuid4().hex[:10***REMOVED******REMOVED***"
...
@classmethod  # type: ignore[arg-type***REMOVED***
def _cli_resolve(cls, args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "capability": capability,
        "factory_id": pair[0***REMOVED***.factory_id if pair else None,
        "forge_id": pair[1***REMOVED***.forge_id if pair else None,
    ...
    ***REMOVED***
    if args.json:
        ...
        _emit_text(
            f"resolve: capability={capability***REMOVED*** factory={payload['factory_id'***REMOVED******REMOVED*** forge={payload['forge_id'***REMOVED******REMOVED***",
            json_mode=False,
        )
    return 0
...
def _cli_run(cls, args: argparse.Namespace) -> int:
    ...
    ***REMOVED***
    if args.json:
        ...
        if result.get("ok"):
            _emit_text(
                f"run: ok artifact={result.get('artifact', {***REMOVED***).get('id')***REMOVED*** "
                f"forge={result.get('artifact', {***REMOVED***).get('forge_id')***REMOVED*** "
                f"overall={result.get('artifact', {***REMOVED***).get('overall')***REMOVED***",
                json_mode=False,
            )
        else:
            _emit_text(f"run: FAILED {result.get('error')***REMOVED***", json_mode=False)
...
def make_argparser(cls) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=cls.PROG,
        description=f"{cls.PROG***REMOVED*** — Phase 12 BaseFactory subclass.",
    )
    parser.add_argument(
        "--data-path", default=str(DEFAULT_DATA_PATH),
        help=f"Opportunity YAML path (default {DEFAULT_DATA_PATH***REMOVED***)",
    )
...
@classmethod  # type: ignore[arg-type***REMOVED***
def main(cls, argv: Optional[List[str***REMOVED******REMOVED*** = None) -> int:
    ...
    try:
        ...
        _emit_text(f"error: {cls.PROG***REMOVED*** unexpected failure: {exc***REMOVED***", json_mode=False)
        return 2


# Attach classmethods to BaseFactory after class body.
BaseFactory._cli_resolve = _cli_resolve  # type: ignore[attr-defined***REMOVED***
BaseFactory._cli_run = _cli_run  # type: ignore[attr-defined***REMOVED***
BaseFactory.make_argparser = make_argparser  # type: ignore[attr-defined***REMOVED***
BaseFactory.main = main  # type: ignore[attr-defined***REMOVED***
