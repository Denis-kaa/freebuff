...
- Проверьте реальные флаги бинаря:
  ...
  ```
- Если флаги отличаются, откройте issue или PR с обновлёнными аргументами в `src_06/workers/lightpanda_worker.py`.
...
**Решение (v1):** храните PandaScript в доступном для Termux пути, например внутри `freebuff/scripts_01/` или `freebuff/projects_17/`.
...
**Решение:**
- Убедитесь, что `freebuff/scripts_01/` доступен в `sys.path`.
- Проверьте, что `scripts_01/context_manager.py` не падает при инициализации.
...
**Причина:** `scripts_01/agent_context_bridge.py` использует константу `_FREEBUFF_ROOT`.
...
**Симптом:** `data_13/context.db` не растёт, конспекты не создаются.
...
- Проверьте права на запись в `freebuff/data_13/`.

---

### Summary assistant-сообщений обрезается

**Симптом:** в `data_13/context.db` видно обрезанный JSON с `... [truncated***REMOVED***`.

**Причина:** `_compact_response` обрезает JSON до 1000 символов, чтобы не переполнять БД.

**Решение:**
- Это ожидаемое поведение v1.
- Для хранения полных ответов используйте файловый стриминг в `sessions_15/`.
