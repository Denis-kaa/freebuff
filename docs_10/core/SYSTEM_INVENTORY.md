...
### 🟢 `core_02/interfaces.py` — Контракты агентов
...
### 🟢 `scripts_01/context_manager.py` — ContextManager
...
### 🟢 `scripts_01/stream_session.py` — Непрерывная запись
...
### 🟢 `scripts_01/stream_bridge.py` — Мост для Buffy
...
### 🟢 `scripts_01/auto_conspect.py` — Автосуммаризация *(cross-layer: Layer 2 + Layer 9)*
- Создаёт сжатый конспект сессии в `context_12/summaries/`
- Сохраняет чекпоинт POST_STEP
- Помечает сессию COMPLETED
- **Статус:** ✅ Production

### 🟢 `scripts_01/bootstrap.py` — Восстановление при старте *(cross-layer: Layer 2 + Layer 9)*
- Ищет последнюю ACTIVE сессию или создаёт новую
- Загружает последний конспект из `context_12/summaries/`
- Интегрирован StreamBridge (авто-стриминг при старте)
- Формирует стартовый промпт для Buffy
- GC при инициализации
- **Статус:** ✅ Production

### 🟢 `data_13/context.db` — SQLite база
- WAL-режим
...
### 🟢 Чекпоинты (файлы)
- 40 чекпоинтов в `context_12/checkpoints/`
- **Статус:** ✅ Active
...
### 🟢 `scripts_01/system_monitor.py` — Мониторинг устройства
...
### 🟢 `scripts_01/scanner.py` — Сканер инструментов
- Обход файловой системы по паттернам (11 категорий)
- JSON-отчёт + рекомендации
- **Статус:** ✅ Production

### 🟢 `scripts_01/auto_save.py` — Автосохранение диалога Buffy 🆕
...
### 🟢 `scripts_01/sdk_bridge.py` — Мост freebuff ↔ termux-ai-agent
- `SmartRouterAdapter` — адаптер роутера
- Конвертация ToolResult ↔ AgentResult
- **Статус:** ✅ Production

### 🟢 `scripts_01/integrate_agent.py` — Интеграция с termux-ai-agent
- `FreebuffBridge` — on_session_start, on_request, on_checkpoint, on_session_end
- Загружает последний конспект при старте
- **Статус:** ✅ Production

### 🟡 `scripts_01/archive/import_sessions.py` — Импорт истории (архивирован)
- Источники: OpenClaw (JSONL), Aider (Markdown), last_context.txt
- **Статус:** 🟡 Архивирован (2026-07-29) — не используется, перемещён в scripts_01/archive/

### 🟡 `scripts_01/archive/import_qwen.py` — Импорт Qwen IDE (архивирован)
- Memories → сессия, File-history → сессии, Projects → метаданные
- **Статус:** 🟡 Архивирован (2026-07-29) — не используется, перемещён в scripts_01/archive/

### 🟡 `scripts_01/archive/phone_mcp_server.py` — MCP-сервер телефона (архивирован)
...
### 🟢 `scripts_01/overlay_server.py` — Overlay Server
...
### 🟢 `scripts_01/overlay_client.py` — Overlay Client
- Отправка статуса агента в оверлей
- `status()`, `done()`, `error()`, `check_command()`
- CLI-режим
- **Статус:** ✅ Production

### 🟢 `scripts_01/overlay_float.sh` — Launcher
- 3 режима: termux-float, tmux split, direct
- **Статус:** ✅ Production

### 🟢 `scripts_01/tg_popup.sh` — Telegram Popup
- tmux popup с TG-клиентом (55%×65%)
- Self-healing: убивает orphaned процессы, чистит лок-файлы
- **Статус:** ✅ Production

### 🟢 `scripts_01/doc_reminder.sh` — Напоминание документации
- Проверка возраста docs_10/
- **Статус:** ✅ Production

### 🟢 `scripts_01/screenshot_tools.sh` — Скриншоты
- **Статус:** ✅ Production
...
### 🟡 `projects_17/tg_terminal_messenger/` — Telegram-клиент
...
- Полные ADR перемещены в `docs_10/engineering-memory/decisions/`.
- `docs_10/decisions/DECISIONS.md` теперь служит индексом со ссылками на отдельные ADR.

### 🟢 `../vision/ROADMAP.md` — План развития 🆕
### 🟢 `IDEAS.md` — Реестр архитектурных идей
- **Путь:** `docs_10/decisions/IDEAS.md` (перемещён из `docs_10/ops/` 2026-07-29)
...
- **Путь:** `docs_10/projects_meta/FILE_REGISTRY.md` (перемещён из корня 2026-07-29)
...
### 🟢 `docs_10/039_12_terminal_ai_studio_mobile.md` — Flutter-видение
...
### 🟢 `docs_10/core/DISTRIBUTED_SESSION_SPECIFICATION_v2.0.md` — Спецификация
...
### 🟢 `pompts_11/017_02_struktura_requirements_testy.md` — Промпт реализации (канон)
- Поэтапный план: 9 фаз, ~145 тестов, 17-21 дней
- Стаб-копия `docs_10/core/PROMPT_IMPLEMENTATION_v1.0.md` архивирована → `trash_21/` (2026-08-01)
- **Статус:** 📋 Фаза 0 готова, Фазы 1-8 — план

### 🟡 `freebuff_plugin_03/mesh/` — Код Mesh
- 7 подпакетов: core_02/, node/, session/, agent/, transport/, storage/
...
### 🟢 `tests_09/test_freebuff.py` — 24 теста
...
### 🟢 `tests_09/core/test_interfaces.py`, `test_router.py`
- **Статус:** ✅ Production
...
| **ИТОГО** | **~55** | **🎯 016_02_arhitektura_reorganizaciya.md + structure.md + 017_02_struktura_requirements_testy.md Phase 0 выполнены** |
...
| Аудит scripts_01/ (2026-07-29) | 🟢 Низкий | ✅ Выполнен — 4 мёртвых скрипта → scripts_01/archive/ |
