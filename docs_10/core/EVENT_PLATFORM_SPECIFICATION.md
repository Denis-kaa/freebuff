...
> **Основание:** [VISION_3.0.md***REMOVED***(../vision/VISION_3.0.md), [ARCHITECTURE_3.0.md***REMOVED***(ARCHITECTURE_3.0.md), [014_02_leviathan_arhitektura.md***REMOVED***(../../pompts_11/014_02_leviathan_arhitektura.md) (концепция #14)  
...
**Текущий статус:**
- Event Bus (scripts_01/event_bus.py) — ✅ Production, publish/subscribe, ~20 тестов
- Event Subscribers (scripts_01/event_subscribers.py) — ✅ Production, 4 теста
- Event Log (SQLite `context_12/events.db`) — ✅ Production, логирование всех событий
...
class EventStore:
    ...
    def __init__(self, db_path: Optional[Path***REMOVED*** = None):
        self._db_path = db_path or Path("context_12/events.db")
...
1. **Snapshot-поиск** — проверяет, есть ли сохранённый snapshot компонента
   (например, `data_13/knowledge_index.v2.snapshot`). Если snapshot есть
...
# 1. Ищет snapshot data_13/knowledge_index.v2.snapshot
...
***REMOVED***
{
    ...
    "inputSchema": {
        "target_type": { "type": "string", "enum": ["decision", "action", "config_change"***REMOVED*** ***REMOVED***,
        "limit": { "type": "number", "default": 20 ***REMOVED***
    ***REMOVED***
***REMOVED***
{
...
- [x***REMOVED*** **MCP tools** — event_search, event_timeline, event_replay, event_audit, event_pulse (реализованы 2026-08-01 в `scripts_01/mcp_server.py`, category `event`; также в `freebuff_plugin_03/mcp_server.py`)
- [ ***REMOVED*** **CLI** — buffy event search/timeline/replay/audit/pulse
- [ ***REMOVED*** **60+ тестов**, 0 failures

---

*Связанные документы: [VISION_3.0.md***REMOVED***(../vision/VISION_3.0.md), [ARCHITECTURE_3.0.md***REMOVED***(ARCHITECTURE_3.0.md), [POLICY_ENGINE_SPECIFICATION.md***REMOVED***(POLICY_ENGINE_SPECIFICATION.md), [scripts_01/event_bus.py***REMOVED***(../../scripts_01/event_bus.py), [scripts_01/event_subscribers.py***REMOVED***(../../scripts_01/event_subscribers.py)*
