...
> **Основание:** [VISION_3.0.md***REMOVED***(../vision/VISION_3.0.md), [014_02_leviathan_arhitektura.md***REMOVED***(../../pompts_11/014_02_leviathan_arhitektura.md) (концепция #4)  

> **⚠️ Важно:** Существующий `scripts_01/bootstrap.py` отвечает за **сессионный bootstrap** (создание сессии, конспект, StreamBridge).
...
> - `BootstrapEngine.session()` — сессионный bootstrap (миграция из `scripts_01/bootstrap.py`)
...
Bootstrap MCP tools регистрируются в `scripts_01/mcp_server.py` по тому же паттерну, что и Bridge Layer:
...
BootstrapEngine
  ...
  ├── scripts_01/ ───────── проверяет наличие всех скриптов
...
| Как быть с scripts_01/bootstrap.py (существующий)? | ✅ Решено: оба объединятся в `BootstrapEngine.session()` + `BootstrapEngine.run()` |
| Как интегрировать MCP tools? | ✅ Решено: паттерн `_get_bootstrap_engine()` + `_register_tools()` как у Bridge Layer |

---

*Связанные документы: [VISION_3.0.md***REMOVED***(../vision/VISION_3.0.md), [ARCHITECTURE_3.0.md***REMOVED***(ARCHITECTURE_3.0.md), [scripts_01/bootstrap.py***REMOVED***(../../scripts_01/bootstrap.py)*
