...
> **Основание:** [VISION_3.0.md***REMOVED***(../vision/VISION_3.0.md), [ARCHITECTURE_3.0.md***REMOVED***(ARCHITECTURE_3.0.md), [014_02_leviathan_arhitektura.md***REMOVED***(../../pompts_11/014_02_leviathan_arhitektura.md) (концепции #10–12)  
...
**🟡 Future extensions (из 014_02_leviathan_arhitektura.md #11):**
...
class PolicyStore:
    ...
    def __init__(self, db_path: Optional[Path***REMOVED*** = None):
        self._db_path = db_path or Path("data_13/policies.db")
...
> **✅ Реализован (2026-08-01, правило 11 / ADR-009):** `policy_override` —
> conversational User-Choice Override. Зарегистрирован в `scripts_01/mcp_server.py`
> (`McpTool`, category `policy`); HTTP-доступ без MCP — `POST /policy/override`
> в `scripts_01/mcp_fastapi.py`. Тесты: `test_policy_conversational.py`,
> `test_mcp_server.py::TestPolicyOverrideTool`,
> `test_mcp_fastapi.py::TestPolicyOverrideEndpoint`.
> Инструменты ниже (`policy_apply/list/status`, `pack_install`, `capability_list`) —
> запланированы (спецификация, см. §12).

```json
{
    "name": "policy_override",
    "description": "Conversational User-Choice Override: назначить Runtime на capability из фразы естественного языка (правило 11, ADR-009)",
    "inputSchema": {
        "type": "object",
        "properties": {
            "message": { "type": "string", "description": "фраза: 'use deepseek instead of claude for coding', 'используй freebuff для research'" ***REMOVED***
        ***REMOVED***,
...
> **Просмотр текущих политик (рядом с override, правило 11):**
> - **CLI** `freebuff policy list` / `freebuff policy get <capability>` — реализован
>   в [freebuff_cli.py***REMOVED***(../../freebuff_cli.py) (`cmd_policy`, User Preferences CLI).
> - **HTTP GET-эндпоинт** `GET /policy/status` в `scripts_01/mcp_fastapi.py` —
>   **✅ реализован (2026-08-01)**: возвращает `{success, data: {count, preferences,
>   policies***REMOVED******REMOVED***` — текущие предпочтения из policies.json. REST-доступ к правилу 11
>   полный: чтение через GET + запись через POST (`/policy/override`).
