...
# Или через Python:
python3 freebuff_plugin_03/api.py        # REST API на :8410
python3 freebuff_plugin_03/mcp_server.py  # MCP сервер (STDIO)
```
...
```bash
# Статус OOM защиты
bash scripts_01/oom_protect.sh --status

# Статус плагина
python3 freebuff_plugin_03/wrapper.py status

# Статус системы
python3 freebuff_cli.py status

# Список сценариев
python3 freebuff_plugin_03/scenario_engine.py list
```
...
python3 freebuff_plugin_03/scenario_engine.py get freelance_parser

# Сгенерировать промт для парсера example.com
python3 freebuff_plugin_03/scenario_engine.py apply freelance_parser \
  --vars '{"URL":"https://example.com","поле1":"title","поле2":"price","формат":"JSON"***REMOVED***'

# Полученный промт можно сразу скормить freebuff:
freebuff --oneshot "$(python3 freebuff_plugin_03/scenario_engine.py apply freelance_parser --vars '{"URL":"..."***REMOVED***' | tail -n +5)"
```
...
```bash
# Начать сессию
python3 freebuff_plugin_03/bridge.py start
# → a1b2c3d4

# Завершить (создаёт конспект)
python3 freebuff_plugin_03/bridge.py end a1b2c3d4 --summary "тест"
# → ✔ Конспект: context_12/summaries/conspect_test_2026-07-29.md
...
bash scripts_01/oom_protect.sh --force
```
...
```bash
# Сценарий
python3 freebuff_plugin_03/scenario_engine.py apply freelance_tg_bot \
...
freebuff --oneshot "$(python3 freebuff_plugin_03/scenario_engine.py apply freelance_tg_bot ...)"
```
...
### Лендинг
```bash
python3 freebuff_plugin_03/scenario_engine.py apply freelance_landing \
...
A: OOM Protection убивает старые freebuff перед запуском. Если всё равно не хватает — запусти `bash scripts_01/oom_protect.sh --force` вручную.

**Q: Как посмотреть результат задачи?**  
A: `python3 freebuff_plugin_03/wrapper.py status` покажет активные сессии. После завершения — `Get /context` или `GET /status`.

**Q: Сценарий не подходит?**  
A: Сценарии — это шаблоны. Их можно редактировать в `freebuff_plugin_03/scenarios/*.md` или добавить свой.
...
echo "=== OOM ==="
bash scripts_01/oom_protect.sh --status | head -5
echo "=== API ==="
curl -s http://127.0.0.1:8410/status 2>/dev/null | python3 -m json.tool || echo "API not running"
echo "=== MCP ==="
echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"***REMOVED***' | python3 freebuff_plugin_03/mcp_server.py 2>/dev/null | head -c 200 || echo "MCP not running"
echo "=== SCENARIOS ==="
