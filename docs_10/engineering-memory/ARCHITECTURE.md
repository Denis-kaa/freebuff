...
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │                    EM Orchestrator                           │   │
│  │   (scripts_01/engineering_memory.py + subscribers)            │   │
│  └────────────────────────┬─────────────────────────────────────┘   │
│                           │                                         │
│         ┌─────────────────┼─────────────────┐                       │
│                          ▼                 ▼                       │
...
4. **Сохранение.** Документ записывается в `docs_10/engineering-memory/` как Markdown с YAML frontmatter.
...
related_components: ["core_02/router.py", "freebuff_plugin_03/runtime/registry.py"***REMOVED***
...
Индексирует `docs_10/engineering-memory/**/*.md`:
...
**Пример:** `docs_10/engineering-memory/PROJECT_BOOK.md`.
...
Review (AI/human)
   │
   ▼
Final (Markdown in docs_10/engineering-memory/)
   │
   ▼
...
- **KnowledgeEngine** индексирует все `.md` файлы в `docs_10/engineering-memory/`.
...
- `docs_10/INDEX.md` обновляется со ссылками на EM разделы.
- `PROJECT_BOOK.md` служит входной точкой.
- Автоматически генерируемый `docs_10/engineering-memory/TIMELINE.md` может отображать записи по датам.
...
- Создать `docs_10/engineering-memory/` и структуру подкаталогов.
...
- Создать `scripts_01/engineering_memory.py` с `EMEngine` и `EMOrchestrator`.
- Добавить подписчика в `scripts_01/event_subscribers.py`: `on_task_completed`, `on_system_error`, `on_decision_logged`.
...
- Настроить `KnowledgeEngine` для индексации `docs_10/engineering-memory/**/*.md`.
...
```python
# scripts_01/engineering_memory.py (draft)
import json
...
- `docs_10/engineering-memory/PROJECT_BOOK.md` — пример Project Chronicle.
- [DECISIONS.md***REMOVED***(../decisions/DECISIONS.md) — индекс архитектурных решений (ADR-001…009).
- `decisions/` — канонические ADR (ADR_NNN_*.md).
- `pompts_11/025_02_principy_agenta.md` — принципы Reuse First.
...
- `scripts_01/context_manager.py` — Context Manager.
