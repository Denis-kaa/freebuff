...
В freebuff **worker** — это Python-класс, который делегирует сложную работу внешнему процессу или сервису. Workers живут в `src_06/workers/` и имеют единый интерфейс:
...
|--------|------|------------|
| **LightpandaWorker** | `src_06/workers/lightpanda_worker.py` | Headless browser automation через Lightpanda |
...
Изменение воркера автоматически активирует триггер **Worker / tool** в `scripts_01/buffy_autodoc.py`. Это означает, что при коммите кода в `src_06/workers/` pre-commit hook ожидает обновления `WORKERS.md` и `CHANGELOG.md`.

- Подробнее о триггерах: [`../core/RULES.md` — Авто-триггер документирования***REMOVED***(../core/RULES.md)
- Установить pre-commit hook: `bash scripts_01/install_hooks.sh`
...
1. Создай `src_06/workers/<name>_worker.py`.
2. Определи класс с суффиксом `*Worker`.
3. Возвращай результат через dataclass (`*Result`).
4. Добавь unit-тесты в `tests_09/test_<name>_worker.py`.
5. Зарегистрируй в `src_06/workers/__init__.py`.
6. Обнови этот документ и `../core/ARCHITECTURE.md`.
7. Обнови `CHANGELOG.md` и убедись, что pre-commit hook (`bash scripts_01/install_hooks.sh`) не блокирует коммит.
