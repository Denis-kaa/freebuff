...
3. **Один случай `shell=True`** в `freebuff_plugin_03/bootstrap/installer.py` (линия 205) — критическая уязвимость, несмотря на якобы полное устранение в v2.8.0.
4. **Дублирующие документы** — 3 версии `AGENTS.md` (root, docs_10/, .freebuff/), fallback-файлы `CLAUDE.md`/`CODY.md`/`.cursorrules`, два MCP-сервера (`scripts_01/mcp_server.py` и `freebuff_plugin_03/mcp_server.py`).
...
| `freebuff_plugin_03/api.py` | ❌ Нет | API endpoints сценариев не тестированы |
| `scripts_01/phone_mcp_server.py` | ❌ Нет | Отдельный MCP-сервер телефона без тестов |
| `scripts_01/dashboard_api.py` | ❌ Нет | Дашборд без тестов |
...
| **Топ-3 файла** | `freebuff_plugin_03/tgbot.py` (45), `freebuff_plugin_03/mcp_server.py` (13), `freebuff_plugin_03/mcp_client.py` (10) |
...
- `name-defined` — `c_ushort`/`c_uint` в `freebuff_plugin_03/bootstrap/checker.py`, `sys` не импортирован в `freebuff_plugin_03/bridge_layer.py`.
...
| Проверка | Результат |
|----------|-----------|
| `shell=True` в `.py` | ⚠️ `freebuff_plugin_03/bootstrap/installer.py:205` |
...
|----------|------------|------------|
| `.bak` файлы | 10+ | В основном в `pompts_11/` и `projects_17/diet_platform/` |
| `__pycache__` директорий | 38 | Не должны попадать в git (в `.gitignore`) |
| `.pyc` файлов | 149 | Артефакты |
| `node_modules` / `dist` | Есть в `buffy-playground_19/` | Ожидаемо для Vite-проекта |
...
|-------------------|-------------------|---------------------------|
| Orchestrator | ✅ `scripts_01/orchestrator.py` | `../core/ARCHITECTURE_3.0.md` |
| Context Keeper | ✅ `scripts_01/context_manager.py` | `BUFFY.md`, `../ops/SESSION_GUIDE.md` |
...
| Auditor | ⚠️ Частично (`drift_check.py`, аудиты) | `docs_10/AUDIT_*.md` |
| Response Writer | ✅ `freebuff_plugin_03/scenario_engine.py` | `../ops/PROMPT_BASE.md` |
| Developer | ✅ Buffy / Codebuff CLI | `BUFFY.md` |
| Frontend Dev | ⚠️ `buffy-playground_19/` | `buffy-playground_19/README.md` |
| DevOps | ⚠️ `scripts_01/oom_protect.sh`, wrapper | `../projects_meta/LIGHTPANDA_INTEGRATION.md` |
| Tester | ✅ 1124 pytest тестов | `tests_09/` |
| Fixer | ️ `scripts_01/drift_check.py` | `DRIFT_REPORT.md` |
| Acceptance Agent | ❌ Нет | ❌ Нет |
| Documenter | ✅ `scripts_01/buffy_autodoc.py`, CHANGELOG | `../core/RULES.md` |
| Retrospective Agent | ⚠️ `scripts_01/auto_conspect.py` | `../ops/SESSION_GUIDE.md` |
...
|---------------------|--------|
| `scripts_01/mcp_server.py` ↔ `scripts_01/tool_runtime.py` | Нет цикла, mcp_server использует tool_runtime |
| `freebuff_plugin_03/bridge_layer.py` ↔ `freebuff_plugin_03/acp_protocol.py` | Нет цикла, bridge использует acp |
| `scripts_01/event_bus.py` ↔ подписчики | Слабая связь через callback, циклов не обнаружено |
| `freebuff_plugin_03/bootstrap/engine.py` ↔ `freebuff_plugin_03/runtime/` | Runtime использует Bootstrap installer, но не наоборот (пока) |

### 3.5 Нарушения Core/Extensions/Labs

| Проверка | Результат |
|----------|-----------|
| Core импортирует Extensions | ⚠️ `scripts_01/context_manager.py` публикует события, но не импортирует Extensions напрямую — ок |
...
|-----------|------------|---------|
| **CODE** | 132 | `scripts_01/*.py`, `freebuff_plugin_03/**/*.py`, `core_02/*.py` |
| **CONFIG** | 58 | `.freebuff/config.json`, `profiles.yaml`, `requirements.txt`, `.gitignore` |
| **DATA** | 109 | `data_13/*.db`, `context_12/**/*.db`, `*.npy`, `*.log` |
...
| **DOC-AUDIT** | 9 | `docs_10/AUDIT_*.md` |
| **DOC-SESSION** | 2 | `docs_10/session_dumps/*.md` |
| **TEST** | 37 | `tests_09/test_*.py` |
...
| **MCP Server** | `scripts_01/mcp_server.py` (79 KB), `freebuff_plugin_03/mcp_server.py` (32 KB) | Два MCP-сервера, разные размеры | Проверить, не дублируют ли они функциональность; возможно, один должен быть обёрткой |
| **Event Bus** | `scripts_01/event_bus.py` (20 KB), `freebuff_plugin_03/event/store.py` (22 KB) | Разные назначения, но похожие имена | ✅ Не дублируют: event_bus — шина, store.py — хранилище Event Platform |
| **Promt-файлы** | `pompts_11/promt*.md`, `pompts_11/new.md` | Много промптов, неясно какие актуальны | Добавить `pompts_11/README.md` с индексом и статусами |

### 4.3 Предлагаемая новая структура

```
freebuff/
├── core_02/                          # Ядро (было scripts_01/core-*)
│   ├── __init__.py
│   ├── interfaces.py
│   └── router.py
├── services_08/                      # Сервисы (было scripts_01/)
│   ├── context_12/
│   ├── memory/
│   ├── knowledge/
│   ├── graph/
...
│   └── mcp/
├── plugins_04/                       # Плагины
│   ├── freebuff_plugin_03/           # основной плагин
│   └── hello_world/
├── cli_07/                           # CLI и entrypoints
│   ├── freebuff_cli.py
│   └── wrapper/
├── tests_09/                         # Тесты (сохранить плоским)
├── docs_10/
│   ├── 01-architecture/
│   ├── 02-specs/
│   ├── 03-audits/
...
│   ├── 07-roadmap/
│   └── 08-references/
├── prompts/                       # pompts_11/ → prompts/
├── projects_17/                        # внешние проекты
├── data_13/                            # БД и индексы
├── scripts_01/                         # админ/utility скрипты (сократить)
├── README.md
├── BUFFY.md
├── CHANGELOG.md
└── SPEC.md
...
|----------|--------------|
| `pompts_11/*.bak` | Удалить или перенести в `pompts_11/archive/` |
| `projects_17/diet_platform/*.bak` | Удалить |
| `__pycache__` / `*.pyc` | Уже в `.gitignore`, но в рабочей директории занимают место |
| `context_12/` runtime-директории | ✅ Оставить, это рабочие данные |
...
|---|-------------|-----------|-------|
| 1 | **Убрать `shell=True` в `bootstrap/installer.py:205`** | S | `freebuff_plugin_03/bootstrap/installer.py` |
| 2 | **Исправить 108 ошибок mypy** | M-L | `freebuff_plugin_03/tgbot.py`, `freebuff_plugin_03/mcp_server.py`, `freebuff_plugin_03/mcp_client.py` и др. |
...
| # | Рекомендация | Сложность | Файлы |
|---|-------------|-----------|-------|
| 4 | **Добавить тесты для `freebuff_plugin_03/api.py`**, `KeyPool`, `StreamSession/StreamBridge` | M | `tests_09/test_api.py`, `tests_09/test_keypool.py` |
...
| 7 | **Исправить неработающие импорты** (`RuntimeRegistry`, `ScenarioBot`) | S | `freebuff_plugin_03/runtime/__init__.py`, `freebuff_plugin_03/tgbot.py` |

### P2: Средние (следующий спринт)

| # | Рекомендация | Сложность | Файлы |
|---|-------------|-----------|-------|
| 8 | **Провести реструктуризацию `scripts_01/` → `services_08/` и `cli_07/`** | L | Множество импортов |
| 9 | **Добавить `KeyPool` как отдельный тестируемый компонент** | M | `scripts_01/model_gateway.py`, `tests_09/test_keypool.py` |
| 10 | **Создать `pompts_11/README.md` с индексом и статусами промптов** | S | `pompts_11/` |
...
| 12 | **Добавить pre-commit hook на mypy zero-errors** | S | `.pre-commit-config.yaml` или `scripts_01/pre-commit` |
| 13 | **Автоматизировать обновление «Current State vs Vision» таблицы** | M | `scripts_01/drift_check.py` |
...
|---|-----|-----------|-----------------|---------------------|
| 1 | Исправить `shell=True` в `bootstrap/installer.py` | S | `freebuff_plugin_03/bootstrap/installer.py` | Устранение критической уязвимости |
| 2 | Исправить 108 mypy ошибок | M | `freebuff_plugin_03/tgbot.py`, `freebuff_plugin_03/mcp_server.py`, `freebuff_plugin_03/mcp_client.py` | `mypy` проходит без ошибок |
...
| 4 | Удалить/заархивировать `.bak` файлы | S | `pompts_11/*.bak`, `projects_17/diet_platform/*.bak` | Чистая рабочая директория |
| 5 | Объединить/прояснить `AGENTS.md` | S | `AGENTS.md`, `../ops/AGENTS.md`, `.freebuff/AGENTS.md` | Единый источник правил для агентов |
| 6 | Исправить импорты `RuntimeRegistry` и `ScenarioBot` | S | `freebuff_plugin_03/runtime/__init__.py`, `freebuff_plugin_03/tgbot.py` | Все ключевые импорты работают |
| 7 | Добавить недостающие тесты (API, KeyPool, StreamSession) | M | `tests_09/test_api.py`, `tests_09/test_keypool.py` | Покрытие критичных компонентов |
| 8 | Провести реструктуризацию папок | L | `scripts_01/`, `freebuff_plugin_03/`, `core_02/` | Упрощённая навигация |
| 9 | Добавить pre-commit mypy hook | S | `scripts_01/pre-commit` | Предотвращение регрессий |
| 10 | Автоматизировать аудит «Current State vs Vision» | M | `scripts_01/drift_check.py` | Регулярная сверка док-код |
...
**Примечание:** `RuntimeRegistry` реализован в `freebuff_plugin_03/runtime/registry.py`, но не экспортирован из `freebuff_plugin_03/runtime/__init__.py`. `ScenarioBot` либо переименован, либо отсутствует в `freebuff_plugin_03/tgbot.py`.
...
*Отчёт создан на основе промпта `pompts_11/038_03_audit_prompt.md`. Никакие изменения в код не внесены.*
