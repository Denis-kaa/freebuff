...
**Дата:** 2026-07-31
**Предмет:** `docs_10/audits/DOMAIN_MODEL_WORKSPACE_OS_2026-07-31.md` (promt24)
...
|---|---|---|---|---|
| 1 | Session | `scripts_01/context_manager.py` | ContextManager, SessionSnapshot, SessionStatus, CheckpointType | ✅ |
| 2 | MemoryEntry | `scripts_01/memory_engine.py` | MemoryEntry, MemoryLevel, ContentType | ✅ |
| 3 | KnowledgeDocument | `scripts_01/knowledge_engine.py` | Document, FtsIndex, TfidfIndex, SemanticIndex | ✅ |
| 4 | GraphNode/Edge | `scripts_01/graph_index.py` | Node, Edge, PathResult, GraphStats | ✅ |
| 5 | RAGResult/Report | `scripts_01/rag_engine.py` | RAGResult, RAGReport, FeatureVector, RAGEngine | ✅ |
| 6 | Project | `scripts_01/scan_projects.py` | scan_projects, detect_git_remote, detect_language | ✅ |
| 7 | Scenario | `freebuff_plugin_03/scenario_engine.py` | Scenario, ScenarioEngine | ✅ |
| 8 | Task/Workflow/Step | `scripts_01/orchestrator.py` | Workflow, Step, DefaultPlanner, ToolExecutor | ✅ |
| 9 | VerificationRule/Result | `scripts_01/verifier.py` | VerificationRule, VerificationResult, Verifier | ✅ |
| 10 | MetricResult/Report | `scripts_01/metrics.py` | MetricResult, MetricsReport, MetricsEngine | ✅ |
| 11 | Runtime | `freebuff_plugin_03/runtime/` | RuntimeAdapter, StdioMCPAdapter, HTTPMCPAdapter, AdapterRegistry, RuntimeRegistry, RuntimeCapabilityRegistry | ✅ |
| 12 | Model | `scripts_01/model_gateway.py` | ModelGateway, BaseProvider, GeminiProvider, OllamaProvider | ✅ |
| 13 | Plugin | `scripts_01/plugin_api.py` | PluginManifest, PluginRegistry, BasePlugin | ✅ |
| 14 | Event | `scripts_01/event_bus.py` | Event, EventBus, Subscription | ✅ |
| 15 | EventEntry/Timeline/Audit | `freebuff_plugin_03/event/` | EventStore, EventReplay, TimelineEngine, AuditEngine, PulseEngine | ✅ |
| 16 | Role | `scripts_01/roles.py` | RoleDefinition, AgentRole, RoleEngine | ✅ |
| 17 | AgentPresence | `scripts_01/presence.py` | AgentPresence, PresenceHistoryEntry, PresenceEngine | ✅ |
| 18 | CollabSession/Message | `scripts_01/collaboration.py` | CollaborationSession, CollabMessage, Participant, CollaborationEngine | ✅ |
| 19 | AgentNode/AgentTask | `scripts_01/distributed_agents.py` | AgentNode, AgentTask, AgentMesh, TaskDistributor, DistributedCoordinator | ✅ |
| 20 | PulseEntry | `scripts_01/project_pulse.py` | PulseEntry, ProjectPulse | ✅ |
| 21 | Notification | `scripts_01/notification.py` | notify, notify_task_complete, notify_error | ✅ |
...
| C6 | Memory ↔ Knowledge | `plugins_04/knowledge_sync/` | плагин: исходник 🔶 утерян, pyc загружается (KnowledgeSyncPlugin); путь исправлен на `__pycache__` | ✅ (уточнено) |
...
| C10 | Runtime ↔ Policy | `runtime_05/registry.py:657`, `policy/engine.py` | RuntimeCapabilityRegistry грузит scores; PolicyEngine потребляется на слое mcp_server (`mcp_server.py:374-384`, init с registry + cap_reg) | ✅ |
...
| C13 | EventBus ↔ 10+ модулей | импорты event_bus | **18 модулей** в scripts_01/ импортируют `from scripts.event_bus import` (больше заявленных 10+) | ✅ |
...
Проверка: `grep` по всему коду (`scripts_01/`, `freebuff_plugin_03/`, `core_02/`).
...
|---|---|---|
| 7 SQLite БД в `data_13/` | `ls data_13/*.db` | ✅ context, verifier, metrics, roles, presence, collaboration, project_pulse |
| 51 MCP-инструмент в `scripts_01/mcp_server.py` | импорт BuffyMcpServer, `len(_tools)` | ✅ 51 |
...
| 4-й плагин hello_world на диске | `ls plugins_04/hello_world/` | ✅ __init__.py + manifest.json |
