...
```python
# tests/conftest.py — добавить фикстуры для Mesh
...
```python
# freebuff_plugin/mesh/core/event.py
...
```python
# freebuff_plugin/mesh/core/event_store.py
...
```python
# freebuff_plugin/mesh/storage/sqlite.py
...
```python
# freebuff_plugin/mesh/core/vector_clock.py
...
```python
# freebuff_plugin/mesh/node/device.py
...
```python
# freebuff_plugin/mesh/node/capability.py
...
```python
# freebuff_plugin/mesh/session/lease.py
...
```python
# freebuff_plugin/mesh/session/session.py
...
```python
# freebuff_plugin/mesh/session/offline.py
...
```python
# freebuff_plugin/mesh/session/sync.py
...
```bash
# 1. Создать структуру
mkdir -p freebuff_plugin/mesh/{core,node,session,agent,transport,storage***REMOVED***
...
pytest tests/test_mesh.py -v --tb=short

# 5. Проверять покрытие
pytest tests/test_mesh.py --cov=freebuff_plugin.mesh --cov-report=term

# 6. Проверять типы
mypy freebuff_plugin/mesh/ --strict
