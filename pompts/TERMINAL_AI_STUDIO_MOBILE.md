...
2. Предоставляет OpenAI-совместимый API эндпоинт для локальной LLM, чтобы модель можно было использовать из любого агентского фреймворка
3. Позволяет указывать модель в приложении — выбор модели, настройка параметров, переключение между локальной и облачными моделями
...
5. Gemma 4 через LiteRT-LM (локальный LLM подход)
...
1. Оценка целесообразности Flutter + локальный API:
...
· Потенциальные подводные камни (Android 15 FGS ограничения , bionic vs glibc , производительность локальной модели )
...
· Как можно адаптировать под FreeBuff + локальную LLM API

3. Архитектура "Оркестратор + Воркер + API эндпоинт":

Предложи архитектуру для гибридного пайплайна:

· Оркестратор (локальная модель) определяет сложность задачи
· Воркеры (FreeBuff, shell_exec, file_operations) выполняют задачи 
· Локальный LLM API эндпоинт для взаимодействия с моделями
...
4. Иерархические воркеры с эскалацией (воркер → FreeBuff → локальный LLM → облачный LLM)
...
2. Интеграция llama-server (или LiteRT-LM) как локального API эндпоинта
...
· ARCHITECTURE.md — архитектура Flutter-приложения + локальный LLM API
