...
Использование:
    python scripts/doctor.py                 # Базовая диагностика
    python scripts/doctor.py --full           # Полная проверка
    python scripts/doctor.py --check-runtime freebuff  # Проверить Runtime
    python scripts/doctor.py --fix            # Авто-исправление
    python scripts/doctor.py --json           # Вывод в JSON
"""
...
def check_tests(report: DoctorReport) -> None:
    ...
    if not tests_dir.exists():
        report.checks.append(CheckResult("Tests", "warn", "tests/ directory not found"))
        return

    try:
        result = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/", "-q", "--tb=no", "--collect-only"***REMOVED***,
            cwd=str(WORKSPACE),
