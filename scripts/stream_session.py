...
Каждое сообщение автоматически сохраняется в:
  context/streams/<topic_YYYY-MM-DD_HHMMSS>/
...
Использование:
    python scripts/stream_session.py start <topic>        # новая сессия
    python scripts/stream_session.py resume <session_id>  # продолжить старую
    python scripts/stream_session.py log <role> <text>    # добавить сообщение
    python scripts/stream_session.py checkpoint <msg>     # чекпоинт
    python scripts/stream_session.py tail                 # последние 20 строк
    python scripts/stream_session.py status               # статус
    python scripts/stream_session.py list                 # все сессии
    python scripts/stream_session.py prune                # GC: старые стримы + abandoned
"""
...
STREAMS_DIR = WORKSPACE / "context" / "streams"
...
def log_message(role: str, content: str) -> Optional[int***REMOVED***:
    ...
    if session_dir is None:
        print("❌ Нет активной сессии. Сначала: python scripts/stream_session.py start <topic>", file=sys.stderr)
        return None
...
def print_status() -> None:
    ...
    if session_dir is None:
        ...
        print("   Начни: python scripts/stream_session.py start \"тема\"")
        return
    log_file = session_dir / "conversation.log"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Примеры:
  python scripts/stream_session.py start "Редизайн TUI"
  python scripts/stream_session.py resume 26275a79
  python scripts/stream_session.py attach 26275a79
  python scripts/stream_session.py log user "Привет"
  python scripts/stream_session.py log assistant --file response.txt
  python scripts/stream_session.py checkpoint "Закончил этап A"
  python scripts/stream_session.py tail
  python scripts/stream_session.py status
  python scripts/stream_session.py list
  python scripts/stream_session.py prune
  python scripts/stream_session.py prune --dry-run
        """,
    )
