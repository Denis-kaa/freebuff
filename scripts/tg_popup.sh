...
#
# Использование:
#   bash scripts/tg_popup.sh          ← открыть TG поверх всего
#   bash scripts/tg_popup.sh start    ← запустить TG в фоне
#   bash scripts/tg_popup.sh kill     ← остановить фоновый TG
#
# В попапе:
...
TG_DIR="$FREEBUFF_ROOT/projects/tg_terminal_messenger"
SESSION="tg-bg"
...
start_bg() {
    ...
    kill -9 $(ps aux 2>/dev/null | grep 'python.*src/ui/app' | awk '{print $2***REMOVED***') 2>/dev/null || true
    ...
    tmux new-session -d -s "$SESSION" \
        "TERM=xterm-256color python src/ui/app.py; echo 'TG_STOPPED'; sleep 300"
