...
> **Основание:** 016_02_arhitektura_reorganizaciya.md §2.3, 040_13_code_quality_standard.md §12.4
...
│                              ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │ LAYER 2: plugins_04/ (Python-расширения)                       │  │
│  │                                                              │  │
...
|-----------|-------|--------|
| **RuntimeAdapter ABC** | `freebuff_plugin_03/runtime/adapter.py` | ✅ Готов |
| **StdioMCPAdapter** | `freebuff_plugin_03/runtime/adapter.py` | ✅ Готов (универсальный для MCP) |
| **HTTPMCPAdapter** | `freebuff_plugin_03/runtime/adapter.py` | ✅ Готов (универсальный для HTTP MCP) |
| **AdapterRegistry** | `freebuff_plugin_03/runtime/adapter.py` | ✅ Готов (register/get/create) |
| **RuntimeRegistry** | `freebuff_plugin_03/runtime/registry.py` | ✅ Готов (+ load из YAML) |
| **RuntimeCapabilityRegistry** | `freebuff_plugin_03/runtime/registry.py` | ✅ Готов (+ load из YAML) |
| **Provider manifests** | `runtime_05/providers/*.yaml` | ✅ 3 манифеста |
| **Plugin система** | `runtime_05/plugins/` | 🟡 Структура создана (плагинов пока нет — MCP покрывает всё) |
| **Recipe система** | `runtime_05/recipes/` | ✅ 2 рецепта + README |
| **Doctor CLI** | `scripts_01/doctor.py` | ✅ Готов |
...
```bash
# Создай файл
nano runtime_05/providers/cursor.yaml
```
...
```bash
mkdir -p runtime_05/recipes/cursor
nano runtime_05/recipes/cursor/RECIPE.md
```
...
```bash
mkdir -p runtime_05/plugins/cursor
nano runtime_05/plugins/cursor/__init__.py
```

## Проверка: новый Runtime без изменения ядра

```bash
# До: трогаем ТОЛЬКО runtime_05/providers/
touch runtime_05/providers/cursor.yaml
...
**Ядро (`freebuff_plugin_03/runtime/registry.py`, `adapter.py`) — без изменений.**
...
├── plugins_04/            # ~5 плагинов для нестандартных Runtime
...
- Community contributions через PR в `runtime_05/providers/`

## Связанные документы

- `docs_10/core/RUNTIME_ABSTRACTION_SPECIFICATION.md` — спецификация RAL
- `docs_10/core/COMPATIBILITY_MATRIX.md` — матрица совместимости
- `docs_10/core/ARCHITECTURE_PRINCIPLES.md` — принципы (Loose coupling, Marketplace-ready)
- `freebuff_plugin_03/INTEGRATION_CONTRACT.md` — контракт ядро↔плагин
- `runtime_05/README.md` — обзор системы Recipes
- `runtime_05/providers/README.md` — формат манифеста
