...
def _cli_propose(args: argparse.Namespace) -> int:
    ...
    opp = propose(opp)
    store.upsert(opp)
    payload = {"opportunity_engine": "propose", "opportunity": opp.to_dict()}
    if args.json:
        _emit_json(payload)
    else:
...
def _cli_run(args: argparse.Namespace) -> int:
    ...
    store.upsert(opp)
    payload = {"opportunity_engine": "run", "dry_run": False, "opportunity": opp.to_dict()}
    if args.json:
        _emit_json(payload)
    else:
        _emit_text(
...
def _cli_status(args: argparse.Namespace) -> int:
    ...
    if opp is None:
        ...
        return 1
    payload = {"opportunity_engine": "status", "opportunity": opp.to_dict()}
    if args.json:
        _emit_json(payload)
    else:
        _emit_text(
