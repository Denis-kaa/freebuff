...
For full context, read `docs_10/core/CORE_PROMPT.md` (identity, duties, constraints, behavior), `BUFFY.md` and `AGENTS.md`.
...
1. Read `docs_10/core/CORE_PROMPT.md` first (single source of truth for behavior).
2. Read `BUFFY.md` and `AGENTS.md`.
3. Read `TASK.md` and `CHANGELOG.md`.
4. Run `python freebuff_cli.py status` to see system state.
5. After changes run `python -m pytest tests_09/ -v` and `python -m mypy scripts_01/ core_02/ --ignore-missing-imports`.
