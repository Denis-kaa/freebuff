...
### Шаг 12. Layout-аудит и фикс мобильного сжатия панелей
- **Программный аудит** (`scripts/layout_audit.py`, playwright): клик через все 5 видов на desktop+mobile, замер overflowX, широчайшего нарушителя, битых картинок, якорного приземления, интеракций. Первый прогон: всё чисто (overflow 0, exit 0) — но overflow-проверка слепа к классу дефектов «сжатие».
- **Найдено код-ревью и подтверждено замером:** двухпанельные демо-виды использовали inline-фиксированные треки (`300px+1fr` в review, `1fr+320px` в team) — при 390px не было overflow (второй трек `minmax(0,…)`), но основная панель сжималась до ~60px: формально «без дефекта», фактически unusable.
- **Фикс:** `.two-pane` классы с breakpoint 900px → одна колонка на мобильном (замер после фикса: track 390px на team/review), desktop без изменений. Аудит расширен замером ширины последней панели (fail < 280px) — чтобы «сжатие» ловилось автоматически.
- Скриншот-проход дополнен видами team/review (desktop+mobile, итого 11 снимков в `/shots/` + копия на телефоне); консоль по-прежнему чистая, видео играет.
- **Урок:** overflow-метрика — необходимое, но недостаточное условие «layout ok»; фиксированные треки рядом с `minmax(0,1fr)` маскируют сжатие под норму. Проверять ширину панелей, не только выход за вьюпорт.

### Шаг 13. Ревью 11 снимков (пиксельный + DOM-аудит) — три реальных дефекта
- **Инструменты:** пиксельный разбор снимков (edge-strips, ink-плотность) + `scripts/probe_edge.py` (элементы у краёв вьюпорта, высота хедера) + `scripts/probe_sections.py` (что реально видно в точках скролла).
- **ДЕФЕКТ 1 (главный): `.section-padding` шортхендом `padding: xxl 0` затирал горизонтальный padding `.container`** (та же специфичность, позже в файле) — текст демо-видов (дашборд/драфт/ревью/родитель) рендерился вплотную к краям экрана. Концепт-вид не страдал (другая структура), поэтому прошлые аудиты прошли. Фикс: longhand top/bottom, шортхенд удалён.
- **ДЕФЕКТ 2:** mobile-хедер после wrap ≈ 139px, а `scroll-margin-top` был 84px — якорные секции приземлялись под хедером на телефоне. Фикс: 152px в breakpoint ≤900px (замер после: top=152 > headerH=139).
- **ДЕФЕКТ 3:** в 32px аватар-круге хедера просвечивал текст промт-слоя IMG-02 («AVATAR · 1:1»). Фикс: `.header-eco .ph-prompt { display: none }`.
- **Артефакт метрики (не продукта):** первые «NO-TEXT» флаги на 02/03/04/09 — погрешность оценки (200px-миниатюра + порог <70): 16px текст антиалиасится в светло-серый; при пороге <140 и DOM-прубе (заголовки/карточки в точке скролла) контент подтверждён.
...
- **Расширение ночного аудита (6910ca9):** второй чек в том же cron-заходе — `screenshots.py --report --strict`: JSON-отчёт рядом с layout-отчётом, exit 1 при console/page-ошибках, упавших запросах, стоящем/не двигающемся видео или нехватке снимков; ротация обоих видов отчётов.

### Шаг 15. Skill Graph (Этап 3.1) + TanStack Query — решение
- **Feature layer** (`features/skill-tree`): `buildSkillGraph` — чистая функция (headless-тестируемая, как вся механика): узлы по закрытому словарю 8 навыков (ANTI-6b), курируемая таблица бустов `SKILL_BOOSTS` (9 правил — дизайнерское решение, ревьюимое попарно, не алгоритм), `effective = min(100, round(stored + boost))`, pulse-флаг при >80, proofs через аддитивное `Proof.skills?`.
- **Бусты двигают эффективный уровень, не хранимый Score** — доказательства привязаны к stored (анти-геймификация §2.1: нельзя «накачать» показатель без артефактов).
- **Widget** (`widgets/skill-graph`): радиальный SVG (узел 14–36px по уровню, рёбра — толщина по весу буста, пунктир у отсутствующих навыков, pulse-кольца), таблица stored-vs-effective, пикер из 200 фрилансеров; вид `#skills`.
- **Смоук поймал реальный баг:** boost округлялся до 0.1 ПОСЛЕ вычисления effective → опубликованный узел не сходился с формулой. Фикс: сначала округляем boost, effective из округлённого. Плюс урок: инвариант в тесте должен повторять ДЕЙСТВИТЕЛЬНЫЙ контракт (round забыл и в тесте). Smoke 82→91.
- **TanStack Query отложен осознанно:** он решает кэширование СЕРВЕРНОГО состояния; вся экосистема сейчас — детерминированный mock. Вводить его до появления API = абстракция без объекта. Первый шаг при появлении бэкенда: `shared/api` контракт уже созрел.
- Деплой: сборка + `#skills` на :8022, снимок 12-desktop-skills в `/shots/`, ночной аудит (layout + media) прогнан вручную — PASS/PASS.

### Шаг 16. Дашборд → граф навыков (карточки-ссылки, followup)
- **Почему:** граф был изолирован — до него добираться только через меню навигации; связь «увел Skill Score на дашборде → клик → граф» должна быть в один клик.
- **Что:** в сайдбаре дашборда — кнопка «Граф навыков: {имя} →» под списком навыков (ведёт к графу ТЕКУЩЕГО пользователя: SkillGraphView уже резолвит `currentUserId` как дефолт пикера) + тизер-карточка «Граф навыков» с пояснением effective-vs-stored (анти-геймификация). Обе — `navigate('skills')` из роутера; CSS `.skill-graph-teaser`/`.skill-graph-link` в theme.css.
- **Урок SSH:** после сбоя `ssh <алиас>` НЕ надо подбирать IP/ключи вручную — так ловится fail2ban-бан; правильный паттерн из SERVER_ACCESS_WHIMCO.md: `ssh -o ControlMaster=no -o ControlPath=none -F ~/.ssh/config whimco` (raw-IP с тем же ключом даёт Permission denied — конфиг содержит то, чего нет в CLI-флагах).
- **Урок Playwright:** `inner_text()` возвращает текст ПОСЛЕ CSS-применения — при `text-transform: uppercase` сравнение делаем `.lower()`-независимым.
- **Проверка:** tsc clean, smoke 91/91; на сервере новый `scripts/probe_dashboard_link.py` — PASS (кнопка → `#skills`, SVG рендерится, 0 console-ошибок); strict-прогон screenshots.py — PASS (видео играет, 12 снимков обновлены на :8022/shots/ и в `Pictures/freestart-shots/`).
