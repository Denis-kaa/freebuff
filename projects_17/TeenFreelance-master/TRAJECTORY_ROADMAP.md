...
| **11. (Phase 4c) Parental Gate** ✅ (2026-09-06) | Генератор: +100 родителей (по 2 детей — точное разбиение 200) + консент `platform_rules`; тип `ParentalConsent` + lifecycle (`pending → granted \| denied`, `granted → revoked`); store: `requestConsent` (идемпотентен) / `grantConsent` / `denyConsent` / `revokeConsent` + **гейт в `approveTask`** (аппрув без активного консента отклонён); `widgets/parent-control` + консент-инбокс (решения по деньгам — единственная родительская action-поверхность); смоук 82 инварианта (+24) | `tsc` clean + SMOKE PASSED |
| **12. (pompts_11/122) Freeстарт-рework** ✅ (2026-09-06) | Концепт-презентация по `FreeStart_Concept.docx`: `shared/concept/content.ts` (тексты + статусы concept/mechanic/hypothesis/open), `widgets/concept-view` (hero+промо-кадр+видео → диагноз → траектория → механики → бренд → открытые вопросы); брендинг Freeстарт (знак извлечён из промо-кадра, favicon/иконки), index.html/OG, «Траектория» больше не название продукта | `tsc` clean + smoke 82 + `vite build` + прод-чеки на whimco |
| **13. Skill Graph (Этап 3.1)** ✅ (2026-09-06) | Feature layer: `buildSkillGraph` (закрытый словарь 8 навыков, курируемая таблица бустов 9 правил, effective = min(100, round(stored+boost)), pulse при >80, proofs через `Proof.skills?` — аддитивно); `widgets/skill-graph` (радиальный SVG: рёбра бустов, pulse-кольца, reduced-motion) + таблица stored-vs-effective + пикер фрилансера; вид `#skills`; smoke 82→91 (+9 инвариантов графа, поймали баг самосогласованности округления). TanStack Query — отложен: серверного состояния ещё нет (mock-экосистема), подключается при появлении API | `tsc` clean + SMOKE PASSED 91 + прод-деплой :8022 |
| **12. (Phase 4d) Остальной интерактив** | Skill Graph (пульсация >80), TanStack Query, port Tech Blog / Live feed | `npm run dev` + e2e-смоук |
