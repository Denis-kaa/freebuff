...
_DEFAULT_WORKS: tuple[WorkPrice, ...] = (
    ...
    # S2 (решение Дениса 2026-09-12, РОАДМАП_v7 §10): запись нужна, чтобы
    # флаг work_plotter_cut был валидным входом wide (validate по FieldSpec)
    # и триггером задания OP-22.
    # Ревью цены (решение Дениса 2026-09-12, РОАДМАП_v7 §10): цена —
    # НАСТРАИВАЕМЫЙ параметр (инъекция WideConfig, см. golden-тест
    # works-инъекции), дефолт осознанно 0/0 — цифры вносит владелец прайсом.
    # Формула зафиксирована unit="m2" (wide_format.py:711-713 → compute
    # _work_quantity: количество = тиражная площадь ширина×высота×тираж),
    # т.е. цена растёт с размером И тиражом — в отличие от unit="шт",
    # который игнорирует размер наклейки. Реальный якорь владельца
    # (wide_format_config.json, GUI-редактор): «Резка по контуру (плёнка,
    # баннер, бэклит, бумага)» м² cost=100 sell=500 — рекомендуемая точка
    # отсчёта при наполнении прайса (выборка и накатка — отдельные работы).
    WorkPrice("Плоттерная резка", unit="m2", cost=0.0, sell=0.0),
)
