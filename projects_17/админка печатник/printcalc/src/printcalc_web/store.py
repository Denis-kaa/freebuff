...
def price_template_csv(conn: sqlite3.Connection) -> str:
    ...
    lines: list[str] = [
        ...
        "# Цена 0 = «по запросу» (цена не опубликована) — заполните её и импортируйте обратно.",
...
DEFAULT_OPERATIONS: tuple[dict[str, Any], ...] = (
    ...
    # S2 (решение S0, PHASE_RULES_R0_REPORT §3): «Плоттерная резка» — контурная/поштучная
    # резка плёнки на плоттере. Код OP-22 закреплён схемой паков (не OP-15 — тот зарезервирован
    # «Накаткой на основу»). Триггер — новый флаг доп-работы wide `work_plotter_cut`
    # (как work_eyelets/work_hemming: если в заказе есть confirmed-флаг — задание создаётся).
    # Цена работы — НЕ здесь (WORK_PRICES канона wide — на ревью S2, решение Дениса:
    # «без цены в v1», РОАДМАП_v7 §10).
    {"code": "OP-22", "name": "Плоттерная резка", "description": "Контурная/поштучная резка плёнки на плоттере",
     "steps": ["Проверить контур в макете", "Настроить плоттер", "Резать по контуру", "Выборка лишнего"],
     "checklist": ["контур замкнут в макете", "рез идёт по контуру", "плёнка не порвана"],
     "equipment": "режущий плоттер", "minutes": 30, "trigger": "work_plotter_cut", "position": 5},
