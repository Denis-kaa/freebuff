...
def test_calculators_endpoint_lists_all(client: ASGITestClient) -> None:
    ...
    ids = [spec["id"] for spec in data["calculators"]]        # Digital + Riso + таблички + широкоформат (этап B) + Вывески + ЧПУ + Дизайн + Себестоимость.
    assert ids == ["cnc", "cost", "design", "digital", "riso", "sign", "tablichki", "wide"]
