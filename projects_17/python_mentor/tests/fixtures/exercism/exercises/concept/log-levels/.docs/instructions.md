...
Each log line is a string formatted as follows: `"[<LVL>***REMOVED***: <MESSAGE>"`.
...
```python
parse_log_level("[XYZ***REMOVED***: Overly specific, out of context message")
#=> LogLevel.Unknown
```

## 3. Convert a log message to the short format

The log level of a log line is quite verbose. To reduce the disk space needed to store the log messages, a short format is defined: `"[<CODE_LEVEL>***REMOVED***:<MESSAGE>"`.
...
# ('Error', 'ERR'), ('Fatal', 'FTL'), ('Unknown', 'UKN')***REMOVED***
```
