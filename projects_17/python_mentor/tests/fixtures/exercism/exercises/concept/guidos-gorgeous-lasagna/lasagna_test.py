...
# Here, we are separating the constant import errors from the function name import errors
    ...
    if 'EXPECTED_BAKE_TIME' in item_name:
        ...
        raise ImportError(f'\n\nMISSING CONSTANT --> \nWe can not find or import the constant {item_name***REMOVED*** in your'
...
class LasagnaTest(unittest.TestCase):
    ...
    def test_bake_time_remaining(self):
        input_data = [1, 2, 5, 10, 15, 23, 33, 39***REMOVED***
        result_data = [39, 38, 35, 30, 25, 17, 7, 1***REMOVED***

        for variant, (time, expected) in enumerate(zip(input_data, result_data), start=1):
            with self.subTest(f'variation #{variant***REMOVED***', time=time, expected=expected):
                actual_result = bake_time_remaining(time)
                failure_msg = (f'Called bake_time_remaining({time***REMOVED***). ' 
                               f'The function returned {actual_result***REMOVED***, but the tests '
                               f'expected {expected***REMOVED*** as the remaining bake time.')

                self.assertEqual(actual_result, expected, msg=failure_msg)

    @pytest.mark.task(taskno=3)
    def test_preparation_time_in_minutes(self):
        input_data = [1, 2, 5, 8, 11, 15***REMOVED***
        result_data = [2, 4, 10, 16, 22, 30***REMOVED***

        for variant, (layers, expected) in enumerate(zip(input_data, result_data), start=1):
            with self.subTest(f'variation #{variant***REMOVED***', layers=layers, expected=expected):
                actual_result = preparation_time_in_minutes(layers)
                failure_msg = (f'Called preparation_time_in_minutes({layers***REMOVED***). '
                               f'The function returned {actual_result***REMOVED***, but the tests '
                               f'expected {expected***REMOVED*** as the preparation time.')
    ...
    def test_elapsed_time_in_minutes(self):
        ...
        result_data = [5, 11, 18, 20, 37, 50***REMOVED***

        for variant, (layers, time, expected) in enumerate(zip(layer_data, time_data, result_data), start=1):
            with self.subTest(f'variation #{variant***REMOVED***', layers=layers, time=time, expected=expected):
                actual_result = elapsed_time_in_minutes(layers, time)
                failure_msg = (f'Called elapsed_time_in_minutes({layers***REMOVED***, {time***REMOVED***). '
                               f'The function returned {actual_result***REMOVED***, but the tests '
                               f'expected {expected***REMOVED*** as the elapsed time.')
    ...
    def test_docstrings_were_written(self):
        ...
        """
        functions = [bake_time_remaining, preparation_time_in_minutes, elapsed_time_in_minutes***REMOVED***

        for variant, function in enumerate(functions, start=1):
            with self.subTest(f'variation #{variant***REMOVED***', function=function):
                actual_result = function.__doc__
                failure_msg = (f'Called {function.__name__***REMOVED***.__doc__. {actual_result***REMOVED*** was returned, '
                               f'but the tests expected a docstring for the {function.__name__***REMOVED*** function.')
