...
def test_attach_to_order_returns_existing_pending(
    ...
    product = services["catalog"***REMOVED***.seller_create(sid, "PA", "d", "x", 50)
    repo.add_keys(product.id, ["PA-1"***REMOVED***)
...
def test_finalize_on_paid_order_is_no_op(services, make_seller, make_buyer):
    ...
    product = services["catalog"***REMOVED***.seller_create(sid, "FN", "d", "x", 700)
    repo.add_keys(product.id, ["FN-1"***REMOVED***)
    order, _ = orders.create_order_for_product(bid, product.id)
    ...
    services["delivery"***REMOVED***.publish(order.id)
...
def test_attach_to_order_after_succeeded_raises(services, make_seller, make_buyer):
    ...
    product = services["catalog"***REMOVED***.seller_create(sid, "AS", "d", "x", 100)
    repo.add_keys(product.id, ["AS-1"***REMOVED***)
    order, _ = orders.create_order_for_product(bid, product.id)
    payment = payments.attach_to_order(order)
