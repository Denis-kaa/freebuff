...
def test_full_path_pending_paid_delivered(services, make_seller, make_buyer):
    ...
    delivery = services["delivery"***REMOVED***

    sid = make_seller(900, "S9")
    bid = make_buyer(9000, "B9")
    product = services["catalog"***REMOVED***.seller_create(sid, "Steam 50$", "Card", "steam", 500)
    repo.add_keys(product.id, ["DLV-1"***REMOVED***)
    ...
    assert all(r["status"***REMOVED*** == "delivered" for r in rows)
...
def test_mark_paid_is_idempotent(services, make_seller, make_buyer):
    ...
    product = services["catalog"***REMOVED***.seller_create(sid, "X", "d", "x", 10)
...
def test_cancel_paid_is_refused(services, make_seller, make_buyer):
    ...
    product = services["catalog"***REMOVED***.seller_create(sid, "Y", "d", "x", 10)
    repo.add_keys(product.id, ["k2"***REMOVED***)
    order, _ = orders.create_order_for_product(bid, product.id)
    orders.mark_paid(order.id)
...
def test_cancel_pending_releases_key(services, make_seller, make_buyer):
    ...
    product = services["catalog"***REMOVED***.seller_create(sid, "Z", "d", "x", 10)
    repo.add_keys(product.id, ["k3"***REMOVED***)
    order, _ = orders.create_order_for_product(bid, product.id)
