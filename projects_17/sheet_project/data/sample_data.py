...
# Именованные коллекции (map[collection_name -> list[Record]]).
...
def get_collections() -> dict[str, list[Record]]:
    """Вернуть все именованные коллекции (записи неизменяемы по конвенции)."""
    return COLLECTIONS


def get_rows(source: str) -> list[Record]:
    ...
    if source not in COLLECTIONS:
        raise DataValidationError(f"Неизвестная коллекция '{source}'")
    return COLLECTIONS[source]


__all__ = ["COLLECTIONS", "get_collections", "get_rows"]
