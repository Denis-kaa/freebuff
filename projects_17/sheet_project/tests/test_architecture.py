...
def test_generator_does_not_import_project_dashboard():
    ...
    offenders = [str(f) for f in files if _imports_module(f, "config.project_dashboard")]
    assert not offenders, f"Ядро импортирует шаблон (нарушение R2): {offenders}"
...
def test_openpyxl_isolation():
    ...
    assert not offenders, f"openpyxl вне generator/validator (нарушение R5): {offenders}"
    ...
    for dirname in ("validator", "config", "data", "styles"):
        for f in _py_files(PROJECT_ROOT / dirname):
            if _imports_under(f, "generator"):
                offenders.append(str(f))
    assert not offenders, f"Запрещённый импорт generator/* из нижнего слоя: {offenders}"
...
def test_config_swap_changes_output_without_core_edit(tmp_path, monkeypatch):
    ...
    def make_config(sheet_name: str, columns: list[str]) -> Workbook:
        sheet = Sheet(
            name=sheet_name,
            columns=[Field(name=c, type=FieldType.TEXT) for c in columns],
            data_source=DataSource(source="rows", field_map={c: c for c in columns}),
        )
        return Workbook(name="cfg", template_id="cfg", template_version="1.0", sheets=[sheet])
    ...
    artifact_a = generate(make_config("ProjectsA", ["id", "name"]), data, theme=None, output_name="a.xlsx")
    artifact_b = generate(make_config("ProjectsB", ["name", "status"]), data, theme=None, output_name="b.xlsx")
    ...
    header_a = [c.value for c in wb_a.active[1]]
    header_b = [c.value for c in wb_b.active[1]]
