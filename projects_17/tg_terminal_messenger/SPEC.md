...
- 📎 **Медиа:** отправка файлов через `FilePicker` — планируется (в MVP не реализовано; Ctrl+F в текущей версии = поиск по чатам)
- 📦 **Архивация:** экспорт истории, скачивание медиа по `chat_id` с прогресс-баром — планируется
