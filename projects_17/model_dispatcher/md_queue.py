...
_TS_RE = re.compile(r"^(\d{8***REMOVED***_\d{6***REMOVED***)_")
...
def queue_dirs(cfg: Dict[str, Any***REMOVED*** | None = None) -> Dict[str, Path***REMOVED***:
    """Возвращает {user, running, done, failed***REMOVED*** как Path (создаёт при отсутствии)."""
    root = resolve_root()
    cfg = cfg or {***REMOVED***
    q = cfg.get("queue", {***REMOVED***)
    names = {
...
class PromptMeta:
    ...
    chat_id: str = ""
    created_at: str = ""
    headers: Dict[str, str***REMOVED*** = field(default_factory=dict)
...
def parse_prompt(path: Path) -> Optional[PromptMeta***REMOVED***:
    ...
    title = ""
    headers: Dict[str, str***REMOVED*** = {***REMOVED***
    body_lines: List[str***REMOVED*** = [***REMOVED***
    in_body = False
    for ln in lines:
        if in_body:
        ...
        if ln.startswith("# TASK:"):
            title = ln[len("# TASK:"):***REMOVED***.strip()
        m = re.match(r"^\*\*([^****REMOVED***+):\*\*\s*(.*)$", ln.strip())
        if m:
            headers[m.group(1).strip()***REMOVED*** = m.group(2).strip()
        if ln.strip() == "---":
            in_body = True
    if not title:
        return None
...
def _derive_task_id(path: Path) -> str:
    ...
    if m:
        return m.group(1)
    return uuid.uuid4().hex[:12***REMOVED***
...
def new_prompt_file(
    ...
    cfg: Dict[str, Any***REMOVED*** | None = None,
) -> Path:
    ...
    tid = uuid.uuid4().hex[:6***REMOVED***
    path = dirs[queue_status***REMOVED*** / f"task_{ts***REMOVED***_{tid***REMOVED***.md"
    stamp = datetime.now(timezone.utc).isoformat(timespec="seconds")
    content = (
        f"# TASK: {title***REMOVED***\n"
        f"**Task ID:** {tid***REMOVED***\n"
        f"**Status:** {status***REMOVED***\n"
        f"**Model:** {model***REMOVED***\n"
        f"**Created At:** {stamp***REMOVED***\n"
        f"---\n"
        f"{text***REMOVED***\n"
    )
...
def scan(status: str = "user", cfg: Dict[str, Any***REMOVED*** | None = None) -> List[PromptMeta***REMOVED***:
    """Сканирует папку очереди (user/running/done/failed), сортирует по имени."""
    dirs = queue_dirs(cfg)
    folder = dirs.get(status, dirs["user"***REMOVED***)
    metas = [***REMOVED***
...
def move_to_status(path: Path, status: str, cfg: Dict[str, Any***REMOVED*** | None = None) -> Path:
    """Перемещает файл промта в папку статуса (atomic rename)."""
    dirs = queue_dirs(cfg)
    target = dirs.get(status, dirs["user"***REMOVED***) / path.name
    path.rename(target)
    return target
...
def queue_counts(cfg: Dict[str, Any***REMOVED*** | None = None) -> Dict[str, int***REMOVED***:
    """Счётчик файлов по папкам очереди."""
    dirs = queue_dirs(cfg)
    counts = {***REMOVED***
    for status, folder in dirs.items():
        counts[status***REMOVED*** = sum(1 for p in folder.glob("*.md") if not p.name.startswith("."))
    return counts
