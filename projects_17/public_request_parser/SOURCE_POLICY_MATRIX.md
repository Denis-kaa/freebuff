...
| SRC-013 | **SuperJob API**: `api.superjob.ru` (поиск вакансий) | Официальный HTTPS API, регистрация приложения → ключ (секрет) | Официальные страницы API (`superjob.ru/info/api_service.html`, `api.superjob.ru`), API-оферта `api_oferta.html`; 800k+ вакансий; публикация и выборка вакансий | Требует соглашения (оферта) и применения по назначению; поля и лимиты нужно задокументировать | Высокая: реальные вакансии (спрос), официальный механизм | `conditional` (после изучения оферты + регистрации приложения) |
| SRC-014 | **Avito API**: `developers.avito.ru` / `avito.ru/developers` | Официальный публичный API, регистрация приложения → ключ | Портал разработчика существует; `avito.ru/legal/pro_tools/public-api`; API в основном для управления СВОИМИ объявлениями/вакансиями и чатами (тарифы) | Чтение чужих вакансий через API — не основной сценарий; ограничения и тарифы нужно подтвердить; запрет скрейпинга актуален | Средняя: работа/вакансии есть, но доступ к чужим объявлениям ограничен | `manual_review` (нужен точный scope и разрешение чтения) |
| SRC-015 | **Zarplata.ru API**: `api.zarplata.ru` (OpenAPI redoc) | Официальный документированный API | Существует OpenAPI-документация (`api.zarplata.ru/openapi/redoc`, github.com/zarplata/api), есть публичная исторзия использования `api.zp.ru/v1` | Проверить текущие Terms/лимиты API и допустимый чтение вакансий | Средняя: близкая альтернатива HH/SuperJob | `manual_review` (проверить Terms + endpoint) |
| SRC-016 | **Фриланс-биржи: Kwork / FL.ru / YouDo / Profi.ru / Freelance.ru** | Веб/SPA; отдельные неофициальные обёртки | Kwork — нет официального API; неофиц. библиотеки (kwork-api) используют скрытый/приватный API, работают через прокси и ловятся капчей/банами; FL.ru/YouDo/Profi — документированного публичного API для чтения заказов нет | Использование неофициальных обёрток нарушает запрет hidden/private API (наш инвариант); ToS площадок не допускают скрейпинг заказов | Продуктово релевантны (там заказчики пишут заявки), но легального механизма нет | `policy_blocked` — до появления официального API/партнёрской программы |: `https://api.hh.ru/vacancies` | Официальный HTTPS API, приложение одобрено (#22931); ключ → env/secret storage | Документированный OpenAPI (`api.hh.ru/openapi/redoc`); live-проверка 2026-08-23: HTTP 200, `text=python` → 6629 вакансий; поля: id, name, description, alternate_url, published_at, employment, experience, salary, area, snippet | **Evidence:**
1.1** — регистрация Приложения на dev.hh.ru обязательна → уникальный API-ключ; **1.5/1.6** — использование сервиса допустимо для целей привлечения работников и трудоустройства; **3.3** — использование материалов допустимо только в целях, соответствующих тематике Сайта (поиск работы/сотрудников/рынок труда); **3.4** — запрет на использование товарных знаков HeadHunter; **3.6** — запрет сбора логинов/паролей; **3.7** — нельзя предоставлять доступ к резюме; **3.11** — запрещено вносить изменения в материалы | Высокая: вакансии = «работодатель ищет исполнителя» (спрос на услуги/работу), фильтры по ключевым словам, `employment_type=project/part_time`; резюме/контакты соискателей не используются | `allowed` (приложение одобрено #22931; ключ+redirect в `hh/info.md` — ignored; live-проверка 2026-08-23 OK; соблюдение §1.5/1.6/3.3/3.11; TTL текста ограничен; никогда не включать резюме/соискателей и не изменять текст) |
...
### SuperJob (SRC-013 candidate)

- [API SuperJob***REMOVED***(https://www.superjob.ru/info/api_service.html) — официальная страница API, 800k+ вакансий.
- [api.superjob.ru***REMOVED***(https://api.superjob.ru/) — портал клиента API.
- [API-оферта***REMOVED***(https://www.superjob.ru/info/api_oferta.html) — условия использования Базы данных SuperJob.

### Avito (SRC-014 candidate)

- [Портал разработчика Avito***REMOVED***(https://www.avito.ru/developers) — каталог API, регистрация приложения.
- [Публичный Avito API — правовой документ***REMOVED***(https://www.avito.ru/legal/pro_tools/public-api) — доступ к API по заявке/ключу.
- [API Avito для бизнеса***REMOVED***(https://www.avito.ru/business/tools/api) — работа с вакансиями (для работодателей).

### Zarplata.ru (SRC-015 candidate)

- [OpenAPI redoc zarplata.ru***REMOVED***(https://api.zarplata.ru/openapi/redoc) — актуальная документация API.
- [github.com/zarplata/api***REMOVED***(https://github.com/zarplata/api) — репозиторий документации.

### Фриланс-биржи (SRC-016)

- [github.com/CrazyMan-IK/kwork-api***REMOVED***(https://github.com/CrazyMan-IK/kwork-api) — неофициальная обёртка: использует скрытый API, требует прокси, возможны баны по IP → подтверждает `policy_blocked`.
- [Пользовательское соглашение Kwork***REMOVED***(https://kwork.ru/terms_of_service).
- YouDo/FL.ru/Profi.ru — публичного API для чтения заказов не обнаружено (iscледование 2026-08-23).
